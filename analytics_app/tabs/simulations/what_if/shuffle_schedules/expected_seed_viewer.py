...
def _select_week(base_df):
    ...
    if mode == "Today's Date":
        ...
        year_choice = c_year.selectbox(
            "Year", ["Select Year"] + [str(y) for y in years], key="exp_seed_year"
        )
        ...
        week_choice = c_week.selectbox(
            "Week", ["Select Week"] + [str(w) for w in weeks], key="exp_seed_week"
        )
...
def _render_expected_seed(base_df, year, week):
    ...
    seed_cols = [
        c for c in all_cols if c.startswith("shuffle_") and c.endswith("_seed")
    ]
