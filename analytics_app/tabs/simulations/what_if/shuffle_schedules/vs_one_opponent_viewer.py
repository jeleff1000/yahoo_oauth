...
class VsOneOpponentViewer:
    ...
    def display(self):
        ...
        st.caption(
            "What if you played every opponent for an entire season? Shows simulated matchup records."
        )
        ...
        with col3:
            include_regular_season = st.checkbox(
                "Regular", value=True, key="include_regular_season"
            )

        with col4:
            include_postseason = st.checkbox(
                "Playoffs", value=False, key="include_postseason"
            )
    ...
    def display_record(self, filtered_df):
        ...
        win_cols = [
            c for c in all_cols if c.startswith("w_vs_") and not c.endswith("_sched")
        ]
        loss_cols = [
            c for c in all_cols if c.startswith("l_vs_") and not c.endswith("_sched")
        ]
        ...
        for opponent in valid_opponents:
            ...
            for manager in managers:
                ...
                wins = (
                    int(manager_data[w_col].sum())
                    if w_col in manager_data.columns
                    else 0
                )
                losses = (
                    int(manager_data[l_col].sum())
                    if l_col in manager_data.columns
                    else 0
                )
        ...
        st.caption(
            "💡 **How to read:** Each row shows a manager's simulated record against every opponent. Green = winning record, red = losing record."
        )
    ...
    def display_win_percentage(self, filtered_df):
        ...
        st.caption(
            "💡 **How to read:** Percentage of simulated games won by each manager against each opponent. Higher is better."
        )
    ...
    def display_games_played(self, filtered_df):
        ...
        st.caption(
            "💡 **How to read:** Total simulated games between each manager and opponent."
        )
