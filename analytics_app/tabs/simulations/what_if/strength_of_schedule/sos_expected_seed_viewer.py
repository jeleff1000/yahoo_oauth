...
def _render_expected_seed(base_df, year, week):
    ...
    seed_cols = [
        c for c in all_cols if c.startswith("opp_shuffle_") and c.endswith("_seed")
    ]
    ...
    if "playoff_seed_to_date" in week_df.columns:
        actual_seed = (
            week_df[["manager", "playoff_seed_to_date"]]
            .drop_duplicates(subset=["manager"])
    ...
    st.caption(
        "Probability of finishing at each playoff seed based on opponent strength."
    )
    ...
    st.caption(
        "💡 **How to read:** Each numbered column shows % chance of that seed. Bye = top 2 seeds, Playoff = top 6 seeds."
    )
