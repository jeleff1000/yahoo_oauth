...
def _build_year_tab(df: pd.DataFrame):
    ...
    selected_year_label = st.selectbox(
        "Select Year", year_options, index=default_index, key="opp_gavi_year_sel"
    )
    ...
    show_charts = st.checkbox(
        "📊 Show Visualizations", value=True, key="show_opp_gavi_charts"
    )
...
class OpponentGaviStatViewer:
    ...
    def display(self):
        ...
        st.caption(
            "How would each manager's opponents perform against the rest of the league?"
        )
