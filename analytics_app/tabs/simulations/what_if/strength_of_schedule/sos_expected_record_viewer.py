...
def _select_week(base_df):
    ...
    if mode == "Today's Date":
        ...
        year_choice = c_year.selectbox(
            "Year", ["Select Year"] + [str(y) for y in years], key="opp_exp_record_year"
        )
        ...
        week_choice = c_week.selectbox(
            "Week", ["Select Week"] + [str(w) for w in weeks], key="opp_exp_record_week"
        )
...
def _render_expected_record(base_df, year, week):
    ...
    shuffle_cols = [
        c for c in all_cols if c.startswith("opp_shuffle_") and c.endswith("_win")
    ]
    ...
    # Vectorized renaming
    rename_map = {
        c: f"{int(c.split('_')[2])}-{week - int(c.split('_')[2])}" for c in shuffle_cols
    }
    ...
    st.caption(
        "Probability of achieving each win-loss record based on opponent strength."
    )
    ...
    st.caption(
        "💡 **How to read:** Each column shows % chance of that record. Higher % in winning records = easier schedule."
    )
