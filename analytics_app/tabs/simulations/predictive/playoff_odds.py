...
class PlayoffOddsViewer:
    ...
    def display(self):
        ...
        st.info(
            "Monte Carlo simulation of remaining games to project playoff and championship odds"
        )
        ...
        # Get odds data
        odds = df[
            (df["year"] == season) & (df["is_playoffs"] == 0) & (df["week"] == week)
        ]
        ...
        with col1:
            avg_playoff_pct = (
                odds_table["p_playoffs"].mean()
                if "p_playoffs" in odds_table.columns
                else 0
            )
        ...
        show_viz = st.checkbox(
            "📊 Show Visualizations", value=True, key="show_playoff_viz"
        )
        ...
        if "p_champ" in odds_table.columns:
            ...
            with col1:
                ...
                fig_champ.update_layout(
                    title="Championship Probability Distribution", height=400
                )
