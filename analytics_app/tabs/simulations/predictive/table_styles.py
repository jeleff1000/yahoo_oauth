...
def render_modern_table(
    ...
    for col in display_df.columns:
        if (
            col not in ["Manager", "Year"]
            and col in display_df.select_dtypes(include=["number"]).columns
        ):
    ...
    custom_css = {
        ...
        ".ag-row-even": {"background-color": "rgba(0, 0, 0, 0.02)"},
        ".ag-row-odd": {"background-color": "transparent"},
        # Hover effect
        ".ag-row:hover": {"background-color": "rgba(102, 126, 234, 0.08) !important"},
        # Mobile optimizations
        "@media (max-width: 768px)": {
            ".ag-header-cell": {"font-size": "0.75em", "padding": "3px 4px"},
            ".ag-cell": {"font-size": "0.75em", "padding": "3px 4px"},
        },
    }
