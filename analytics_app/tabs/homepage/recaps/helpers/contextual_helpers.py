...
def get_optimal_lineup_message(row: pd.Series) -> Optional[str]:
    ...
    if margin is not None and opponent_points is not None:
        ...
        could_have_won = (margin < 0) and (
            team_points + potential_swing > opponent_points
        )
        near_flip = (margin < 0) and (
            team_points + potential_swing >= opponent_points - 3
        )
    ...
    if win_flag is False and could_have_won:
        ...
        return format_dialogue(
            template, optimal_diff=optimal_diff, margin_pos_str=margin_pos_str
        )
        ...
        return format_dialogue(
            template, optimal_diff=optimal_diff, optimal_pct=optimal_pct
        )
...
def get_optimal_record_message(row: pd.Series) -> Optional[str]:
    ...
    optimal_comment = (
        "You're maximizing your roster!"
        if optimal_win == win
        else "Better lineup decisions could improve your record."
    )
...
def get_weekly_ranking_message(row: pd.Series) -> Optional[str]:
    ...
    opp_pts_week_pct * 100 if opp_pts_week_pct <= 1 else opp_pts_week_pct
...
def get_alternate_schedule_message(row: pd.Series) -> Optional[str]:
    ...
    playoff_pct = (
        shuffle_avg_playoffs * 100
        if shuffle_avg_playoffs <= 1
        else shuffle_avg_playoffs
    )
    ...
    if shuffle_avg_wins is not None:
        ...
        msg += " " + format_dialogue(
            wins_template, shuffle_avg_wins=shuffle_avg_wins, wins_to_date=wins_to_date
        )
...
def get_milestone_message(
    row: pd.Series, prev_row: Optional[pd.Series] = None
) -> Optional[str]:
...
def get_player_percentile_message(player_row: pd.Series) -> Optional[str]:
    ...
    percentile = None
    position = _safe_get(
        player_row, "fantasy_position", _safe_get(player_row, "position", "PLAYER")
    )
    player_name = _safe_get(
        player_row, "player", _safe_get(player_row, "player_name", "Player")
    )
    ...
    return format_dialogue(
        template, player=player_name, position=position, percentile=pct_value
    )
...
def build_player_spotlight_lines(player_row: pd.Series) -> List[str]:
    ...
    if _truthy(_safe_get(player_row, "number_one_player")):
        ...
        if points is not None:
            lines.append(
                format_dialogue(n1_templates["league_week"], player=name, points=points)
            )
        else:
    ...
    if _truthy(_safe_get(player_row, "number_one_position")):
        lines.append(
            format_dialogue(
                n1_templates["position_week"], player=name, position=position
            )
        )
    if _truthy(_safe_get(player_row, "number_one_position_season")):
        lines.append(
            format_dialogue(
                n1_templates["position_season"], player=name, position=position
            )
        )
...
def generate_all_contextual_messages(
    row: pd.Series, prev_row: Optional[pd.Series] = None
...
def compose_weekly_paragraph(row: pd.Series) -> Optional[str]:
    ...
    g("team_name", g("manager", "Your team"))
    ...
    if spread is not None:
        if (spread < 0 and margin is not None and margin < 0) or (
            spread > 0 and margin is not None and margin > 0
        ):
...
def compose_season_paragraph(row: pd.Series) -> Optional[str]:
    ...
    wins = _safe_float(g("wins_to_date"))
    losses = _safe_float(g("losses_to_date"))
    ...
    if wins is not None and losses is not None:
        if seed is not None:
            lines.append(
                f"You're {int(wins)}-{int(losses)} and sit at seed #{int(seed)} right now."
            )
        else:
    ...
    if p_playoffs is not None:
        lines.append(
            f"Playoff odds sit at {pct_str(p_playoffs)}{'' if p_bye is None else f', with a {pct_str(p_bye)} shot at a bye' }."
        )
    ...
    if wins_vs_shuffle is not None:
        if wins_vs_shuffle >= 1.0:
            ...
            lines.append(
                f"Schedule has cost you about {abs(wins_vs_shuffle):.1f} wins."
            )
        else:
    ...
    if shuffle_pct is not None:
        lines.append(
            f"Across shuffled schedules, you'd make the playoffs {pct_str(shuffle_pct)} of the time."
        )
...
def build_player_spotlight_paragraph(
    ...
    *,
    ...
    name_candidates = ["player", "Player", "player_name", "Player_Name", "name", "Name"]
    ...
    for idx, row in top_df.iterrows():
        ...
        pts_txt = (
            f"{pts:.1f}" if isinstance(pts, (int, float)) and not pd.isna(pts) else "—"
        )
    ...
    if isinstance(improved_row, pd.Series) and not improved_row.empty:
        imp_name = str(
            _safe_get(
                improved_row, name_col, _safe_get(improved_row, "player", "A player")
            )
        )
        ...
        if delta is not None and delta > 0:
            tail_bits.append(
                f"Most Improved: {imp_name} (+{delta:.1f} pts week-over-week)."
            )
