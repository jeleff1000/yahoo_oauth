...
def display_manager_draft_grades(draft_data: pd.DataFrame) -> None:
    ...
    if missing:
        st.warning(
            f"Missing required columns: {', '.join(missing)}. Run the draft_value_metrics_v3 pipeline to generate these metrics."
        )
        return
    ...
    manager_cols = [
        ...
        "manager_draft_score",
        "manager_draft_grade",
        "manager_draft_percentile",
        "manager_draft_percentile_year",
    ...
    manager_df = (
        df[df["manager"].notna() & (df["manager"] != "")]
        .groupby(["manager", "year"])
        .first()
        .reset_index()
    )
    ...
    if manager_df.empty:
        st.warning(
            "No manager draft grades available. Run the draft enrichment pipeline."
        )
        return
    ...
    with st.expander("🔍 Filters", expanded=False):
        ...
        with col1:
            ...
            selected_years = st.multiselect(
                "Year", years, default=years, key="mgr_grade_years"
            )

        with col2:
            managers = sorted(manager_df["manager"].unique().tolist())
            selected_managers = st.multiselect(
                "Manager", managers, default=[], key="mgr_grade_managers"
            )
    ...
    with col2:
        avg_spar = (
            filtered["manager_total_spar"].mean()
            if "manager_total_spar" in filtered.columns
            else 0
        )
    ...
    with col3:
        ...
        avg_hit_rate = (
            filtered["manager_hit_rate"].mean()
            if "manager_hit_rate" in filtered.columns
            else 0
        )
    ...
    view = st.radio(
        "View",
        ["Leaderboard", "Year-over-Year", "Career Summary"],
        horizontal=True,
        key="mgr_grade_view",
    )
...
def display_leaderboard(df: pd.DataFrame) -> None:
    ...
    if "All-Time %ile" in display_df.columns:
        display_df["All-Time %ile"] = (
            display_df["All-Time %ile"].round(1).astype(str) + "%"
        )
    ...
    styled = display_df.style.applymap(
        style_grade, subset=["Grade"] if "Grade" in display_df.columns else []
    )
    ...
    if "Grade" in display_df.columns:
        ...
        with col1:
            ...
            leaderboard["_base_grade"] = leaderboard["manager_draft_grade"].apply(
                get_base_grade
            )
            grade_counts = (
                leaderboard["_base_grade"]
                .value_counts()
                .reindex(["A", "B", "C", "D", "F"])
                .fillna(0)
            )
        ...
        with col2:
            ...
            fig = px.histogram(
                leaderboard,
                x="manager_draft_score",
                nbins=15,
                color_discrete_sequence=["#667eea"],
            )
            fig.update_layout(
                xaxis_title="Draft Score", yaxis_title="Count", height=300
            )
            st.plotly_chart(fig, use_container_width=True)
...
def display_year_over_year(df: pd.DataFrame) -> None:
    ...
    pivot_data = df.pivot_table(
        index="manager", columns="year", values="manager_draft_grade", aggfunc="first"
    )
    ...
    if "manager_draft_score" in df.columns:
        fig = px.line(
            ...
            labels={
                "year": "Year",
                "manager_draft_score": "Draft Score",
                "manager": "Manager",
            },
        )
...
def display_career_summary(df: pd.DataFrame) -> None:
    ...
    fig.update_layout(
        xaxis_title="Manager", yaxis_title="Draft Score", barmode="group", height=400
    )
