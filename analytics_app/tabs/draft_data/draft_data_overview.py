...
def validate_and_prep_data(draft_data: pd.DataFrame) -> pd.DataFrame:
    ...
    if "is_keeper_cost" not in draft_data.columns or df["is_keeper_cost"].isna().all():
        if "is_keeper_status" in df.columns and "cost" in df.columns:
            try:
                df["is_keeper_cost"] = (
                    df["cost"].where(df["is_keeper_status"] == 1, 0).fillna(0)
                )
            except Exception:
                df["is_keeper_cost"] = 0
...
@st.fragment
def display_draft_data_overview(
    ...
    try:
        ...
        try:
            ...
            st.dataframe(
                (draft_data.head(20) if hasattr(draft_data, "head") else draft_data)
            )
        except Exception:
            pass
        return
    ...
    with st.expander("🔎 Filters", expanded=False):
        try:
            ...
            with c3:
                positions = (
                    sorted(df["yahoo_position"].dropna().unique().tolist())
                    if "yahoo_position" in df.columns
                    else []
                )
                selected_positions = st.multiselect(
                    "Position", options=positions, placeholder="All", key="draft_pos"
                )

            # Row 2: Manager
            managers = (
                sorted(df["manager"].dropna().astype(str).unique().tolist())
                if "manager" in df.columns
                else []
            )
            selected_managers = st.multiselect(
                "Manager", options=managers, placeholder="Manager", key="draft_mgr"
            )
            ...
            try:
                mask = (
                    (df["year"] >= start_year)
                    & (df["year"] <= end_year)
                    & (df["yahoo_position"].isin(filter_positions))
                    & (df["manager"].isin(filter_managers))
                )
            ...
            st.warning(f"Could not show filters: {e}")
            {
                ...
                "positions": (
                    df["yahoo_position"].unique().tolist()
                    if "yahoo_position" in df.columns
                    else []
                ),
                "managers": (
                    df["manager"].unique().tolist() if "manager" in df.columns else []
                ),
            }
    ...
    for idx, (col, name) in enumerate(zip(cols, main_tab_names)):
        with col:
            ...
            if st.button(
                name, key=f"draft_main_{idx}", use_container_width=True, type=btn_type
            ):
                if not is_active:
    ...
    if current_main_idx == 0:
        ...
        with overview_tabs[1]:
            ...
            try:
                from .graphs.draft_round_efficiency import (
