...
def build_report_card_html(
    ...
    for pick in picks_data:
        ...
        if pick["headshot_url"]:
            ...
            headshot_html = (
                f'<div class="player-headshot-placeholder">{pick["initials"]}</div>'
            )

        # Add keeper mark if applicable
        keeper_mark = (
            '<span class="keeper-mark">K</span>' if pick.get("is_keeper") else ""
        )
        ...
        grade_class = (
            f"grade-{pick_base_grade}"
            if pick_base_grade in ["A", "B", "C", "D", "F"]
            else ""
        )
...
def display_draft_report_card(draft_data: pd.DataFrame) -> None:
    ...
    if missing:
        st.warning(
            f"Missing required columns: {', '.join(missing)}. Run the draft enrichment pipeline."
        )
        return
    ...
    with col1:
        selected_manager = st.selectbox(
            "Select Manager", managers, key="report_card_manager"
        )
    with col2:
    ...
    # Filter data
    manager_year_df = df[
        (df["manager"] == selected_manager) & (df["year"] == selected_year)
    ].copy()
    ...
    manager_grade = (
        manager_year_df["manager_draft_grade"].iloc[0]
        if "manager_draft_grade" in manager_year_df.columns
        else "N/A"
    )
    manager_percentile = (
        manager_year_df["manager_draft_percentile_alltime"].iloc[0]
        if "manager_draft_percentile_alltime" in manager_year_df.columns
        else None
    )
    total_spar = (
        manager_year_df["manager_total_spar"].iloc[0]
        if "manager_total_spar" in manager_year_df.columns
        else None
    )
    hit_rate = (
        manager_year_df["manager_hit_rate"].iloc[0]
        if "manager_hit_rate" in manager_year_df.columns
        else None
    )
    ...
    if manager_percentile is not None:
        manager_percentile = (
            float(manager_percentile) if pd.notna(manager_percentile) else None
        )
    ...
    is_auction = (
        manager_year_df["cost"].notna().any()
        if "cost" in manager_year_df.columns
        else False
    )
    ...
    for _, row in manager_year_df.iterrows():
        ...
        spar_val = (
            row.get("manager_spar")
            if pd.notna(row.get("manager_spar"))
            else row.get("spar")
        )
        ...
        # Check if keeper
        is_keeper = (
            row.get("is_keeper_status", 0) == 1 or row.get("is_keeper_status")
        )
