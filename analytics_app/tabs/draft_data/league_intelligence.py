...
class LeagueIntelligence:
    ...
    def __init__(
        ...
        self.df[self.spar_col] = pd.to_numeric(
            self.df[self.spar_col], errors="coerce"
        ).fillna(0)
        self.df[self.cost_col] = pd.to_numeric(
            self.df[self.cost_col], errors="coerce"
        ).fillna(0)
    ...
    def _bootstrap_confidence_interval(
        self, values: pd.Series, n_bootstrap: int = 1000, confidence: float = 0.95
    ) -> Tuple[float, float]:
    ...
    def calculate_position_efficiency(self) -> Dict[str, EfficiencyMetric]:
        ...
        for pos in self.positions:
            ...
            pos_df = pos_df.copy()
            pos_df["pick_efficiency"] = pos_df[self.spar_col] / pos_df[
                self.cost_col
            ].clip(lower=0.1)
        ...
        sorted_positions = sorted(
            results.values(), key=lambda x: x.efficiency_ratio, reverse=True
        )
    ...
    def analyze_cheap_pick_strategy(
        self, cheap_threshold: Optional[float] = None
    ) -> Dict[str, Any]:
        """
        ...
        pos_value = (
            cheap_picks.groupby(self.pos_col)
            .agg(
                {
        ...
        manager_year_stats = (
            self.df.groupby([self.manager_col, self.year_col])
            .agg(
                {
                    "manager_total_spar": (
                        "first"
                        if "manager_total_spar" in self.df.columns
                        else (self.spar_col, "sum")
                    )
                }
            )
            .reset_index()
        ...
        cheap_counts = (
            cheap_picks.groupby([self.manager_col, self.year_col])
            .size()
            .reset_index(name="num_cheap")
        )
        manager_year_stats = manager_year_stats.merge(
            cheap_counts, on=[self.manager_col, self.year_col], how="left"
        )
        ...
        if len(manager_year_stats) >= self.MIN_STRATEGY_SAMPLES:
            ...
            if max_cheap <= min_cheap + 1:
                ...
                try:
                    ...
                    bin_performance = (
                        manager_year_stats.groupby("cheap_bin", observed=True)
                        .agg({"manager_total_spar": ["mean", "count", "std"]})
                        .round(1)
                    )
                    ...
                    if not valid_bins.empty:
                        optimal_bin = valid_bins.loc[
                            valid_bins["avg_spar"].idxmax(), "cheap_bin"
                        ]
                    elif not bin_performance.empty:
                        optimal_bin = bin_performance.loc[
                            bin_performance["avg_spar"].idxmax(), "cheap_bin"
                        ]
                except Exception:
                    optimal_bin = f"{min_cheap}-{max_cheap}"

                # Calculate correlation
                correlation = manager_year_stats["num_cheap"].corr(
                    manager_year_stats["manager_total_spar"]
                )
        ...
        result = {
            ...
            "bin_performance": (
                bin_performance.to_dict("records") if not bin_performance.empty else []
            ),
    ...
    def analyze_position_strategy(
        ...
        # Analyze each tier
        tier_stats = (
            pos_df.groupby("tier", observed=True)
            .agg(
                {
                    self.spar_col: ["mean", "median", "count", "std"],
                    self.cost_col: "mean",
        ...
        valid_tiers["spar_per_dollar"] = valid_tiers["avg_spar"] / valid_tiers[
            "avg_cost"
        ].clip(lower=0.5)
        ...
        valid_tiers[valid_tiers["tier"] == "mid"]
        ...
        if not budget_tier.empty:
            budget_tier["spar_per_dollar"].iloc[0]
        ...
        premium_spar = (
            premium_data["avg_spar"].iloc[0] if not premium_data.empty else None
        )
        ...
        return {
            ...
            "spar_advantage_vs_premium": (
                round(spar_advantage, 2) if spar_advantage else None
            ),
            "efficiency_analysis": valid_tiers[
                ["tier", "avg_spar", "avg_cost", "spar_per_dollar"]
            ].to_dict("records"),
            "sample_size": len(pos_df),
            "confidence": self.confidence_level,
        }
    ...
    def analyze_winning_patterns(self, top_percentile: float = 0.8) -> Dict[str, Any]:
        """
        ...
        if "manager_total_spar" in self.df.columns:
            manager_spar = (
                self.df.groupby([self.manager_col, self.year_col])
                .agg({"manager_total_spar": "first"})
                .reset_index()
            )
        else:
            manager_spar = (
                self.df.groupby([self.manager_col, self.year_col])
                .agg({self.spar_col: "sum"})
                .reset_index()
            )
            manager_spar.columns = [
                self.manager_col,
                self.year_col,
                "manager_total_spar",
            ]
        ...
        bottom_threshold = manager_spar["manager_total_spar"].quantile(
            1 - top_percentile
        )

        top_seasons = manager_spar[manager_spar["manager_total_spar"] >= threshold]
        bottom_seasons = manager_spar[
            manager_spar["manager_total_spar"] <= bottom_threshold
        ]
        ...
        for pos in self.positions:
            ...
            differences[pos] = {
                ...
                "recommendation": (
                    ...
                    else ("decrease" if top_pct < bottom_pct - 1 else "neutral")
                ),
            }
    ...
    def profile_managers(self) -> Dict[str, Dict]:
        ...
        for manager in self.df[self.manager_col].unique():
            ...
            # Bust/breakout rates
            bust_rate = (
                mgr_df["is_bust"].mean() * 100 if "is_bust" in mgr_df.columns else None
            )
            breakout_rate = (
                mgr_df["is_breakout"].mean() * 100
                if "is_breakout" in mgr_df.columns
                else None
            )
    ...
    def analyze_slot_efficiency(self) -> Dict[str, Dict]:
        ...
        df_ranked = self.df.groupby(
            [self.manager_col, self.year_col, self.pos_col], group_keys=False
        ...
        for pos in self.positions:
            ...
            pos_df["position_rank"] = (
                pos_df.groupby([self.manager_col, self.year_col])[self.cost_col]
            ...
            for rank in range(1, 6):  # Analyze up to 5 deep at each position
                ...
                pos_median_cost = self.df[self.df[self.pos_col] == pos][
                    self.cost_col
                ].median()
                ...
                results[slot_name] = {
                    ...
                    "recommendation": self._get_slot_recommendation(
                        pos, rank, avg_cost, avg_spar, spar_per_dollar
                    ),
                }

        return results

    def _get_slot_recommendation(
        self,
        pos: str,
        rank: int,
        avg_cost: float,
        avg_spar: float,
        spar_per_dollar: float,
    ) -> str:
        """Generate recommendation for a specific slot based on data, not assumptions."""

        # Calculate league-wide benchmarks for comparison
        league_avg_spar_per_dollar = self.df[self.spar_col].sum() / max(
            self.df[self.cost_col].sum(), 1
        )
    ...
    def calculate_optimal_allocation(
        ...
        if strategy == "league_average":
            ...
            total = pos_spend.sum()
            return {
                pos: round(budget * (spend / total), 2)
            ...
            if patterns.get("confidence") == "insufficient_data":
                return self.calculate_optimal_allocation(
                    budget, roster_structure, "league_average"
                )

            allocation = {}
            for pos, data in patterns["position_differences"].items():
                allocation[pos] = round(
                    budget * data["top_drafter_allocation"] / 100, 2
                )
            return allocation

        else:  # 'beat_league' - exploit inefficiencies
            efficiency = self.calculate_position_efficiency()
            self.analyze_cheap_pick_strategy()
            patterns = self.analyze_winning_patterns()
            ...
            allocation = {
                pos: round(budget * pct, 2) for pos, pct in normalized.items()
            }
    ...
    def analyze(self) -> LeagueInsights:
        ...
        te_strategy = (
            self.analyze_position_strategy("TE") if "TE" in self.positions else {}
        )

        # Calculate optimal allocation - detect budget from data
        # Use the average total spend per manager-year as the budget baseline
        manager_year_spend = self.df.groupby([self.manager_col, self.year_col])[
            self.cost_col
        ].sum()
        ...
        optimal_allocation = self.calculate_optimal_allocation(
            budget=detected_budget, strategy="beat_league"
        )
    ...
    def calculate_mathematical_optimal(
        self, budget: float = None, roster_slots: List[str] = None
    ) -> Dict[str, Any]:
        """
        ...
        if roster_slots is None:
            ...
            roster_slots = [
                s for s in starter_priority + bench_priority if s in slot_curves
            ]
        else:
        ...
        def neg_total_spar(allocations):
            return -sum(
                expected_spar(allocations[i], slot_curves[slot])
        ...
        bounds = [
            (max(1, slot_curves[slot]["min_cost"]), slot_curves[slot]["max_cost"])
        ...
        # Clip to bounds
        initial = [
            max(bounds[i][0], min(bounds[i][1], initial[i]))
            ...
            slot_results.append(
                {
                    ...
                    "confidence": (
                        "high"
                        if curve["n"] >= 50
                        else "medium" if curve["n"] >= 20 else "low"
                    ),
                }
            )
        ...
        # Generate insights
        spend_more = [
            s for s in slot_results if s["cost_diff"] > 2 and s["confidence"] != "low"
        ]
        spend_less = [
            s for s in slot_results if s["cost_diff"] < -2 and s["confidence"] != "low"
        ]
        ...
        if spend_more:
            ...
            insights.append(
                f"SPEND MORE on {top_more['slot']}: +${top_more['cost_diff']:.0f} → +{top_more['spar_gain']:.1f} SPAR"
            )
        if spend_less:
            top_less = spend_less[0]
            insights.append(
                f"SPEND LESS on {top_less['slot']}: ${top_less['cost_diff']:.0f} with only {top_less['spar_gain']:.1f} SPAR loss"
            )

        improvement = total_optimal_spar - total_league_spar
        pct_improvement = 100 * improvement / max(total_league_spar, 1)
        insights.append(
            f"Total improvement: +{improvement:.0f} SPAR ({pct_improvement:.1f}% vs league average)"
        )
    ...
    def _build_slot_curves(self) -> Dict[str, Dict]:
        ...
        for pos in self.positions:
            ...
            pos_df["slot_rank"] = pos_df.groupby([self.manager_col, self.year_col])[
                self.cost_col
            ].rank(method="first", ascending=False)
        ...
        if te_strategy.get("recommended_strategy"):
            ...
            if savings > 5 and strategy != "premium":
                recs.append(
                    ...
                    + (
                        f" with {spar_adv:.1f} more expected SPAR than premium"
                        if spar_adv and spar_adv > 0
                        else ""
                    )
                )
