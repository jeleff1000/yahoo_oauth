...
def display_draft_market_trends(prefix=""):
    ...
    yearly_trends = draft_data.groupby(['year', 'yahoo_position']).agg({
        'cost': 'mean',
        'spar': 'mean',
        'draft_roi': 'mean'
    }).reset_index()
    ...
    bucket_stats = bucket_data.groupby('cost_bucket').agg({
        'cost': 'mean',
        'spar': 'mean',
        'draft_roi': 'mean',
        'player': 'count'
    }).reset_index()
    bucket_stats.columns = ['Cost Bucket', 'Avg Cost', 'Avg SPAR', 'Avg SPAR/$', 'Count']
    ...
    fig.add_trace(go.Bar(
        ...
        name='Avg SPAR/$',
        marker_color=['darkgreen' if x > 1 else 'lightgreen' if x > 0 else 'red'
    ...
    if not bucket_stats.empty and len(bucket_stats) > 0:
        ...
        with col1:
            st.metric("Best SPAR/$ Bucket", f"Bucket {int(best_bucket['Cost Bucket'])}",
                      delta=f"{best_bucket['Avg SPAR/$']:.2f} SPAR/$")
        with col2:
            st.metric("Worst SPAR/$ Bucket", f"Bucket {int(worst_bucket['Cost Bucket'])}",
                      delta=f"{worst_bucket['Avg SPAR/$']:.2f} SPAR/$")
        with col3:
            st.metric("Most Popular Bucket", f"Bucket {int(most_popular['Cost Bucket'])}",
