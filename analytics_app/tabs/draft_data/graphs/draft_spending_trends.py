...
def display_draft_spending_trends(prefix=""):
    ...
    with st.expander("Show/Hide Options", expanded=False):
        ...
        with col1:
            show_drafted = st.checkbox(
                "Show Drafted", value=True, key=f"{prefix}_drafted"
            )
        with col2:
            show_keepers = st.checkbox(
                "Show Keepers", value=True, key=f"{prefix}_keepers"
            )
        ...
        if include_positions:
            if selected_manager == "League Average":
                ...
                for pos in include_positions:
                    ...
                    yearly_totals = (
                        pos_data.groupby("year")
                        .agg({"cost": "sum", "manager": "nunique"})
                        .reset_index()
                    )
                    yearly_totals["avg_cost"] = (
                        yearly_totals["cost"] / yearly_totals["manager"]
                    )
    ...
    st.plotly_chart(fig, use_container_width=True, key=f"{prefix}_spending_chart")
    ...
    if selected_position == "All Positions" and include_positions:
        ...
        if "Total SPAR" in summary_df.columns:
            column_config["Total SPAR"] = st.column_config.NumberColumn(
                "Total SPAR", format="%.1f"
            )
            column_config["Avg SPAR per Player"] = st.column_config.NumberColumn(
                "Avg SPAR/Player", format="%.1f"
            )
