...
def display_draft_round_efficiency(prefix=""):
    ...
    round_stats = data.groupby('round').agg({
        'cost': ['mean', 'sum'],
        'spar': ['mean', 'sum'],
        'draft_roi': 'mean',
        'player': 'count'
    }).round(2)
    round_stats.columns = ['Avg Cost', 'Total Spent', 'Avg SPAR', 'Total SPAR', 'Avg SPAR/$', 'Picks']
    ...
    if not round_stats.empty and len(round_stats) > 0:
        ...
        with col1:
            st.metric("Best SPAR/$ Round", f"Round {best_roi_round}",
                      delta=f"{round_stats.loc[best_roi_round, 'Avg SPAR/$']:.2f} SPAR/$")
        with col2:
            st.metric("Worst SPAR/$ Round", f"Round {worst_roi_round}",
                      delta=f"{round_stats.loc[worst_roi_round, 'Avg SPAR/$']:.2f} SPAR/$")
        with col3:
            st.metric("Best SPAR Round", f"Round {best_spar_round}",
