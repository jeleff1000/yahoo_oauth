...
def display_career_draft(draft_data):
    ...
    with col1:
        ...
        selected_team_managers = st.multiselect(
            "Select Manager", options=team_managers, default=[], key="manager"
        )
    with col2:
        yahoo_positions = sorted(
            [
                pos
                for pos in draft_data["yahoo_position"].unique().tolist()
                if pos is not None
            ]
        )
        selected_yahoo_positions = st.multiselect(
            "Select Position", options=yahoo_positions, default=[], key="yahoo_position"
        )

    names_full = sorted(
        [name for name in draft_data["player"].unique().tolist() if name is not None]
    )
    selected_names_full = st.multiselect(
        "Search Player Name", options=names_full, default=[], key="player"
    )
    ...
    if selected_yahoo_positions:
        draft_data = draft_data[
            draft_data["yahoo_position"].isin(selected_yahoo_positions)
        ]
    ...
    if "manager_spar" in draft_data.columns:
        draft_data["spar"] = pd.to_numeric(
            draft_data["manager_spar"], errors="coerce"
        ).fillna(0)
    elif "spar" in draft_data.columns:
        draft_data["spar"] = pd.to_numeric(draft_data["spar"], errors="coerce").fillna(
            0
        )
    ...
    if "times_drafted" not in draft_data.columns:
        ...
        if "pick" in draft_data.columns:
            draft_data["times_drafted"] = (
                pd.to_numeric(draft_data["pick"], errors="coerce").fillna(0) >= 1
            ).astype(int)
        else:
            draft_data["times_drafted"] = (
                pd.to_numeric(draft_data.get("cost", 0), errors="coerce").fillna(0) > 0
            ).astype(int)
    ...
    if "is_keeper_cost" not in draft_data.columns:
        if "is_keeper_status" in draft_data.columns:
            draft_data["is_keeper_cost"] = (
                draft_data["cost"]
                .where(draft_data["is_keeper_status"] == 1, 0)
                .fillna(0)
            )
        else:
    ...
    try:
        aggregated_data = (
            draft_data.groupby(["player", "yahoo_position"]).agg(agg_dict).reset_index()
        )
    ...
    column_config = {}
    if "spar" in aggregated_data.columns:
        column_config["spar"] = st.column_config.NumberColumn(
            "Total Manager SPAR",
            format="%.1f",
            help="Total Manager SPAR (actual value while rostered) across all drafts",
        )
    if "spar_per_dollar" in aggregated_data.columns:
        column_config["spar_per_dollar"] = st.column_config.NumberColumn(
            "SPAR/$", format="%.2f", help="Total Manager SPAR per total dollar spent"
        )
