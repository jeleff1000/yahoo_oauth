...
def display_draft_preferences(draft_data):
    ...
    draft_data['cost'] = pd.to_numeric(draft_data.get('cost'), errors='coerce').fillna(0)
    ...
    # Get options
    years = sorted(list({int(y) for y in draft_data['year'].dropna().unique().tolist()}))
    if not years:
    ...
    with col1:
        ...
        start_idx = 0
        start_year = st.selectbox("Start Year", years, index=start_idx, key="pref_start")
    with col2:
    ...
    with viz_tab:
        display_preference_charts(draft_data, selected_years, selected_manager, allowed_positions)

    with tables_tab:
        display_preference_tables(draft_data, selected_years, selected_manager, allowed_positions)

    with trends_tab:
        display_preference_trends(draft_data, selected_years, selected_manager, allowed_positions)


@st.fragment
def display_preference_charts(draft_data, selected_years, selected_manager, allowed_positions):
    ...
    if 'season_ppg' in drafted_only.columns:
        drafted_only['season_ppg'] = pd.to_numeric(drafted_only['season_ppg'], errors='coerce')
    else:
    ...
    pos_spending = drafted_only[drafted_only['cost'] > 0].groupby('yahoo_position').agg({
        'cost': ['sum', 'mean', 'count']
    }).reset_index()
    ...
    fig1.add_trace(
        go.Bar(x=pos_spending['Position'], y=pos_spending['Total Spent'],
               name='Total', marker_color='#3b82f6'),
        row=1, col=1
    )

    fig1.add_trace(
        go.Bar(x=pos_spending['Position'], y=pos_spending['Avg Cost'],
               name='Average', marker_color='#10b981'),
        row=1, col=2
    )
    ...
    drafted_only['pos_rank'] = drafted_only.groupby(['year', 'yahoo_position'])['cost'].rank(
        method='first', ascending=False
    )
    drafted_only['pos_rank'] = drafted_only['pos_rank'].replace([np.inf, -np.inf], np.nan).fillna(0).astype(int)

    # Aggregate by position and rank
    heatmap_data = drafted_only[
        (drafted_only['cost'] > 0) &
        (drafted_only['yahoo_position'].isin(allowed_positions))
        ].groupby(['yahoo_position', 'pos_rank']).agg({
        'cost': 'mean',
        'season_ppg': 'mean'
    }).reset_index()
    ...
    allocation['Percentage'] = (allocation['Total Spent'] / allocation['Total Spent'].sum() * 100).round(1)
...
@st.fragment
def display_preference_tables(draft_data, selected_years, selected_manager, allowed_positions):
    ...
    drafted = prepare_preference_table(drafted, selected_manager, allowed_positions, is_keeper=False)
    ...
    kept = prepare_preference_table(kept, selected_manager, allowed_positions, is_keeper=True)
...
def prepare_preference_table(data, selected_manager, allowed_positions, is_keeper=False):
    ...
    if not is_keeper:
        ...
        data['pos_rank'] = data['pos_rank'].replace([np.inf, -np.inf], np.nan).fillna(0).astype(int)
        ...
        agg_data.columns = ['Position Rank', 'Position', 'Manager', 'Avg Cost', 'Max Cost',
                            'Min Cost', 'Median Cost', 'Avg PPG', 'Avg Points', 'Times Drafted'] \
            if selected_manager != 'League Average' else \
            ['Position Rank', 'Position', 'Avg Cost', 'Max Cost',
             'Min Cost', 'Median Cost', 'Avg PPG', 'Avg Points', 'Times Drafted']
    else:
        ...
        agg_data.columns = ['Position', 'Manager', 'Avg Cost', 'Max Cost',
                            'Min Cost', 'Median Cost', 'Avg PPG', 'Avg Points', 'Times Kept'] \
            if selected_manager != 'League Average' else \
            ['Position', 'Avg Cost', 'Max Cost',
             'Min Cost', 'Median Cost', 'Avg PPG', 'Avg Points', 'Times Kept']
    ...
    if not is_keeper and 'Position Rank' in agg_data.columns:
        agg_data['rank_num'] = agg_data['Position Rank'].str.extract(r'(\d+)').fillna(0).astype(int)
...
@st.fragment
def display_preference_trends(draft_data, selected_years, selected_manager, allowed_positions):
    ...
    cost_trend = drafted[drafted['cost'] > 0].groupby(['year', 'yahoo_position']).agg({
        'cost': 'mean'
    }).reset_index()
    ...
    fig1 = px.line(
        ...
        markers=True,
        labels={'cost': 'Average Cost ($)', 'year': 'Year', 'yahoo_position': 'Position'},
    ...
    pick_trend = drafted.groupby(['year', 'yahoo_position']).size().reset_index(name='count')
    ...
    fig2 = px.bar(
        ...
        y='count',
        color='yahoo_position',
        labels={'count': 'Number of Picks', 'year': 'Year', 'yahoo_position': 'Position'},
    ...
    if 'manager_spar' in value_data.columns:
        value_data['manager_spar_num'] = pd.to_numeric(value_data['manager_spar'], errors='coerce').fillna(0)
        value_data['value'] = value_data['manager_spar_num'] / value_data['cost']
    elif 'draft_roi' in value_data.columns:
        value_data['value'] = pd.to_numeric(value_data['draft_roi'], errors='coerce').fillna(0)
    elif 'spar' in value_data.columns:
        value_data['spar_num'] = pd.to_numeric(value_data['spar'], errors='coerce').fillna(0)
        value_data['value'] = value_data['spar_num'] / value_data['cost']
    else:
        value_data['value'] = 0

    value_trend = value_data.groupby(['year', 'yahoo_position']).agg({
        'value': 'mean'
    }).reset_index()
    ...
    fig3 = px.line(
        ...
        color='yahoo_position',
        markers=True,
        labels={'value': 'Manager SPAR ROI', 'year': 'Year', 'yahoo_position': 'Position'},
