...
def display_draft_summary(draft_data: pd.DataFrame) -> None:
    ...
    st.markdown("*Complete draft history with grades, value tiers, and performance stats*")
    ...
    if 'manager' in df.columns:
        ...
        df = df[(df['manager'] != '') & (df['manager'] != 'None') & (df['manager'].notna())]

    if 'year' in df.columns:
        df['year'] = df['year'].astype(str)

    # Ensure numeric columns
    numeric_cols = ['cost', 'pick', 'round', 'total_fantasy_points', 'season_ppg',
                    'games_played', 'weeks_rostered', 'weeks_started',
                    'manager_spar', 'season_overall_rank', 'season_position_rank']
    ...
    keeper_count = (pd.to_numeric(df['is_keeper_status'], errors='coerce') == 1).sum() if 'is_keeper_status' in df.columns else 0
    breakout_count = (pd.to_numeric(df['is_breakout'], errors='coerce') == 1).sum() if 'is_breakout' in df.columns else 0
    bust_count = (pd.to_numeric(df['is_bust'], errors='coerce') == 1).sum() if 'is_bust' in df.columns else 0
    ...
    with col6:
        st.metric("Breakouts", f"{breakout_count:,}", help="Late-round picks with top finishes")
    with col7:
        st.metric("Busts", f"{bust_count:,}", help="Early-round picks with bottom finishes")
    ...
    with st.expander("🔍 Filters", expanded=True):
        ...
        with col1:
            ...
            selected_grades = st.multiselect("Draft Grade", available_grades if available_grades else grades,
                                             default=[], key="draft_summary_grade")
        ...
        with col2:
            ...
            selected_tiers = st.multiselect("Value Tier", available_tiers if available_tiers else tiers,
                                            default=[], key="draft_summary_tier")
        ...
        with col1:
            include_keepers = st.checkbox("Include Keepers", value=True, key="draft_summary_keepers")
        with col2:
            min_cost = st.number_input("Min Cost", min_value=0, value=0, key="draft_summary_min_cost")
        with col3:
            sort_by = st.selectbox("Sort by", ["Recent", "Points ↓", "PPG ↓", "Cost ↓", "Overall Rank", "Grade ↓", "SPAR ↓"],
    ...
    if not include_keepers and 'is_keeper_status' in filtered.columns:
        filtered = filtered[pd.to_numeric(filtered['is_keeper_status'], errors='coerce') != 1]
    ...
    if show_breakouts and 'is_breakout' in filtered.columns:
        filtered = filtered[pd.to_numeric(filtered['is_breakout'], errors='coerce') == 1]
    ...
    display_cols = [
        'year', 'round', 'pick', 'manager', 'player', 'yahoo_position', 'nfl_team',
        'cost', 'draft_grade', 'value_tier', 'total_fantasy_points', 'season_ppg',
        'games_played', 'manager_spar', 'season_position_rank', 'is_keeper_status'
    ]
    ...
    column_config = {
        ...
        'draft_grade': st.column_config.TextColumn("Grade", width="small", help="A-F based on SPAR percentile"),
        'value_tier': st.column_config.TextColumn("Value", width="small", help="Steal/Good Value/Fair/Overpay/Bust"),
        'total_fantasy_points': st.column_config.NumberColumn("Points", format="%.1f", width="small"),
        'season_ppg': st.column_config.NumberColumn("PPG", format="%.2f", width="small"),
        'games_played': st.column_config.NumberColumn("Games", format="%d", width="small"),
        'weeks_rostered': st.column_config.NumberColumn("Rostered", format="%.0f", help="Weeks on roster", width="small"),
        'weeks_started': st.column_config.NumberColumn("Started", format="%.0f", help="Weeks started", width="small"),
        'manager_spar': st.column_config.NumberColumn("SPAR", format="%.1f", help="SPAR while rostered", width="small"),
        'season_overall_rank': st.column_config.NumberColumn("Overall Rank", format="%d", width="small"),
        'season_position_rank': st.column_config.NumberColumn("Pos Rank", format="%d", width="small"),
        'is_keeper_status': st.column_config.CheckboxColumn("Keeper", help="Keeper pick", width="small"),
    }
    ...
    if 'draft_grade' in filtered.columns and filtered['draft_grade'].notna().any():
        ...
        grade_counts = filtered['draft_grade'].value_counts().reindex(['A', 'B', 'C', 'D', 'F']).fillna(0)
        cols = st.columns(5)
        for i, (grade, count) in enumerate(grade_counts.items()):
            with cols[i]:
                color = GRADE_COLORS.get(grade, '#6c757d')
                st.markdown(f"<div style='text-align:center;'><span style='font-size:2em;color:{color};font-weight:bold;'>{grade}</span><br/>{int(count)} picks</div>", unsafe_allow_html=True)

    # Export
    csv = filtered[display_cols].to_csv(index=False)
    st.download_button("📥 Export", csv, "draft_summary.csv", "text/csv", key="draft_summary_export")
