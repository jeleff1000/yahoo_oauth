...
def get_basic_stats(team_data: pd.DataFrame) -> pd.DataFrame:
    """
    Returns weekly basic team stats aggregated by manager only.
    ...
    df["total_tds"] = (
        ...
        + pd.to_numeric(df.get("def_tds", 0), errors="coerce").fillna(0)
    )
    ...
    if sort_cols:
        df = df.sort_values(
            by=sort_cols, ascending=[False, False, False], na_position="last"
        )
