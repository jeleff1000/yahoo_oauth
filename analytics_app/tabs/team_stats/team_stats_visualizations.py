...
class TeamStatsVisualizer:
    ...
    def __init__(
        self, team_data_by_position: pd.DataFrame, team_data_by_manager: pd.DataFrame
    ):
    ...
    def display_weekly_visualizations(self):
        ...
        selected_chart = st.selectbox(
            "Select Visualization", chart_types, key="weekly_viz_selector"
        )
    ...
    def plot_points_over_time(self):
        ...
        # Filter data
        filtered_df = df[
            df["year"].isin(selected_years) & df["manager"].isin(selected_managers)
        ]
        ...
        fig.update_layout(xaxis_tickangle=-45, hovermode="x unified", showlegend=True)
    ...
    def plot_td_breakdown(self):
        ...
        selected_year = st.selectbox(
            "Select Year", available_years, key="td_breakdown_year"
        )
    ...
    def plot_yards_breakdown(self):
        ...
        available_years = sorted(df["year"].dropna().unique().tolist(), reverse=True)
        selected_year = st.selectbox(
            "Select Year", available_years, key="yards_breakdown_year"
        )
    ...
    def plot_position_contribution(self):
        ...
        available_years = sorted(df["year"].dropna().unique().tolist(), reverse=True)
        selected_year = st.selectbox(
            "Select Year", available_years, key="position_contribution_year"
        )
        ...
        selected_manager = st.selectbox(
            "Select Manager", available_managers, key="position_contribution_manager"
        )
    ...
    def display_season_visualizations(self):
        ...
        selected_chart = st.selectbox(
            "Select Visualization", chart_types, key="season_viz_selector"
        )
    ...
    def plot_season_points_comparison(self):
        ...
        selected_years = st.multiselect(
            ...
            available_years,
            default=(
                available_years[:3] if len(available_years) >= 3 else available_years
            ),
    ...
    def plot_position_heatmap(self):
        ...
        # Year selector
        available_years = sorted(df["year"].dropna().unique().tolist(), reverse=True)
        selected_year = st.selectbox("Select Year", available_years, key="heatmap_year")
    ...
    def display_career_visualizations(self):
        ...
        selected_chart = st.selectbox(
            "Select Visualization", chart_types, key="career_viz_selector"
        )
    ...
    def plot_career_totals(self):
        ...
        if selected_metric in df.columns:
            ...
            fig = px.bar(
                ...
                labels={
                    selected_metric: available_metrics[selected_metric],
                    "manager": "Manager",
                },
    ...
    def plot_position_dominance(self):
        ...
        df["total_points"] = pd.to_numeric(
            df.get("total_points", df.get("points", 0)), errors="coerce"
        )

        # Aggregate by manager and position
        agg_df = (
            df.groupby(["manager", "fantasy_position"])
        ...
        pivot_df = agg_df.pivot(
            index="manager", columns="fantasy_position", values="total_points"
        ).fillna(0)
    ...
    def plot_alltime_leaders(self):
        ...
        with col2:
            top_n = st.slider(
                ...
                max_value=15,
                value=10,
                key="alltime_leaders_top_n",
            )

        if selected_category in df.columns:
            df[selected_category] = pd.to_numeric(
                df[selected_category], errors="coerce"
            )

            # Get top N
            top_df = df.nlargest(top_n, selected_category).sort_values(
                selected_category, ascending=True
            )
