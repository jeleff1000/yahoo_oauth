...
class TeamStatsFilterPanel:
    ...
    def display_advanced_filters(self) -> Dict[str, Any]:
        ...
        with st.expander("⚙️ Advanced Filters", expanded=False):
            ...
            filters["include_playoffs"] = st.checkbox(
                "Include Playoffs", value=True, key=f"{self.prefix}_include_playoffs"
            )
    ...
    def apply_filters(
        ...
        if "position" in quick_filters and quick_filters["position"] != "All":
            pos_col = (
                "fantasy_position"
                if "fantasy_position" in filtered_df.columns
                else "Position"
            )
            if pos_col in filtered_df.columns:
                filtered_df = filtered_df[
                    filtered_df[pos_col] == quick_filters["position"]
                ]
        ...
        if "year_start" in quick_filters and "year_end" in quick_filters:
            ...
            if year_col in filtered_df.columns:
                filtered_df[year_col] = pd.to_numeric(
                    filtered_df[year_col], errors="coerce"
                )
                filtered_df = filtered_df[
        ...
        if "managers" in quick_filters and quick_filters["managers"]:
            ...
            if manager_col in filtered_df.columns:
                filtered_df = filtered_df[
                    filtered_df[manager_col].isin(quick_filters["managers"])
                ]
        ...
        if advanced_filters:
            ...
            if "week_range" in advanced_filters:
                ...
                if week_col in filtered_df.columns:
                    filtered_df[week_col] = pd.to_numeric(
                        filtered_df[week_col], errors="coerce"
                    )
                    filtered_df = filtered_df[
            ...
            if "min_points" in advanced_filters and advanced_filters["min_points"] > 0:
                ...
                if points_col in filtered_df.columns:
                    filtered_df[points_col] = pd.to_numeric(
                        filtered_df[points_col], errors="coerce"
                    )
                    filtered_df = filtered_df[
                        filtered_df[points_col] >= advanced_filters["min_points"]
                    ]
            ...
            if not advanced_filters.get("include_playoffs", True):
                if "game_type" in filtered_df.columns:
                    ...
                    filtered_df = filtered_df[
                        pd.to_numeric(filtered_df["week"], errors="coerce") <= 14
                    ]
            ...
            if not advanced_filters.get("include_bench", False):
                if "roster_position" in filtered_df.columns:
                    filtered_df = filtered_df[
                        ~filtered_df["roster_position"].isin(["BN", "IR"])
                    ]
    ...
    def display_active_filters(
        ...
        if "year_start" in quick_filters and "year_end" in quick_filters:
            if quick_filters["year_start"] == quick_filters["year_end"]:
                ...
                active_filters.append(
                    f"Years: {quick_filters['year_start']}-{quick_filters['year_end']}"
                )
        ...
        if advanced_filters:
            if "week_range" in advanced_filters:
                ...
                if week_start != min(self.available_weeks) or week_end != max(
            ...
            if "min_points" in advanced_filters and advanced_filters["min_points"] > 0:
                active_filters.append(
                    f"Min Points: {advanced_filters['min_points']:.0f}"
                )
...
def render_year_selector(
    prefix: str, available_years: List[int], allow_multiple: bool = False
) -> Any:
    """
...
def render_week_slider(
    prefix: str, min_week: int = 1, max_week: int = 17
) -> Tuple[int, int]:
    """
...
def render_manager_selector(
    ...
    if allow_multiple:
        ...
        selected = st.selectbox(
            "Select Manager", options=available_managers, key=f"{prefix}_manager_single"
        )
