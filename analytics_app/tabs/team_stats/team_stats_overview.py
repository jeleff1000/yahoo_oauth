...
def render_weekly_team():
    ...
    include_regular_season = st.session_state.get(
    ...
    with st.spinner("Loading weekly team data..."):
        ...
        if (
            (team_data_by_pos is not None and not team_data_by_pos.empty)
            or (team_data_by_mgr is not None and not team_data_by_mgr.empty)
            or (
                team_data_by_lineup_pos is not None
                and not team_data_by_lineup_pos.empty
            )
        ):
            WeeklyTeamViewer(
                team_data_by_pos, team_data_by_mgr, team_data_by_lineup_pos
            ).display()
        else:
...
def render_season_team():
    ...
    # Get filter values from session state
    include_regular_season = st.session_state.get(
        "season_team_include_regular_season", True
    )
...
def render_visualizations():
    ...
    st.markdown(
        "*Visual analytics for team stats - explore scoring patterns, position performance, and manager comparisons*"
    )
    ...
    with viz_tabs[0]:
        ...
        include_playoffs = st.session_state.get("weekly_team_include_playoffs", True)
        include_consolation = st.session_state.get(
    ...
    with viz_tabs[1]:
        ...
        include_playoffs = st.session_state.get("season_team_include_playoffs", True)
        include_consolation = st.session_state.get(
            "season_team_include_consolation", False
        )
