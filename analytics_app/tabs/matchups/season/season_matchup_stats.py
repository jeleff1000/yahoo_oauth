...
class SeasonMatchupStatsViewer:
    ...
    def _get_final_playoff_result(group):
        ...
        playoff_filter = (
            group["playoff_round"].notna() & (group["playoff_round"] != "")
            if "playoff_round" in group.columns
        ...
        if "consolation_round" in group.columns:
            consolation_filter = group["consolation_round"].notna() & (
                group["consolation_round"] != ""
            )
        ...
        playoff_round = (
            last_game["playoff_round"] if "playoff_round" in last_game.index else ""
        )
        consolation_round = (
            last_game["consolation_round"]
            if "consolation_round" in last_game.index
            else ""
        )
    ...
    def display(self, prefix=""):
        ...
        if "win" in self.df.columns:
            ...
            aggregation_type = st.toggle(
                "Per Game", value=False, key=f"{prefix}_aggregation_type"
            )
            ...
            for (manager, year), group in self.df.groupby(["manager", "year"]):
                ...
                if "sacko" in self.df.columns:
                    ...
                    sacko_flag = False
                sacko_flags.append(
                    {"manager": manager, "year": year, "sacko": sacko_flag}
                )

                # Playoff result
                playoff_result = self._get_final_playoff_result(group)
                playoff_results.append(
                    {"manager": manager, "year": year, "playoff_result": playoff_result}
                )
            ...
            agg_dict = {
                col: aggregation_func for col in agg_columns if col in self.df.columns
            }
            ...
            aggregated_df = (
                self.df.groupby(["manager", "year"]).agg(agg_dict).reset_index()
            )

            # Add additional stats separately after the main aggregation
            if "team_points" in self.df.columns:
                best_week = (
                    self.df.groupby(["manager", "year"])["team_points"]
                    .max()
                    .reset_index()
                )
                best_week.columns = ["manager", "year", "team_points_max"]
                aggregated_df = pd.merge(
                    aggregated_df, best_week, on=["manager", "year"], how="left"
                )
                ...
                aggregated_df = pd.merge(
                    aggregated_df, worst_week, on=["manager", "year"], how="left"
                )
                ...
                aggregated_df = pd.merge(
                    aggregated_df, std_dev, on=["manager", "year"], how="left"
                )
            ...
            if "final_playoff_seed" in self.df.columns:
                ...
                final_seed = (
                    self.df.sort_values("week")
                    .groupby(["manager", "year"])["final_playoff_seed"]
                    .apply(last_non_null)
                    .reset_index()
                )
                final_seed.columns = ["manager", "year", "final_playoff_seed"]
                aggregated_df = pd.merge(
                    aggregated_df, final_seed, on=["manager", "year"], how="left"
                )
            ...
            aggregated_df = pd.merge(
                aggregated_df, sacko_df, on=["manager", "year"], how="left"
            )
            aggregated_df = pd.merge(
                aggregated_df, playoff_df, on=["manager", "year"], how="left"
            )
            ...
            if aggregation_type:
                ...
                aggregated_df[columns_to_round_2] = aggregated_df[
                    columns_to_round_2
                ].round(2)
                aggregated_df[columns_to_round_3] = aggregated_df[
                    columns_to_round_3
                ].round(3)
            ...
            column_config = {
                ...
                "Year": st.column_config.TextColumn(
                    "Year", help="Season year", width="small"
                ),
                ...
                "Playoffs": st.column_config.TextColumn(
                    "Playoffs", help="Made playoffs", width="small"
                ),
                ...
                "Champ": st.column_config.TextColumn(
                    "Champ", help="Won championship", width="medium"
                ),
                "Sacko": st.column_config.TextColumn(
                    "Sacko", help="Finished last place", width="medium"
                ),
            }
            ...
            avg_win_pct = (
                display_df["Win %"].mean()
                if "Win %" in display_df.columns and total_seasons > 0
                else 0
            )
            avg_ppg = (
                display_df["PPG"].mean()
                if "PPG" in display_df.columns and total_seasons > 0
                else 0
            )
            avg_papg = (
                display_df["PA/G"].mean()
                if "PA/G" in display_df.columns and total_seasons > 0
                else 0
            )
            best_season = (
                display_df["Best"].max()
                if "Best" in display_df.columns and total_seasons > 0
                else 0
            )
            championships = (
                len(display_df[display_df["Champ"] == "🏆 Champion"])
                if "Champ" in display_df.columns
                else 0
            )
            sackos = (
                len(display_df[display_df["Sacko"] == "💩 Sacko"])
                if "Sacko" in display_df.columns
                else 0
            )
