...
class SeasonProjectedStatsViewer:
    ...
    def display(self, prefix=""):
        ...
        if (
            "manager_proj_score" in self.df.columns
            and "team_projected_points" not in self.df.columns
        ):
            self.df["team_projected_points"] = self.df["manager_proj_score"]
        if (
            "opponent_proj_score" in self.df.columns
            and "opponent_projected_points" not in self.df.columns
        ):
    ...
    def _prepare_display_data(self) -> pd.DataFrame:
        ...
        df["surprise_outcome"] = (df["underdog_wins"] == 1) | (
            df["favorite_losses"] == 1
        )
        ...
        df["moderate"] = (df["abs_proj_score_error"] > 10) & (
            df["abs_proj_score_error"] <= 20
        )
        ...
        display_df["proj_diff"] = (
            display_df["team_points"] - display_df["team_projected_points"]
        )
        display_df["beat_projection"] = display_df["proj_diff"] > 0
        display_df["proj_win"] = display_df["expected_odds"] > 0.5  # Majority favored
        display_df["proj_margin"] = (
            display_df["team_projected_points"]
            - display_df["opponent_projected_points"]
        )

        # Drop intermediate calculation columns
        display_df = display_df.drop(
            columns=["accurate", "moderate", "inaccurate"], errors="ignore"
        )
    ...
    def _calculate_stats(self, df: pd.DataFrame) -> dict:
        ...
        inaccurate_pct = (
            (total_inaccurate / total_games * 100) if total_games > 0 else 0
        )
    ...
    def _render_enhanced_table(self, df: pd.DataFrame, prefix: str, per_game: bool):
        ...
        column_config = {
            ...
            "W": st.column_config.NumberColumn(
                "W", help="Wins", format="%.1f" if per_game else "%d", width="small"
            ),
            "L": st.column_config.NumberColumn(
                "L", help="Losses", format="%.1f" if per_game else "%d", width="small"
            ),
            "Your Pts": st.column_config.NumberColumn(
                "Your Pts",
                help=(
                    "Total points scored" if not per_game else "Average points per game"
                ),
                format="%.2f",
                width="small",
            ),
            "Your Proj": st.column_config.NumberColumn(
                ...
                help=(
                    ...
                    if not per_game
                    else "Average projected points per game"
                ),
                format="%.2f",
                width="small",
            ),
            ...
            "Beat Proj": st.column_config.NumberColumn(
                "Beat Proj",
                help=(
                    "Games beat projection"
                    if not per_game
                    else "Rate of beating projection"
                ),
            ...
            "Margin": st.column_config.NumberColumn(
                "Margin", help="Point differential", format="%.2f", width="small"
            ),
            ...
            "Beat Spread": st.column_config.NumberColumn(
                "Beat Spread",
                help=(
                    "Games beat the spread"
                    if not per_game
                    else "Rate of beating spread"
                ),
                format="%.2f" if per_game else "%d",
                width="small",
            ),
            ...
            "Proj W": st.column_config.NumberColumn(
                "Proj W", help="Projected wins", format="%.1f", width="small"
            ),
            ...
            "Accuracy": st.column_config.TextColumn(
                "Accuracy", help="Projection accuracy tier", width="small"
            ),
        }
    ...
    def _render_quick_stats(self, stats: dict, per_game: bool):
        ...
        with col2:
            st.metric(
                "Actual Wins", f"{stats['total_wins']:,}", help="Total actual wins"
            )
        ...
        with col6:
            st.metric(
                "Beat Spread", f"{stats['beat_spread']}", help="Games beat the spread"
            )
        ...
        with col10:
            st.metric(
                "Upset Wins", f"{stats['underdog_wins']}", help="Wins as underdog"
            )

        with col11:
            st.metric(
                "Upset Losses", f"{stats['favorite_losses']}", help="Losses as favorite"
            )

        with col12:
            upset_rate = (
                (
                    stats["total_surprises"]
