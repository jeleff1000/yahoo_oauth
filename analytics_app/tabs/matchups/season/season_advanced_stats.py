...
class SeasonAdvancedStatsViewer:
    ...
    def _prepare_display_data(self, prefix) -> pd.DataFrame:
        ...
        # Calculate Close Win %
        display_df["close_win_pct"] = (
            ((display_df["close_win"] / display_df["close_margin"]) * 100)
            .fillna(0)
            .round(1)
        )
        ...
        display_df["accuracy_pct"] = (
            ((display_df["accurate"] / display_df["games"]) * 100).fillna(0).round(1)
        )
        ...
        display_df = display_df.drop(
            columns=["accurate", "moderate", "inaccurate", "games"], errors="ignore"
        )
    ...
    def _calculate_stats(self, df: pd.DataFrame) -> dict:
        ...
        above_median_rate = (
            (
                df["Above Median"].sum()
                / (df["Above Median"].sum() + df["Below Median"].sum())
            )
            * 100
            if (df["Above Median"].sum() + df["Below Median"].sum()) > 0
            else 0
        )
    ...
    def _render_enhanced_table(self, df: pd.DataFrame, prefix: str, per_game: bool):
        ...
        column_config = {
            ...
            "W": st.column_config.NumberColumn(
                "W", help="Wins", format="%.2f" if per_game else "%d", width="small"
            ),
            "L": st.column_config.NumberColumn(
                "L", help="Losses", format="%.2f" if per_game else "%d", width="small"
            ),
    ...
    def _render_quick_stats(self, stats: dict):
        ...
        with col2:
            st.metric(
                "Avg GPA", f"{stats['avg_gpa']:.2f}", help="Average grade point average"
            )
        ...
        with col4:
            st.metric(
                "Low GPA Seasons", f"{stats['low_gpa']}", help="Seasons with GPA < 2.0"
            )
