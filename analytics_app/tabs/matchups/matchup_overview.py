...
def display_matchup_overview(df_dict, prefix=""):
    ...
    sub_tab_name = (
        sub_tab_names[subtab_idx] if subtab_idx < len(sub_tab_names) else "Weekly"
    )
    ...
    if sub_tab_name == "Weekly":
        ...
        st.markdown(
            "*Visual analytics for managers - explore scoring patterns, performance, and efficiency*"
        )
        ...
        try:
            from .graphs.optimal_lineup_efficiency import (
        ...
        if graph_imports:
            ...
            with graph_tabs[0]:
                if "scoring_trends" in graph_imports:
                    try:
                        graph_imports["scoring_trends"](
                            df_wrapper, prefix=f"{prefix}_scoring_trends"
                        )
                    except Exception as e:
                        logger.error(
                            f"Error displaying scoring trends graph: {e}", exc_info=True
                        )
            ...
            with graph_tabs[1]:
                if "win_percentage" in graph_imports:
                    try:
                        graph_imports["win_percentage"](
                            df_wrapper, prefix=f"{prefix}_win_pct"
                        )
                    except Exception as e:
                        logger.error(
                            f"Error displaying win percentage graph: {e}", exc_info=True
                        )
                        st.error(f"Error displaying graph: {str(e)}")
                else:
            ...
            with graph_tabs[2]:
                if "power_rating" in graph_imports:
                    try:
                        graph_imports["power_rating"](
                            df_wrapper, prefix=f"{prefix}_power_rating"
                        )
                    except Exception as e:
                        logger.error(
                            f"Error displaying power rating graph: {e}", exc_info=True
                        )
                        st.error(f"Error displaying graph: {str(e)}")
                else:
            ...
            with graph_tabs[3]:
                if "scoring_distribution" in graph_imports:
                    try:
                        graph_imports["scoring_distribution"](
                            prefix=f"{prefix}_score_dist"
                        )
                    except Exception as e:
                        logger.error(
                            f"Error displaying scoring distribution graph: {e}",
                            exc_info=True,
                        )
                        st.error(f"Error displaying graph: {str(e)}")
                else:
            ...
            with graph_tabs[4]:
                if "margin_of_victory" in graph_imports:
                    try:
                        graph_imports["margin_of_victory"](
                            df_wrapper, prefix=f"{prefix}_margin"
                        )
                    except Exception as e:
                        logger.error(
                            f"Error displaying margin of victory graph: {e}",
                            exc_info=True,
                        )
                        st.error(f"Error displaying graph: {str(e)}")
                else:
            ...
            with graph_tabs[5]:
                if "lineup_efficiency" in graph_imports:
                    try:
                        graph_imports["lineup_efficiency"](
                            df_wrapper, prefix=f"{prefix}_efficiency"
                        )
                    except Exception as e:
                        logger.error(
                            f"Error displaying lineup efficiency graph: {e}",
                            exc_info=True,
                        )
                        st.error(f"Error displaying graph: {str(e)}")
                else:
            ...
            with graph_tabs[6]:
                if "playoff_vs_regular" in graph_imports:
                    try:
                        graph_imports["playoff_vs_regular"](
                            df_wrapper, prefix=f"{prefix}_playoff"
                        )
                    except Exception as e:
                        logger.error(
                            f"Error displaying playoff performance graph: {e}",
                            exc_info=True,
                        )
                        st.error(f"Error displaying graph: {str(e)}")
                else:
            ...
            with graph_tabs[7]:
                if "strength_of_schedule" in graph_imports:
                    try:
                        graph_imports["strength_of_schedule"](
                            df_wrapper, prefix=f"{prefix}_sos"
                        )
                    except Exception as e:
                        logger.error(
                            f"Error displaying strength of schedule graph: {e}",
                            exc_info=True,
                        )
                        st.error(f"Error displaying graph: {str(e)}")
                else:
            ...
            st.warning(
                "Manager graphs are currently unavailable. Some graph modules could not be loaded."
            )
