...
def filter_matchup_data(
    ...
    filtered_df = df[
        df["manager"].isin(selected_managers) & df["opponent"].isin(selected_opponents)
    ]
    ...
    with st.expander("🔎 Filters", expanded=False):
        ...
        with toggle_cols[0]:
            regular_season = st.checkbox(
                "Regular", value=True, key=f"{prefix}_regular_season"
            )
        with toggle_cols[1]:
            playoffs = st.checkbox("Playoffs", value=True, key=f"{prefix}_playoffs")
        with toggle_cols[2]:
            consolation = st.checkbox(
                "Consolation", value=consolation_default, key=f"{prefix}_consolation"
            )
        with toggle_cols[3]:
