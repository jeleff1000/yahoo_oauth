...
def display_power_rating_graph(df_dict=None, prefix="graphs_manager_power_rating"):
    ...
    with col1:
        start_year = st.selectbox(
            "Start Year", options=available_years, index=0, key=f"{prefix}_start_year"
        )
    ...
    with col3:
        selected_managers = st.multiselect(
            ...
            default=(
                managers_available[:5]
                if len(managers_available) >= 5
                else managers_available
            ),
    ...
    for manager in selected_managers:
        manager_data = df_filtered[df_filtered["manager"] == manager].sort_values(
            "cumulative_week"
        )
    ...
    with col1:
        ...
        latest_ratings = (
            df_filtered.sort_values("cumulative_week")
            .groupby("manager")
            .last()
            .reset_index()
        )
        latest_ratings = latest_ratings[
            ["manager", "power_rating", "year", "week"]
    ...
    with col2:
        ...
        rating_stats = (
            df_filtered.groupby("manager")["power_rating"]
            .agg(["min", "max", "mean"])
            .round(2)
        )
    ...
    with st.expander("🔍 Power Rating vs Performance", expanded=False):
        ...
        correlation_data = (
            df_filtered.groupby("manager")
            .agg({"power_rating": "mean", "win": "mean", "team_points": "mean"})
            .reset_index()
        )
        ...
        st.plotly_chart(
            fig_scatter, use_container_width=True, key=f"{prefix}_correlation"
        )
    ...
    with st.expander("💡 Key Insights", expanded=False):
        if len(latest_ratings) >= 1:
            ...
            if start_year != end_year:
                start_ratings = (
                    df_filtered[df_filtered["year"] == start_year]
                    .groupby("manager")["power_rating"]
                    .mean()
                )
                end_ratings = (
                    df_filtered[df_filtered["year"] == end_year]
                    .groupby("manager")["power_rating"]
                    .mean()
                )
                ...
                if common_managers:
                    improvements = {
                        mgr: end_ratings[mgr] - start_ratings[mgr]
                        for mgr in common_managers
                    }
