...
def display_scoring_distribution_graph(prefix=""):
    ...
    stats = (
        ...
        .agg({"team_points": ["mean", "std", "min", "max", "count"], "win": "sum"})
        .round(2)
    )
    ...
    stats["coefficient_of_variation"] = (
        (stats["std_dev"] / stats["avg_points"]) * 100
    ).round(
        1
    ...
    tab1, tab2, tab3 = st.tabs(
        ["📊 Box Plots", "📈 Scatter Analysis", "📋 Stats Table"]
    )
    ...
    with tab1:
        ...
        manager_medians = (
            data.groupby("manager")["team_points"].median().sort_values(ascending=False)
        )
        ...
        fig.update_layout(
            ...
            title=dict(text=f"Distribution - {selected_year}", x=0.5, xanchor="center"),
        )
        ...
        st.caption(
            "💡 Box: 25th-75th percentile | Line in box: median | Diamond: mean | Whiskers: range (excl. outliers)"
        )
    ...
    with tab2:
        ...
        fig2.add_trace(
            go.Scatter(
                ...
                + stats["win_pct"].astype(str)
                + "%<br>Games: "
                + stats["games"].astype(str)
                + "<extra></extra>",
            )
        )
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "CV %": st.column_config.NumberColumn(
                    format="%.1f%%", help="Lower = more consistent"
                ),
