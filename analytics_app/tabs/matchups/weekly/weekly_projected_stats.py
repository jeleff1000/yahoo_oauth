...
class WeeklyProjectedStatsViewer:
    ...
    def _prepare_display_data(self) -> pd.DataFrame:
        ...
        df["beat_spread"] = (df["team_points"] - df["opponent_points"]) > df[
            "expected_spread"
        ]
        ...
        df["accuracy_tier"] = df["abs_proj_score_error"].apply(
            self._categorize_accuracy
        )
    ...
    def _calculate_stats(self, df: pd.DataFrame) -> dict:
        ...
        beat_spread_pct = (
            (beat_spread_count / total_games * 100) if total_games > 0 else 0
        )

        # Projection accuracy
        accurate_projections = len(df[df["Result"] == df["Proj Result"]])
        projection_accuracy = (
            (accurate_projections / total_games * 100) if total_games > 0 else 0
        )
        ...
        favored_win_pct = (
            (favored_wins / len(favored_games) * 100) if len(favored_games) > 0 else 0
        )
        ...
        underdog_win_pct = (
            (underdog_wins / len(underdog_games) * 100)
        ...
        accuracy_dist = (
            df["Accuracy"].value_counts().to_dict() if "Accuracy" in df.columns else {}
        )
    ...
    def _render_enhanced_table(self, df: pd.DataFrame, prefix: str):
        ...
        column_config = {
            ...
            "Favored?": st.column_config.TextColumn(
                "Favored?", help="Were you favored to win?", width="small"
            ),
            "Win %": st.column_config.TextColumn(
                "Win %", help="Pre-game win probability", width="small"
            ),
            "Your Pts": st.column_config.NumberColumn(
                "Your Pts", help="Actual points scored", format="%.2f", width="small"
            ),
            "Your Proj": st.column_config.NumberColumn(
                "Your Proj", help="Projected points", format="%.2f", width="small"
            ),
            ...
            "Beat Proj?": st.column_config.TextColumn(
                "Beat Proj?", help="Scored above projection", width="small"
            ),
            "Opp Pts": st.column_config.NumberColumn(
                "Opp Pts", help="Opponent actual points", format="%.2f", width="small"
            ),
            ...
            "Beat Spread?": st.column_config.TextColumn(
                "Beat Spread?", help="Beat the spread", width="small"
            ),
            "Result": st.column_config.TextColumn(
                "Result", help="Actual game result", width="small"
            ),
            "Proj Result": st.column_config.TextColumn(
                "Proj Result", help="Projected game result", width="small"
            ),
            "Upset?": st.column_config.TextColumn(
                "Upset?", help="Result defied expectations", width="small"
            ),
    ...
    def _render_quick_stats(self, stats: dict):
        ...
        with col2:
            ...
            st.metric("Actual Record", actual_record, help="Actual win-loss record")

        with col3:
            projected_record = f"{stats['projected_wins']}-{stats['projected_losses']}"
            st.metric(
                "Projected Record", projected_record, help="What projections predicted"
            )
