    ...
    def display(self, prefix):
        ...
        if "win" in merged_data.columns:
            ...
            merged_data["position_rank"] = (
                merged_data.groupby(["Manager", "week", "year", "fantasy position"])[
                    "points"
                ]
            ...
            merged_data["fantasy position"] = merged_data.apply(
                lambda row: (
                    f"{row['fantasy position']}{row['position_rank']}"
                    if row["fantasy position"] in ["RB", "WR"]
                    else row["fantasy position"]
                ),
                axis=1,
            )
            ...
            merged_data["fantasy position"] = pd.Categorical(
                merged_data["fantasy position"],
                categories=unique_position_order,
                ordered=True,
            )
