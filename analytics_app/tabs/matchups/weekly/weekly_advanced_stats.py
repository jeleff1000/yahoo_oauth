...
class WeeklyAdvancedStatsViewer:
    ...
    def _calculate_stats(self, df: pd.DataFrame) -> dict:
        ...
        if "Efficiency" in df.columns:
            ...
            stats["perfect_lineups"] = (
                len(df[df["Efficiency"] >= 98.0])
        ...
        if "Close Game" in df.columns:
            stats["close_games"] = (df["Close Game"]).sum()
    ...
    def _render_enhanced_table(self, df: pd.DataFrame, prefix: str):
        ...
        column_config = {
            ...
            "Grade": st.column_config.TextColumn(
                "Grade", help="Letter grade (A, B, C, D, F)", width="small"
            ),
            ...
            "Efficiency": st.column_config.TextColumn(
                "Efficiency", help="Lineup efficiency (actual/optimal)", width="small"
            ),
            "Bench Pts": st.column_config.NumberColumn(
                "Bench Pts", help="Points left on bench", format="%.1f", width="small"
            ),
            ...
            "Close Game": st.column_config.TextColumn(
                "Close Game", help="Decided by < 10 points", width="small"
            ),
            ...
            "W Streak": st.column_config.NumberColumn(
                "W Streak", help="Current winning streak", format="%d", width="small"
            ),
            "L Streak": st.column_config.NumberColumn(
                "L Streak", help="Current losing streak", format="%d", width="small"
            ),
            "Round": st.column_config.TextColumn(
                "Round", help="Playoff round (if applicable)", width="medium"
            ),
            "Win Prob": st.column_config.TextColumn(
                "Win Prob", help="Pre-game win probability", width="small"
            ),
        }
    ...
    def _render_quick_stats(self, stats: dict):
        ...
        if any(
            key in stats
            for key in ["max_gpa", "max_power", "max_win_streak", "perfect_lineups"]
        ):
            st.markdown("---")
