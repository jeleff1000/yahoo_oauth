...
class SeasonAdvancedStatsViewer:
    ...
    def _calculate_stats(self, df: pd.DataFrame) -> dict:
        ...
        above_avg_rate = (
            (
                df["Above Lg Avg"].sum()
                / (df["Above Lg Avg"].sum() + df["Below Lg Avg"].sum())
                * 100
            )
            if (df["Above Lg Avg"].sum() + df["Below Lg Avg"].sum()) > 0
            else 0
        )
    ...
    def _render_enhanced_table(self, df: pd.DataFrame, prefix: str, per_game: bool):
        ...
        column_config = {
            ...
            "GPA": st.column_config.NumberColumn(
                "GPA", help="Career grade point average", format="%.2f", width="small"
            ),
            ...
            "Team Mean": st.column_config.NumberColumn(
                "Team Mean", help="Team career mean score", format="%.2f", width="small"
            ),
    ...
    def _render_quick_stats(self, stats: dict):
        ...
        with col6:
            ...
            close_pct = (
                (stats["total_close_w"] / stats["total_close"] * 100)
                if stats["total_close"] > 0
                else 0
            )
            st.metric(
                "Close Game Record",
