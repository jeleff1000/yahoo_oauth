...
class CareerMatchupStatsViewer:
    ...
    def _prepare_display_data(self, prefix) -> pd.DataFrame:
        ...
        unique_games = df.groupby("manager")["manager_year"].apply(
            lambda x: len(df[df["manager_year"].isin(x)])
        )
        ...
        champion_count = (
            df.groupby(["manager", "year"])["champion"].max().groupby("manager").sum()
            if "champion" in df.columns
            else None
        )
        sacko_count = (
            df.groupby(["manager", "year"])["sacko"].max().groupby("manager").sum()
            if "sacko" in df.columns
            else None
        )
        playoffs_count = (
            df.groupby(["manager", "year"])["is_playoffs"]
            .max()
            .groupby("manager")
            .sum()
        ...
        if "team_points" in df.columns:
            ...
            aggregated_df = pd.merge(
                aggregated_df, worst_week, on="manager", how="left"
            )
        ...
        if "win" in aggregated_df.columns and "loss" in aggregated_df.columns:
            ...
            aggregated_df["win_pct"] = (
                aggregated_df["win"] / aggregated_df["games"] * 100
            ).round(1)

        if "team_points" in aggregated_df.columns and "games" in aggregated_df.columns:
            aggregated_df["ppg"] = (
                aggregated_df["team_points"] / aggregated_df["games"]
            ).round(2)

        if (
            "opponent_points" in aggregated_df.columns
            and "games" in aggregated_df.columns
        ):
            aggregated_df["papg"] = (
                aggregated_df["opponent_points"] / aggregated_df["games"]
            ).round(2)

        if (
            "team_points" in aggregated_df.columns
            and "opponent_points" in aggregated_df.columns
        ):
            aggregated_df["point_diff"] = (
                aggregated_df["team_points"] - aggregated_df["opponent_points"]
            ).round(1)

        # Championship/playoff rates (per season)
        if (
            "champion" in aggregated_df.columns
            and "unique_years" in aggregated_df.columns
        ):
            aggregated_df["champ_rate"] = (
                aggregated_df["champion"] / aggregated_df["unique_years"] * 100
            ).round(1)

        if "sacko" in aggregated_df.columns and "unique_years" in aggregated_df.columns:
            aggregated_df["sacko_rate"] = (
                aggregated_df["sacko"] / aggregated_df["unique_years"] * 100
            ).round(1)

        if (
            "is_playoffs" in aggregated_df.columns
            and "unique_years" in aggregated_df.columns
        ):
            aggregated_df["playoff_rate"] = (
                aggregated_df["is_playoffs"] / aggregated_df["unique_years"] * 100
            ).round(1)
        ...
        display_df = aggregated_df[
            [c for c in display_cols if c in aggregated_df.columns]
        ].copy()
    ...
    def _calculate_stats(self, df: pd.DataFrame) -> dict:
        ...
        # Win rate
        avg_win_pct = (
            df["Win %"].mean() if "Win %" in df.columns and total_managers > 0 else 0
        )

        # Scoring
        avg_ppg = df["PPG"].mean() if "PPG" in df.columns and total_managers > 0 else 0
        avg_papg = (
            df["PA/G"].mean() if "PA/G" in df.columns and total_managers > 0 else 0
        )
        best_week = (
            df["Best"].max() if "Best" in df.columns and total_managers > 0 else 0
        )
        ...
        avg_champ_rate = (
            df["Champ %"].mean()
            if "Champ %" in df.columns and total_managers > 0
            else 0
        )
        avg_playoff_rate = (
            df["Playoff %"].mean()
            if "Playoff %" in df.columns and total_managers > 0
            else 0
        )
        avg_sacko_rate = (
            df["Sacko %"].mean()
            if "Sacko %" in df.columns and total_managers > 0
            else 0
        )
    ...
    def _render_enhanced_table(self, df: pd.DataFrame, prefix: str, per_game: bool):
        ...
        column_config = {
            ...
            "Seasons": st.column_config.NumberColumn(
                "Seasons", help="Number of seasons played", format="%d", width="small"
            ),
            ...
            "PF": st.column_config.NumberColumn(
                "PF", help="Total points for", format="%.2f", width="small"
            ),
            "PA": st.column_config.NumberColumn(
                "PA", help="Total points against", format="%.2f", width="small"
            ),
            "Best": st.column_config.NumberColumn(
                "Best", help="Best single-week score", format="%.2f", width="small"
            ),
            "Worst": st.column_config.NumberColumn(
                "Worst", help="Worst single-week score", format="%.2f", width="small"
            ),
            ...
            "Playoffs": st.column_config.NumberColumn(
                "Playoffs", help="Playoff appearances", format="%d", width="small"
            ),
            ...
            "Champs": st.column_config.NumberColumn(
                "Champs", help="Championships won", format="%d", width="small"
            ),
            ...
            "Sackos": st.column_config.NumberColumn(
                "Sackos", help="Last place finishes", format="%d", width="small"
            ),
    ...
    def _render_quick_stats(self, stats: dict):
        ...
        with col3:
            st.metric(
                "Total Games", f"{stats['total_games']:,}", help="Total games played"
            )
        ...
        with col5:
            st.metric(
                "Avg PPG", f"{stats['avg_ppg']:.2f}", help="Average points per game"
            )
        ...
        with col13:
            st.metric(
                "Sackos", f"{stats['total_sackos']:,}", help="Total last place finishes"
            )
        ...
        with col16:
            competitiveness = (
                (stats["avg_champ_rate"] + stats["avg_playoff_rate"]) / 2
                if stats["avg_playoff_rate"] > 0
                else 0
            )
            st.metric(
                "Competitiveness",
