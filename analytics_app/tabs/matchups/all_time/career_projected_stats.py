...
class SeasonProjectedStatsViewer:
    ...
    def _prepare_display_data(self, prefix) -> pd.DataFrame:
        ...
        # Spread performance
        df["win_vs_spread"] = (df["team_points"] - df["opponent_points"]) > df[
            "expected_spread"
        ]
        df["loss_vs_spread"] = (df["team_points"] - df["opponent_points"]) <= df[
            "expected_spread"
        ]
        ...
        display_df["proj_accuracy"] = (
            display_df["accurate"] / display_df["games"] * 100
        ).round(1)
        display_df["spread_win_rate"] = (
            display_df["win_vs_spread"] / display_df["games"] * 100
        ).round(1)
    ...
    def _calculate_stats(self, df: pd.DataFrame) -> dict:
        ...
        # Projection accuracy
        overall_accuracy = (
            (df["Accurate"].sum() / total_games * 100) if total_games > 0 else 0
        )
        avg_abs_error = (
            df["Abs Proj Error"].sum() / total_games if total_games > 0 else 0
        )

        # Spread performance
        total_spread_wins = df["W vs Spread"].sum()
        spread_win_rate = (
            (total_spread_wins / total_games * 100) if total_games > 0 else 0
        )
        ...
        moderate_pct = (
            (df["Moderate"].sum() / total_games * 100) if total_games > 0 else 0
        )
        inaccurate_pct = (
            (df["Inaccurate"].sum() / total_games * 100) if total_games > 0 else 0
        )
    ...
    def _render_enhanced_table(self, df: pd.DataFrame, prefix: str, per_game: bool):
        ...
        column_config = {
            ...
            "PF": st.column_config.NumberColumn(
                "PF", help="Points for", format="%.2f", width="small"
            ),
            "PA": st.column_config.NumberColumn(
                "PA", help="Points against", format="%.2f", width="small"
            ),
            "Proj PF": st.column_config.NumberColumn(
                "Proj PF", help="Projected points for", format="%.2f", width="small"
            ),
            "Proj PA": st.column_config.NumberColumn(
                "Proj PA", help="Projected points against", format="%.2f", width="small"
            ),
            ...
            "Avg Odds": st.column_config.NumberColumn(
                "Avg Odds", help="Average win probability", format="%.3f", width="small"
            ),
    ...
    def _render_quick_stats(self, stats: dict):
        ...
        with col10:
            st.metric(
                "Total Upsets", f"{stats['total_upsets']:,}", help="Wins as underdog"
            )
