...
class KeeperDataViewer:
    ...
    def display(self):
        ...
        year_manager_combos = (
            df.groupby(["year", "manager"]).size().reset_index(name="roster_size")
        )
        num_years = df["year"].nunique()
        num_managers_per_year = (
            year_manager_combos.groupby("year")["manager"].nunique().mean()
        )
        ...
        total_kept = (
            df[df["kept_next_year"]].groupby(["player", "year"]).size().shape[0]
        )

        # Average keeper price for players who were actually kept
        avg_keeper_price = df[df["kept_next_year"]]["keeper_price"].mean()
        ...
        with col3:
            st.metric(
                "Avg Keeper Cost",
                f"${avg_keeper_price:.1f}" if pd.notna(avg_keeper_price) else "N/A",
                help="Average cost of players who were actually kept",
            )
        with col4:
            keep_rate = (
                (total_kept / total_keeper_slots * 100) if total_keeper_slots > 0 else 0
            )
            st.metric(
                "Keep Rate",
                f"{keep_rate:.1f}%",
    ...
    def _display_keeper_explorer(self, df):
        ...
        with col1:
            selected_years = st.multiselect(
                ...
                key="keeper_explorer_years",
            )
            selected_managers = st.multiselect(
                "Manager(s)", managers, default=[], key="keeper_explorer_managers"
            )
        with col2:
            selected_positions = st.multiselect(
                "Position(s)", positions, default=[], key="keeper_explorer_positions"
            )
            show_only = st.radio(
                "Show",
                ["All Players", "Keepers Only"],
                horizontal=True,
                key="keeper_explorer_show",
            )
        with col3:
        ...
        if show_only == "Keepers Only":
            filtered = filtered[filtered["is_keeper_status"]]
        ...
        if "cost" in display_df.columns and "spar" in display_df.columns:
            display_df["spar_per_dollar"] = (
                display_df["spar"] / display_df["cost"]
            ).fillna(0)
        ...
        st.dataframe(
            ...
            column_config={
                "headshot_url": st.column_config.ImageColumn(
                    "", width="small", help="Player headshot"
                ),
                ...
                "keeper_price": st.column_config.NumberColumn(
                    "Keep $", width="small", format="$%.0f", help="Keeper price"
                ),
                "cost": st.column_config.NumberColumn(
                    "Draft $", width="small", format="$%.0f", help="Draft cost"
                ),
                ...
                "is_keeper_status": st.column_config.CheckboxColumn(
                    "Keeper", width="small", help="Is this a keeper?"
                ),
                ...
                "spar_per_dollar": st.column_config.NumberColumn(
                    "SPAR/$", width="small", format="%.2f", help="SPAR per cost dollar"
                ),
            },
            hide_index=True,
            use_container_width=True,
    ...
    def _display_analytics(self, df):
        ...
        with col1:
            selected_year = st.selectbox(
                "Filter by Year", ["All Years"] + years, index=0, key="analytics_year"
            )
        with col2:
        ...
        keepers = filtered_df[filtered_df["is_keeper_status"]].copy()
        ...
        with col1:
            ...
            pos_counts = (
                keepers.groupby("yahoo_position").size().sort_values(ascending=True)
            )
            fig = go.Figure(
                go.Bar(
                    x=pos_counts.values,
        ...
        with col2:
            ...
            keepers_with_cost["spar_per_dollar"] = (
                keepers_with_cost["spar"] / keepers_with_cost["cost"]
            )
            avg_spar_efficiency = (
                keepers_with_cost.groupby("yahoo_position")["spar_per_dollar"]
                .mean()
        ...
        if selected_year == "All Years" and len(keepers["year"].unique()) > 1:
            ...
            year_trends = (
                keepers.groupby("year")
                .agg({"player": "count", "keeper_price": "mean"})
                .reset_index()
            )
    ...
    def _display_best_keepers(self, df):
        ...
        with col2:
            ...
            efficiency_df["spar_per_dollar"] = (
                efficiency_df["spar"] / efficiency_df["cost"]
            )
        ...
        with col4:
            ...
            manager_keeper_df["spar_per_dollar"] = (
                manager_keeper_df["spar"] / manager_keeper_df["cost"]
            )

            manager_stats = (
                manager_keeper_df.groupby("manager")
                .agg(
                    {
                        "player": "count",
