...
class CombinedMatchupStatsViewer:
    ...
    def display(self, prefix):
        ...
        if "Slot" in df.columns:
            df["position_rank"] = (
                df.groupby(["Manager", "Week", "Year", "Slot"])["Pts"]
            ...
            df["Slot"] = df.apply(
                lambda row: (
                    f"{row['Slot']}{row['position_rank']}"
                    if row["Slot"] in ["RB", "WR"]
                    else row["Slot"]
                ),
                axis=1,
            )
