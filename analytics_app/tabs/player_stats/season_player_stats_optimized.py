...
from md.tab_data_access.players import (
    load_season_player_data as load_players_season_data,
)
...
from .season_player_subprocesses.season_player_matchup_stats import (
...
class OptimizedSeasonPlayerViewer:
    ...
    def display_sort_options(self, viewer_type: str, active_position: str = None):
        ...
        with col1:
            sort_col = st.selectbox(
                ...
                available_cols,
                index=(
                    available_cols.index(st.session_state[sort_col_key])
                    if st.session_state[sort_col_key] in available_cols
                    else 0
                ),
    ...
    def display(self):
        ...
        # Create tabs
        tabs = st.tabs(
            ["Basic Stats", "Advanced Stats", "Matchup Stats", "Optimal Lineup"]
        )
        ...
        # ==================== OPTIMAL LINEUP TAB ====================
        with tabs[3]:
            from .season_player_subprocesses.optimal_lineup_season import (
                OptimalLineupSeasonViewer,
            )
        ...
        with st.spinner("Loading season data..."):
            ...
            if season_data is not None and not season_data.empty:
                ...
                self.display_basic.display_table_with_load_more(
                    basic_stats_df, total_available=len(basic_stats_df)
                )
            else:
        ...
        with st.spinner("Loading advanced season data..."):
            ...
            if season_data is not None and not season_data.empty:
                advanced_stats_df = get_advanced_stats(
                    season_data, active_position or "All"
                )
                self.display_advanced.display_table_with_load_more(
                    advanced_stats_df, total_available=len(advanced_stats_df)
                )
            else:
                st.warning("No data available for the selected filters.")
