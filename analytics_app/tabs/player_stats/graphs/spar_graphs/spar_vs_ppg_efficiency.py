...
def display_spar_vs_ppg_efficiency(prefix=""):
    ...
    spar_data = (
        ...
        .agg({"player_spar": "sum", "nfl_position": "first"})
        .reset_index()
    )
    ...
    season_data = load_players_season_data(
        year=[int(selected_year)], rostered_only=True
    )
    ...
    season_data = season_data.merge(
        spar_data, on="player", how="inner", suffixes=("", "_spar")
    )
    ...
    # Convert to numeric
    season_data["player_spar"] = pd.to_numeric(
        season_data["player_spar"], errors="coerce"
    )
    season_data["season_ppg"] = pd.to_numeric(
        season_data["season_ppg"], errors="coerce"
    )
    season_data["fantasy_games"] = pd.to_numeric(
        season_data.get("fantasy_games", season_data.get("games_played", 0)),
        errors="coerce",
    )
    ...
    season_data["efficiency_ratio"] = (
        season_data["player_spar"] / season_data["season_ppg"]
    )
    ...
    with col1:
        ...
        top_efficiency = top_efficiency.rename(
            columns={
                "player": "Player",
                "nfl_position": "Pos",
                "season_ppg": "PPG",
        ...
        top_efficiency["SPAR/PPG"] = top_efficiency["SPAR/PPG"].apply(
            lambda x: f"{x:.3f}"
        )
    ...
    with col2:
        ...
        bottom_efficiency = bottom_efficiency.rename(
            columns={
                "player": "Player",
                "nfl_position": "Pos",
                "season_ppg": "PPG",
        ...
        bottom_efficiency["SPAR"] = bottom_efficiency["SPAR"].apply(
            lambda x: f"{x:.2f}"
        )
        bottom_efficiency["SPAR/PPG"] = bottom_efficiency["SPAR/PPG"].apply(
            lambda x: f"{x:.3f}"
        )
