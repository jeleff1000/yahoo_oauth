...
def display_weekly_spar_waterfall(prefix=""):
    ...
    if (
        "player_weekly_spar" not in weekly_data.columns
        or "manager_weekly_spar" not in weekly_data.columns
    ):
        st.error(
            "Weekly SPAR data not available. Make sure SPAR calculations have been run."
        )
        return

    # Convert to numeric
    weekly_data["player_weekly_spar"] = pd.to_numeric(
        weekly_data["player_weekly_spar"], errors="coerce"
    )
    weekly_data["manager_weekly_spar"] = pd.to_numeric(
        weekly_data["manager_weekly_spar"], errors="coerce"
    )
    ...
    weekly_data["missed_spar"] = (
        weekly_data["player_weekly_spar"] - weekly_data["manager_weekly_spar"]
    )
    ...
    colors = [
        "#10B981" if ms > 0 else "#6B7280" for ms in weekly_data["manager_weekly_spar"]
    ]
