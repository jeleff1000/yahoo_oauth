...
def display_spar_consistency_scatter(prefix=""):
    ...
    player_stats.loc[
        (player_stats["total_spar"] >= median_spar)
        & (player_stats["spar_std"] < median_std),
        "category",
    ] = "🌟 Elite Consistent"
    player_stats.loc[
        (player_stats["total_spar"] >= median_spar)
    ...
    tabs = st.tabs(
        ["🌟 Elite Consistent", "💥 Boom/Bust", "📊 Low Consistent", "❌ Volatile"]
    )
    ...
    for tab, (cat, desc) in zip(tabs, categories):
        with tab:
            ...
            if not cat_data.empty:
                cat_data = cat_data.sort_values("total_spar", ascending=False)[
                    ["player", "position", "total_spar", "spar_std", "cv", "games"]
                ].head(10)
                ...
                cat_data["Total SPAR"] = cat_data["Total SPAR"].apply(
                    lambda x: f"{x:.2f}"
                )
