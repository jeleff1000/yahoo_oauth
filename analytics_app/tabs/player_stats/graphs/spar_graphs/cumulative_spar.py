...
def display_cumulative_spar_over_season(prefix=""):
    ...
    if (
        "player_weekly_spar" not in filtered.columns
        or "manager_weekly_spar" not in filtered.columns
    ):
        st.error("Weekly SPAR data not available.")
        return

    # Convert to numeric
    filtered["player_weekly_spar"] = pd.to_numeric(
        filtered["player_weekly_spar"], errors="coerce"
    )
    filtered["manager_weekly_spar"] = pd.to_numeric(
        filtered["manager_weekly_spar"], errors="coerce"
    )
    ...
    for player_name in players:
        ...
        player_df["cumulative_player_spar"] = player_df["player_weekly_spar"].cumsum()
        player_df["cumulative_manager_spar"] = player_df["manager_weekly_spar"].cumsum()
        player_df["cumulative_gap"] = (
            player_df["cumulative_player_spar"] - player_df["cumulative_manager_spar"]
        )
        ...
        st.plotly_chart(
            fig2, use_container_width=True, key=f"{prefix}_gap_{player_name}"
        )
    ...
    with st.expander("📋 Season Totals"):
        ...
        for player_name in players:
            ...
            capture_pct = (
                (total_manager / total_player * 100) if total_player > 0 else 0
            )
