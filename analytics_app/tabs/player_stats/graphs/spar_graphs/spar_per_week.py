...
def display_spar_per_week_played(prefix=""):
        ...
        .agg(
            {
                "player_spar": [
                    "sum",
                    "count",
                ],  # sum for total, count for weeks played
                "manager_spar": "sum",
                "nfl_position": "first",
                "manager": "first",
            }
    ...
    weeks_started = (
        weekly_data[weekly_data["manager_spar"] > 0]
        .groupby("player")
        .size()
        .reset_index(name="weeks_started")
    )
    ...
    season_data["player_spar_per_week"] = (
        season_data["player_spar"] / season_data["weeks_played"]
    )
    season_data["manager_spar_per_week"] = (
        season_data["manager_spar"] / season_data["weeks_started"]
    )
    ...
    with st.expander("📊 Top Performers by SPAR/Week"):
        top_data = season_data.nlargest(15, "player_spar_per_week")[
            [
                "player",
                "nfl_position",
                "player_spar_per_week",
                "manager_spar_per_week",
                "weeks_played",
                "weeks_started",
            ]
        ].copy()

        top_data["capture_rate"] = (
            top_data["manager_spar_per_week"] / top_data["player_spar_per_week"] * 100
        ).round(1)
        ...
        top_data["Player SPAR/wk"] = top_data["Player SPAR/wk"].apply(
            lambda x: f"{x:.2f}"
        )
        top_data["Manager SPAR/wk"] = top_data["Manager SPAR/wk"].apply(
            lambda x: f"{x:.2f}"
        )
