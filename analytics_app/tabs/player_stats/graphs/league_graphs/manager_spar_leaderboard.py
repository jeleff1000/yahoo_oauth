...
def display_manager_spar_leaderboard(prefix=""):
    ...
    manager_stats = (
        ...
        .agg({"player_spar": "sum", "manager_spar": "sum", "missed_spar": "sum"})
        .reset_index()
    )
    ...
    with st.expander("📊 Breakdown by Position"):
        ...
        if "nfl_position" in season_data.columns:
            position_breakdown = (
                season_data.groupby(["manager", "nfl_position"])
                .agg(
                    {"player_spar": "sum", "manager_spar": "sum", "missed_spar": "sum"}
                )
                .reset_index()
            )

            position_breakdown["capture_rate"] = (
                position_breakdown["manager_spar"]
                / position_breakdown["player_spar"]
                * 100
            ).fillna(0)

            # Pivot for display
            pivot_player = position_breakdown.pivot(
                index="manager", columns="nfl_position", values="player_spar"
            ).fillna(0)

            pivot_manager = position_breakdown.pivot(
                index="manager", columns="nfl_position", values="manager_spar"
            ).fillna(0)

            pivot_capture = position_breakdown.pivot(
                index="manager", columns="nfl_position", values="capture_rate"
            ).fillna(0)
            ...
            with tab1:
                st.dataframe(
                    pivot_player.style.format("{:.1f}"), use_container_width=True
                )

            with tab2:
                st.dataframe(
                    pivot_manager.style.format("{:.1f}"), use_container_width=True
                )

            with tab3:
                st.dataframe(
                    pivot_capture.style.format("{:.1f}%"), use_container_width=True
                )
