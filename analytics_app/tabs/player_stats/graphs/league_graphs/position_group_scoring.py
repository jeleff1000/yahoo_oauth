...
def display_position_group_scoring_graphs(prefix=""):
    ...
    # Manager filter
    managers = sorted(
        [m for m in player_data["manager"].dropna().unique() if m and str(m).strip()]
    )

    if managers:
        manager_options = [
            "All Players (1999+)",
            "All Rostered Players (2014+)",
        ] + managers
        ...
        if manager_filter == "All Players (1999+)":
            ...
            player_data = player_data[
                player_data["manager"].str.contains(manager_filter, na=False)
            ]
    else:
        st.info(
            "No manager data available - showing all players from full history (1999+)"
        )

    # Position selection
    position_order = ["QB", "RB", "WR", "TE", "K", "DEF"]
    available_positions = [
        pos for pos in position_order if pos in player_data["nfl_position"].unique()
    ]
    ...
    with col1:
        ...
        summary = agg.groupby("nfl_position", as_index=False).agg(
            {"avg_ppg": "mean", "points": "sum", "fantasy_games": "sum"}
        )
