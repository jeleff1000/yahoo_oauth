...
def _get_top_players_by_position(
    df: pd.DataFrame,
    position: str,
    limit: int,
    criteria: str,
    use_season_ppg: bool = True,
) -> pd.DataFrame:
    ...
    if has_year:
        ...
        for _, row in df.iterrows():
            ...
            if weekly_stats is not None and not weekly_stats.empty:
                row["max_week"] = (
                    float(weekly_stats.iloc[0]["max_week"])
                    if pd.notna(weekly_stats.iloc[0]["max_week"])
                    else 0.0
                )
                row["times_optimal"] = (
                    int(weekly_stats.iloc[0]["times_optimal"])
                    if pd.notna(weekly_stats.iloc[0]["times_optimal"])
                    else 0
                )
                row["headshot_url"] = (
                    str(weekly_stats.iloc[0]["headshot_url"])
                    if pd.notna(weekly_stats.iloc[0]["headshot_url"])
                    else DEFAULT_HEADSHOT
                )
            else:
                row["max_week"] = 0.0
                row["times_optimal"] = 0
        ...
        df = df.merge(
            weekly_stats[
                [
                    "player_key" if merge_key == "player_key" else "player",
                    "max_week",
                    "times_optimal",
                    "headshot_url",
                ]
...
def display_optimal_season_visual():
    ...
    if not selected_players.empty and "player_key" in selected_players.columns:
        flex_eligible = flex_eligible[
            ~flex_eligible["player_key"].isin(selected_players["player_key"])
        ]
    elif not selected_players.empty and "player" in selected_players.columns:
        flex_eligible = flex_eligible[
            ~flex_eligible["player"].isin(selected_players["player"])
        ]

    if not flex_eligible.empty:
        flex = _get_top_players_by_position(
            flex_eligible,
            flex_eligible["nfl_position"].iloc[0],
            1,
            criteria,
            use_season_ppg=True,
        )
...
def display_optimal_career_visual():
    ...
    # 1 DEF
    def_team = _get_top_players_by_position(
        df, "DEF", 1, criteria, use_season_ppg=False
    )
    lineup_parts.append(def_team)
    ...
    # Display visual table
    _render_optimal_visual_table(
        lineup, f"{start_year}-{end_year}", criteria, use_season_ppg=False
    )


@st.fragment
def _render_optimal_visual_table(
    data: pd.DataFrame, year_label: str, criteria: str, use_season_ppg: bool = True
):
    """
    ...
    total_max_week = (
        float(data["max_week"].sum()) if "max_week" in data.columns else 0.0
    )
    total_times_optimal = (
        int(data["times_optimal"].sum()) if "times_optimal" in data.columns else 0
    )
    ...
    for _, row in data.iterrows():
        ...
        html.append(
            f"<img src='{headshot}' class='opt-player-img' alt='{player_name}' loading='lazy' decoding='async'>"
        )
        ...
        html.append(
            f"<td{td_class_2}><div class='opt-stat-cell'>{points:,.1f}</div></td>"
        )
        ...
        html.append(
            f"<td{td_class_4}><div class='opt-stat-cell'>{max_week:.1f}</div></td>"
        )

        td_class_5 = ' class="highlight-col"' if highlight_col == 5 else ""
        html.append(
            f"<td{td_class_5}><div class='opt-stat-cell'>{times_optimal}</div></td>"
        )
