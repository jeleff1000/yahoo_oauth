...
class PaginationManager:
    ...
    def display_controls(self, df: pd.DataFrame):
        ...
        with col1:
            if st.button(
                "⏮️ First", key=f"{self.key_prefix}_first", disabled=(current_page == 1)
            ):
                st.session_state[self.offset_key] = 0
                st.rerun()
        ...
        with col2:
            if st.button(
                "◀️ Prev", key=f"{self.key_prefix}_prev", disabled=(current_page == 1)
            ):
                st.session_state[self.offset_key] = max(
                    0, current_offset - current_limit
                )
                st.rerun()

        with col3:
            st.markdown(
                f"**Page {current_page} of {total_pages}** ({total_count:,} total records)"
            )
