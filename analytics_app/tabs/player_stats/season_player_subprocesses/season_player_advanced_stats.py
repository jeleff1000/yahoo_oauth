...
def get_advanced_stats(player_data, position):
    """
    Return advanced stats for season view adapted from weekly advanced logic.
    Data is already aggregated by player+year from the database. This function
    renames columns to the weekly-friendly labels and selects position-specific
    columns (without Week, keeping Year).
    ...
    if position == "All":
        ...
        # Ensure Year formatting
        if 'Year' in df.columns:
            df['Year'] = pd.to_numeric(df['Year'], errors='coerce').astype('Int64').astype(str).str.replace(',', '')

        core_columns = ["Player","Team","Year","Manager","Points","SPAR","Player SPAR","Manager SPAR","Position","Roster Slot"]
        ...
        if 'Points' in result.columns:
            result = result.sort_values("Points", ascending=False, na_position="last").reset_index(drop=True)
    ...
    numeric_columns = [
        ...
        "rushing_yards", "rush_att", "carries", "rushing_tds", "rushing_fumbles", "rushing_fumbles_lost",
        ...
        "fg_made", "fg_att", "fg_pct", "fg_long", "fg_made_0_19",
        "fg_made_20_29", "fg_made_30_39", "fg_made_40_49", "fg_made_50_59", "fg_miss",
        "pat_made", "pat_att", "pat_missed", "def_sacks", "def_sack_yards", "def_qb_hits",
        "def_interceptions", "def_interception_yards", "def_pass_defended", "def_tackles_solo",
        "def_tackle_assists", "def_tackles_with_assist", "def_tackles_for_loss",
        "def_tackles_for_loss_yards", "def_fumbles", "def_fumbles_forced",
        "def_safeties", "def_tds", "pts_allow", "dst_points_allowed", "points_allowed",
        "pass_yds_allowed", "rushing_yds_allowed", "total_yds_allowed",
        "fumble_recovery_opp", "fumble_recovery_tds", "kick_and_punt_ret_td",
        "3_and_outs", "4_dwn_stops"
    ]
    ...
    # Rename to weekly-friendly labels (adapted for season: Year instead of Week)
    column_renames = {
        'player': 'Player',
        'nfl_team': 'Team',
        'year': 'Year',
    ...
    for pos in df['Position'].unique():
        ...
        if pos == 'QB':
            columns = [
                'Player','Team','Year','Manager','Points','SPAR','Player SPAR','Manager SPAR','Position','Roster Slot',
                'Pass Yds','Pass TD','INT Thrown','Comp','Pass Att','Comp%','Air Yds','Pass YAC',
                'Pass 1D','Pass EPA','CPOE','PACR','Rush Yds','Rush Att','YPC','Rush TD','Rush Fum','Rush FL','Rush 1D','Rush EPA','Rush 2PT'
            ]
            # Derived
            ...
            if 'Rush Yds' in pos_df.columns and 'Rush Att' in pos_df.columns:
                pos_df['YPC'] = (pd.to_numeric(pos_df['Rush Yds'], errors='coerce') / pd.to_numeric(pos_df['Rush Att'], errors='coerce')).round(1)

        elif pos == 'RB':
            columns = [
                'Player','Team','Year','Manager','Points','SPAR','Player SPAR','Manager SPAR','Position','Roster Slot',
                'Rush Yds','Rush Att','YPC','Rush TD','Rush Fum','Rush FL','Rush 1D','Rush EPA','Rush 2PT',
                'Rec','Rec Yds','YPR','Rec TD','Targets','Catch%','Rec Fum','Rec FL','Rec 1D','Rec EPA','Rec 2PT','Tgt %','WOPR','RACR','Rec Air Yds','Rec YAC','Air Yds %'
            ]
            if 'Rush Yds' in pos_df.columns and 'Rush Att' in pos_df.columns:
                pos_df['YPC'] = (pd.to_numeric(pos_df['Rush Yds'], errors='coerce') / pd.to_numeric(pos_df['Rush Att'], errors='coerce')).round(1)
            if 'Rec Yds' in pos_df.columns and 'Rec' in pos_df.columns:
                pos_df['YPR'] = (pd.to_numeric(pos_df['Rec Yds'], errors='coerce') / pd.to_numeric(pos_df['Rec'], errors='coerce')).round(1)
            if 'Rec' in pos_df.columns and 'Targets' in pos_df.columns:
                pos_df['Catch%'] = (pd.to_numeric(pos_df['Rec'], errors='coerce') / pd.to_numeric(pos_df['Targets'], errors='coerce') * 100).round(1)
            ...
            columns = [
                'Player','Team','Year','Manager','Points','SPAR','Player SPAR','Manager SPAR','Position','Roster Slot',
                'FGM','FGA','FG%','Long','FG Miss','FG 0-19','FG 20-29','FG 30-39','FG 40-49','FG 50+','XPM','XPA','XP Miss'
            ]

        elif pos == 'DEF':
            columns = [
                'Player','Team','Year','Manager','Points','SPAR','Player SPAR','Manager SPAR','Position','Roster Slot',
                'Sacks','Sack Yds','QB Hits','INT','INT Yds','PD','Solo Tkl','Ast Tkl','Total Tkl','TFL','TFL Yds','Fum Rec','FF','Fum TD','Saf','Def TD','Ret TD','PA','Total PA','Pass Yds Allow','Rush Yds Allow','Total Yds Allow','3&Outs','4D Stops'
            ]
        else:
            columns = ['Player','Team','Year','Manager','Points','SPAR','Player SPAR','Manager SPAR','Position','Roster Slot']
    ...
    if results:
        ...
        # Sort by Points desc only (year already grouped)
        if 'Points' in result.columns:
