...
def display_weekly_add_drop(
    ...
    t["Result"] = (
        t["result_emoji"].fillna("") + " " + t["transaction_result"].fillna("")
    )
    ...
    add_value_col = (
        ...
        if "manager_spar_ros_managed" in t.columns
        else "ppg_after_transaction"
    )
    drop_value_col = (
        "player_spar_ros_total"
        if "player_spar_ros_total" in t.columns
        else "ppg_after_transaction"
    )
    ...
    avg_faab = (
        adds[adds["faab_bid"] > 0]["faab_bid"].mean()
        if "faab_bid" in adds.columns and len(adds[adds["faab_bid"] > 0]) > 0
        else 0
    )

    # Calculate totals using appropriate value columns
    total_value_added = (
        adds[add_value_col].sum() if add_value_col in adds.columns else 0
    )
    total_value_dropped = (
        drops[drop_value_col].sum() if drop_value_col in drops.columns else 0
    )
    ...
    if (
        len(adds) > 0
        and add_value_col in adds.columns
        and adds[add_value_col].notna().any()
    ):
    ...
    if (
        len(drops) > 0
        and drop_value_col in drops.columns
        and drops[drop_value_col].notna().any()
    ):
    ...
    with tab1:
        ...
        if best_pickup is not None or worst_drop is not None:
            ...
            with col1:
                if best_pickup is not None:
                    best_val = (
                        best_pickup[add_value_col]
                        if pd.notna(best_pickup[add_value_col])
                        else 0
                    )
                    faab_val = best_pickup.get("faab_bid", 0)
                    faab_val = faab_val if pd.notna(faab_val) else 0
                    st.success(
                        f"🌟 **Best:** {best_pickup['player_name']} (+{best_val:.1f} {value_label}, ${faab_val:.0f}) - {best_pickup['manager']}"
                    )
            with col2:
                if worst_drop is not None:
                    worst_val = (
                        worst_drop[drop_value_col]
                        if pd.notna(worst_drop[drop_value_col])
                        else 0
                    )
                    st.error(
                        f"💔 **Worst:** {worst_drop['player_name']} ({worst_val:.1f} {value_label} lost) - {worst_drop['manager']}"
                    )

        st.markdown(
            "<div style='margin: 0.75rem 0; border-top: 1px solid rgba(100,116,139,0.3);'></div>",
            unsafe_allow_html=True,
        )
        ...
        with st.expander("🔍 Filters", expanded=False):
            ...
            with col1:
                year_filter = st.selectbox(
                    "Year",
                    ["All"] + sorted(t["year"].unique().tolist(), reverse=True),
                    key="weekly_year",
            ...
            with col2:
                week_filter = st.selectbox(
                    "Week",
                    ["All"] + sorted(t["week"].dropna().unique().tolist()),
                    key="weekly_week",
                    label_visibility="collapsed",
                )
            with col3:
                manager_filter = st.selectbox(
                    "Manager",
                    ["All"] + sorted(t["manager"].dropna().unique().tolist()),
                    key="weekly_manager",
                    label_visibility="collapsed",
                )
            with col4:
            ...
            with col5:
                ...
                if "transaction_grade" in t.columns:
                    grade_options += sorted(
                        [
                            g
                            for g in t["transaction_grade"].dropna().unique().tolist()
                            if g
                        ]
                    )
            ...
            with col6:
                sort_by = st.selectbox(
                    "Sort",
                    ["Recent First", "Score ↓", "SPAR ↓", "Grade ↓", "FAAB ↓", "ROI ↓"],
                    key="weekly_sort",
                    label_visibility="collapsed",
                )
            ...
            with col3:
                ...
                if "transaction_result" in t.columns:
                    result_options += sorted(
                        t["transaction_result"].dropna().unique().tolist()
                    )
        ...
        if "transaction_id" in filtered.columns and len(filtered) > 0:
            ...
            add_grouped = (
                adds_filtered.groupby("transaction_id").agg(agg_dict).reset_index()
            )
            ...
            drop_grouped = (
                drops_filtered.groupby("transaction_id").agg(drop_cols).reset_index()
            )
            ...
            combined["manager"] = safe_coalesce(
                combined, "manager_added", "manager_dropped", ""
            )

            combined["player_added"] = safe_coalesce(
                combined, "player_name_added", "player_name", "(None)"
            )
            combined["pos_added"] = safe_coalesce(
                combined, "position_added", "position", ""
            )
            combined["player_dropped"] = safe_coalesce(
                combined, "player_name_dropped", default="(None)"
            )
            combined["pos_dropped"] = safe_coalesce(
                combined, "position_dropped", default=""
            )

            # Numeric columns with safe handling
            if add_value_col in combined.columns:
                combined["spar_added"] = pd.to_numeric(
                    combined[add_value_col], errors="coerce"
                ).fillna(0)
            else:
            ...
            drop_spar_col = (
                f"{drop_value_col}_dropped"
                if f"{drop_value_col}_dropped" in combined.columns
                else drop_value_col
            )
            if drop_spar_col in combined.columns:
                combined["spar_dropped"] = pd.to_numeric(
                    combined[drop_spar_col], errors="coerce"
                ).fillna(0)
            else:
            ...
            combined["faab"] = (
                pd.to_numeric(combined["faab_bid"], errors="coerce").fillna(0)
                if "faab_bid" in combined.columns
                else 0
            )
            ...
            combined["grade"] = (
                combined["transaction_grade"].fillna("")
                if "transaction_grade" in combined.columns
                else ""
            )
            combined["value_tier"] = (
                combined["faab_value_tier"].fillna("")
                if "faab_value_tier" in combined.columns
                else ""
            )
            combined["regret"] = (
                combined["drop_regret_tier"].fillna("")
                if "drop_regret_tier" in combined.columns
                else ""
            )
            ...
            if sort_by == "Score ↓":
                ...
                combined = combined.sort_values(
                    "efficiency", ascending=False, na_position="last"
                )
            else:
                combined = combined.sort_values(
                    ["year", "week"], ascending=[False, False]
                )
            ...
            def style_spar_column(val):
                ...
                if val > 0:
                    ...
                    return (
                        f"background-color: rgba(239, 68, 68, {intensity * 0.4})"  # Red
                    )

            st.markdown(
                f"**{len(display_data):,} transactions** (from {len(filtered):,} adds/drops)"
            )

            # Create styled dataframe
            display_data.style.applymap(
                style_spar_column,
                subset=[
                    c
                    for c in ["spar_added", "spar_dropped", "net_spar"]
            ...
            st.dataframe(
                ...
                column_config={
                    "week": st.column_config.NumberColumn(
                        "Wk", format="%d", width="small", help="Week of transaction"
                    ),
                    "year": st.column_config.TextColumn("Yr", width="small"),
                    "manager": st.column_config.TextColumn("Manager", width="medium"),
                    "grade": st.column_config.TextColumn(
                        "📊", width="small", help="Transaction grade (A-F)"
                    ),
                    "player_added": st.column_config.TextColumn(
                        "➕ Added", width="medium"
                    ),
                    "pos_added": st.column_config.TextColumn("Pos", width="small"),
                    "spar_added": st.column_config.NumberColumn(
                        "SPAR+", format="%.1f", help="SPAR gained from pickup"
                    ),
                    "player_dropped": st.column_config.TextColumn(
                        "➖ Dropped", width="medium"
                    ),
                    "pos_dropped": st.column_config.TextColumn("Pos", width="small"),
                    "spar_dropped": st.column_config.NumberColumn(
                        "SPAR-", format="%.1f", help="SPAR lost from drop"
                    ),
                    "net_spar": st.column_config.NumberColumn(
                        "NET", format="%+.1f", help="Net SPAR = SPAR+ minus SPAR-"
                    ),
                    "faab": st.column_config.NumberColumn(
                        "💰", format="$%.0f", help="FAAB spent"
                    ),
                    "efficiency": st.column_config.NumberColumn(
                        "$/PT", format="%.2f", help="SPAR per FAAB dollar"
                    ),
                    "Result": st.column_config.TextColumn(
                        "Outcome",
                        width="medium",
                        help="Transaction result classification",
                    ),
                },
            )
            ...
            st.download_button(
                "📥 Export",
                csv,
                f"weekly_add_drops_{year_filter}.csv",
                "text/csv",
                key="weekly_export",
            )
...
def _render_analytics_tab(
    ...
    with col1:
        ...
        if "transaction_grade" in t.columns and t["transaction_grade"].notna().any():
            ...
            if HAS_CHART_THEMES:
                fig = create_grade_bar_chart(
                    grade_counts.to_dict(), "Transactions by Grade"
                )
            else:
            ...
            total_points = sum(
                grade_points.get(g, 0) * c for g, c in grade_counts.items()
            )
            ...
            st.info(
                "Grade data not available - regenerate source data with new engagement metrics"
            )
    ...
    with col2:
        ...
        if "faab_value_tier" in adds.columns and adds["faab_value_tier"].notna().any():
            ...
            tier_counts = tier_counts.reindex(
                [tc for tc in tier_order if tc in tier_counts.index]
            )

            # Use themed chart if available
            if HAS_CHART_THEMES:
                fig = create_faab_tier_chart(
                    tier_counts.to_dict(), "FAAB Value Distribution (Adds Only)"
                )
            else:
                colors = ["#0d47a1", "#1f77b4", "#2ca02c", "#ffbb78", "#d62728"][
                    : len(tier_counts)
                ]
                fig = go.Figure(
                    go.Bar(
    ...
    with col1:
        if (
            "drop_regret_tier" in drops.columns
            and drops["drop_regret_tier"].notna().any()
        ):
            ...
            regret_counts = regret_counts.reindex(
                [tc for tc in tier_order if tc in regret_counts.index]
            )

            # Use themed chart if available
            if HAS_CHART_THEMES:
                fig = create_regret_bar_chart(
                    regret_counts.to_dict(), "Drop Regret Distribution"
                )
            else:
                colors = [
                    "#2ca02c",
    ...
    with col2:
        if (
            "drop_regret_score" in drops.columns
            and drops["drop_regret_score"].notna().any()
        ):
            # Top regret drops
            ...
            if len(top_regrets) > 0:
                st.dataframe(
                    ...
                    column_config={
                        ...
                        "drop_regret_score": st.column_config.NumberColumn(
                            "SPAR Lost", format="%.1f"
                        ),
    ...
    if "timing_category" in t.columns and t["timing_category"].notna().any():
        timing_data = (
            t.groupby("timing_category")
            .agg({"transaction_type": "count", "net_manager_spar_ros": "mean"})
            .reset_index()
        )
        ...
        timing_data["sort_order"] = timing_data["Timing"].map(
            {tc: i for i, tc in enumerate(timing_order)}
        )
        ...
        with col1:
            fig = px.bar(
                timing_data, x="Timing", y="Count", title="Transaction Volume by Timing"
            )
            if HAS_CHART_THEMES:
...
def _render_leaderboards_tab(
    adds: pd.DataFrame, drops: pd.DataFrame, add_value_col: str, drop_value_col: str
) -> None:
        ...
        if (
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "faab_bid": st.column_config.NumberColumn(
                        "FAAB", format="$%.0f", width="small"
                    ),
                    "transaction_grade": st.column_config.TextColumn(
                        "Grade", width="small"
                    ),
                },
            )
            ...
            if len(steals) > 0:
                ...
                st.dataframe(
                    ...
                    column_config={
                        ...
                        "player_name": st.column_config.TextColumn("Player"),
                        "manager": st.column_config.TextColumn("Manager"),
                        add_value_col: st.column_config.NumberColumn(
                            "SPAR", format="%.1f"
                        ),
                        "faab_bid": st.column_config.NumberColumn(
                            "FAAB", format="$%.0f"
                        ),
                        "spar_efficiency": st.column_config.NumberColumn(
                            "SPAR/$", format="%.2f"
                        ),
                    },
                )
        ...
        if (
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "position": st.column_config.TextColumn("Pos", width="small"),
                    "manager": st.column_config.TextColumn("Manager"),
                    drop_value_col: st.column_config.NumberColumn(
                        "SPAR Lost", format="%.1f"
                    ),
                    "drop_regret_tier": st.column_config.TextColumn("Regret"),
                },
