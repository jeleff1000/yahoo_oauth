...
def display_career_trade_data(transaction_df, player_df, draft_history_df):
    ...
    career_agg["trades_per_season"] = (
        career_agg["total_trades"] / career_agg["seasons_active"]
    )
    career_agg["net_spar_per_season"] = (
        career_agg["career_net_spar"] / career_agg["seasons_active"]
    )
    career_agg["win_rate"] = (
        trade_df.groupby("manager")["net_spar"]
    ...
    career_agg["Rank"] = (
        career_agg["career_net_spar"].rank(ascending=False, method="min").astype(int)
    )
    career_agg["Grade"] = career_agg["career_net_spar"].apply(
        lambda x: (
            ...
            if x > 500
                ...
                "🥈 Great"
                if x > 250
                else "🥉 Good" if x > 100 else "📊 Average" if x > 0 else "📉 Poor"
            )
        )
    )
    ...
    with tab1:
        ...
        with col3:
            st.metric(
                "Avg Career NET SPAR", f"{career_agg['career_net_spar'].mean():.1f}"
            )
        with col4:
            st.metric("Avg Win Rate", f"{career_agg['win_rate'].mean():.1f}%")
        with col5:
            st.metric(
                "Avg Trades/Season", f"{career_agg['trades_per_season'].mean():.1f}"
            )
        ...
        with col3:
            sort_by = st.selectbox(
                "Sort by",
                ["Rank", "NET SPAR", "Win Rate", "Efficiency", "Trades"],
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "seasons_active": st.column_config.NumberColumn(
                    "Seasons", format="%d", width="small"
                ),
                "total_trades": st.column_config.NumberColumn(
                    "Trades", format="%d", width="small"
                ),
                "career_net_spar": st.column_config.NumberColumn(
                    "Career NET", format="%.1f", help="Total NET SPAR across all trades"
                ),
                "avg_net_per_trade": st.column_config.NumberColumn(
                    "Avg NET", format="%.1f", help="Average NET SPAR per trade"
                ),
                "net_spar_per_season": st.column_config.NumberColumn(
                    "NET/Season", format="%.1f", help="NET SPAR per season"
                ),
                "win_rate": st.column_config.NumberColumn("Win %", format="%.1f"),
                "trades_per_season": st.column_config.NumberColumn(
                    "Trades/Szn", format="%.1f", width="small"
                ),
    ...
    with tab2:
        ...
        with col1:
            ...
            fig = px.scatter(
                ...
                labels={
                    ...
                    "career_net_spar": "Career NET SPAR",
                },
            )
        ...
        with col2:
            ...
            if "avg_trade_score" in career_agg.columns:
                fig = px.scatter(
                    ...
                    y="avg_net_per_trade",
                    size="total_trades",
                    color="avg_net_per_trade",
                    hover_data=["manager"],
                    title="Trade Score vs Avg NET SPAR",
                    color_continuous_scale="Viridis",
                    labels={
                        "avg_trade_score": "Avg Trade Score",
        ...
        top10 = career_agg.nlargest(10, "career_net_spar").sort_values(
            "career_net_spar", ascending=True
        )
        ...
        with col1:
            ...
            efficient = (
                career_agg[career_agg["total_trades"] >= 5]
                .nlargest(10, "avg_net_per_trade")[
                    [
                        "manager",
                        "avg_net_per_trade",
                        "career_net_spar",
            ...
            st.dataframe(
                efficient[
                    ["Rank", "manager", "avg_net_per_trade", "total_trades", "win_rate"]
                ],
                hide_index=True,
                use_container_width=True,
                column_config={
        ...
        with col2:
            ...
            st.dataframe(
                best_wr[
                    ["Rank", "manager", "win_rate", "career_net_spar", "total_trades"]
                ],
                hide_index=True,
                use_container_width=True,
                column_config={
    ...
    with tab3:
        ...
        if managers:
            ...
            for manager in managers:
                ...
                values = [
                    (
                        mgr_data["career_net_spar"]
                        / career_agg["career_net_spar"].max()
                        * 100
                        if career_agg["career_net_spar"].max() > 0
                        else 0
                    ),
                    (
                        mgr_data["avg_net_per_trade"]
                        / career_agg["avg_net_per_trade"].max()
                        * 100
                        if pd.notna(mgr_data["avg_net_per_trade"])
                        and career_agg["avg_net_per_trade"].max() > 0
                        else 0
                    ),
                    mgr_data["win_rate"] if pd.notna(mgr_data["win_rate"]) else 0,
                    mgr_data["total_trades"] / career_agg["total_trades"].max() * 100,
                    (
                        mgr_data["avg_trade_score"]
                        / career_agg["avg_trade_score"].max()
                        * 100
                        if "avg_trade_score" in career_agg.columns
                        and pd.notna(mgr_data.get("avg_trade_score"))
                        and career_agg["avg_trade_score"].max() > 0
                        else 0
                    ),
                ]
                fig.add_trace(
            ...
            st.dataframe(
                compare_df[display_cols], hide_index=True, use_container_width=True
            )
