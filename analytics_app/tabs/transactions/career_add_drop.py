...
def display_career_add_drop(transaction_df, player_df):
    ...
    season_df = display_season_add_drop(
        transaction_df, player_df, return_df=True, key_prefix="career_internal"
    )
    ...
    career_agg["spar_per_season"] = (
        career_agg["career_net_spar"] / career_agg["seasons_played"]
    )
    career_agg["score_per_season"] = (
        career_agg["career_total_score"] / career_agg["seasons_played"]
    )
    career_agg["pts_per_season"] = (
        career_agg["career_net_pts"] / career_agg["seasons_played"]
    )
    career_agg["moves_per_season"] = (
        career_agg["total_moves"] / career_agg["seasons_played"]
    )
    career_agg["efficiency"] = career_agg["career_net_spar"] / career_agg["total_moves"]

    # Rankings (based on Total Score - our weighted metric)
    career_agg["Rank"] = (
        career_agg["career_total_score"].rank(ascending=False, method="min").astype(int)
    )
    career_agg["Grade"] = career_agg["career_total_score"].apply(
        lambda x: (
            ...
            if x > 1000
                ...
                "🥈 Great"
                if x > 500
                else "🥉 Good" if x > 250 else "📊 Average" if x > 0 else "📉 Poor"
            )
        )
    )
    ...
    with tab1:
        ...
        with col1:
            ...
            """.format(
                    len(career_agg), int(total_moves)
                ),
                unsafe_allow_html=True,
            )
        ...
        with col3:
            ...
            """.format(
                    avg_score, avg_net_spar, avg_efficiency
                ),
                unsafe_allow_html=True,
            )
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "career_total_score": st.column_config.NumberColumn(
                    "Career Score", format="%+.0f", help="Weighted score"
                ),
                "score_per_season": st.column_config.NumberColumn(
                    "Score/Season", format="%+.1f"
                ),
                "seasons_played": st.column_config.NumberColumn("Seasons", format="%d"),
                "total_moves": st.column_config.NumberColumn(
                    "Total Moves", format="%d"
                ),
                "career_net_spar": st.column_config.NumberColumn(
                    "Career Net SPAR", format="%.1f"
                ),
                "efficiency": st.column_config.NumberColumn("SPAR/Move", format="%.2f"),
                "total_faab": st.column_config.NumberColumn(
                    "Total FAAB", format="$%.0f"
                ),
                "avg_spar_efficiency": st.column_config.NumberColumn(
                    "SPAR/FAAB", format="%.2f", help="Average SPAR per FAAB dollar"
                ),
            },
        )
        ...
        st.download_button(
            "📥 Export", csv, "career_add_drops.csv", "text/csv", key="career_export"
        )
    ...
    with tab2:
        ...
        fig = px.bar(
            career_agg.sort_values("career_net_spar", ascending=False).head(10),
            x="manager",
            y=["total_spar_added", "total_spar_dropped"],
            ...
            for manager in managers:
                ...
                values = [
                    ...
                    * 100,
                    ...
                    (
                        mgr_data["avg_spar_efficiency"]
                        / career_agg["avg_spar_efficiency"].max()
                        * 100
                        if pd.notna(mgr_data["avg_spar_efficiency"])
                        else 0
                    ),
                    mgr_data["total_moves"] / career_agg["total_moves"].max() * 100,
                    (
                        mgr_data["avg_score"] / career_agg["avg_score"].max() * 100
                        if pd.notna(mgr_data["avg_score"])
                        else 0
                    ),
                ]
                fig.add_trace(
