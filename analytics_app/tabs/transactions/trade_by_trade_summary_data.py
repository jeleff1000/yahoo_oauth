...
def display_trade_by_trade_summary_data(
    ...
    spar_col = (
        ...
        else "manager_spar_ros"
    )
    pts_col = (
        "total_points_ros_managed"
        if "total_points_ros_managed" in t.columns
        else "total_points_rest_of_season"
    )
    ppg_col = (
        "ppg_ros_managed" if "ppg_ros_managed" in t.columns else "ppg_after_transaction"
    )
    weeks_col = (
        "weeks_ros_managed"
        if "weeks_ros_managed" in t.columns
        else "weeks_rest_of_season"
    )

    # Group by transaction_id and manager to get each side
    trade_sides = (
        t.groupby(["transaction_id", "manager"])
        .agg(
            {
    ...
    spar_map = trade_sides.set_index(["transaction_id", "manager"])[
        "spar_managed"
    ].to_dict()
    ...
    trade_sides["rank_improvement"] = (
        trade_sides["avg_rank_at"] - trade_sides["avg_rank_after"]
    )
    ...
    players_map = trade_sides.set_index(["transaction_id", "manager"])[
        "players"
    ].to_dict()
    ...
    with tab1:
        ...
        best_trade = (
            both_sides_df.nlargest(1, "net_spar") if len(both_sides_df) > 0 else None
        )
        if best_trade is not None and len(best_trade) > 0:
            best = best_trade.iloc[0]
            st.success(
                f"🌟 **Best Trade:** {best['manager']} (+{best['net_spar']:.1f} NET SPAR) - Gave up {best['players_gave_up']}, Got {best['players_acquired']}"
            )
        ...
        with st.expander("🔍 Filters", expanded=True):
            ...
            with col1:
                year_filter = st.selectbox(
                    "Year",
                    ["All"]
                    + sorted(both_sides_df["year"].unique().tolist(), reverse=True),
                    key="trade_year",
                )
            with col2:
        ...
        if manager_filter:
            filtered = filtered[
                filtered["manager"].str.contains(manager_filter, case=False, na=False)
            ]
        if partner_filter:
            filtered = filtered[
                filtered["partner"].str.contains(partner_filter, case=False, na=False)
            ]
        ...
        st.dataframe(
            ...
            column_config={
                ...
                "spar_managed": st.column_config.NumberColumn(
                    "SPAR Got",
                    format="%.1f",
                    help="SPAR from players you acquired",
                    width="small",
                ),
                "players_gave_up": st.column_config.TextColumn("Gave Up"),
                "partner_spar": st.column_config.NumberColumn(
                    "SPAR Lost",
                    format="%.1f",
                    help="SPAR from players you gave up",
                    width="small",
                ),
                "net_spar": st.column_config.NumberColumn(
                    "NET SPAR", format="%.1f", help="SPAR Got - SPAR Lost"
                ),
        ...
        st.download_button(
            "📥 Export",
            csv,
            f"trades_{year_filter}.csv",
            "text/csv",
            key="trade_by_trade_export",
        )
    ...
    with tab2:
        ...
        mgr_stats["win_rate"] = (
            trade_sides.groupby("manager")["net_spar"]
        ...
        with col1:
            ...
            trade_outcomes = (
                both_sides_df.groupby("transaction_id")["net_spar"]
            ...
            dist_data = (
                trade_outcomes["outcome_bin"].value_counts().sort_index().reset_index()
            )
        ...
        colors = [
            "#2ca02c" if x > 0 else "#d62728" for x in mgr_ranked["career_net_spar"]
        ]
        ...
        fig.add_trace(
            go.Bar(
                ...
                customdata=mgr_ranked[
                    ["total_trades", "avg_net_spar", "win_rate"]
                ].values,
            )
        )
    ...
    with tab3:
            ...
            if len(lopsided_data) > 0:
                ...
                st.dataframe(
                    ...
                    column_config={
                        ...
                        "winner_got": st.column_config.TextColumn(
                            "Got", help="Players winner acquired"
                        ),
                        "loser_name": st.column_config.TextColumn("Loser"),
                        "loser_got": st.column_config.TextColumn(
                            "Got", help="Players loser acquired"
                        ),
                        "net_diff": st.column_config.NumberColumn(
                            "Diff", format="%.1f", help="NET SPAR difference"
                        ),
                    },
                )
            else:
        ...
        with col1:
            ...
            efficient = (
                mgr_stats[mgr_stats["total_trades"] >= 3]
                .nlargest(10, "avg_net_spar")[
            ...
            st.dataframe(
                efficient[
                    ["Rank", "manager", "avg_net_spar", "total_trades", "win_rate"]
                ],
                hide_index=True,
                use_container_width=True,
                column_config={
                    ...
                    "avg_net_spar": st.column_config.NumberColumn(
            ...
            if len(high_spar_data) > 0:
                ...
                st.dataframe(
                    ...
                    column_config={
                        ...
                        "total_spar": st.column_config.NumberColumn(
                            "Total SPAR",
                            format="%.1f",
                            help="Combined SPAR from both sides",
                        ),
                    },
                )
            else:
                ...
                partner_pairs.append(
                    {
                        ...
                        "net_spar_a": (
                            row1["net_spar"]
                            if row1["manager"] == managers[0]
                            else row2["net_spar"]
                        ),
                        "net_spar_b": (
                            row2["net_spar"]
                            if row2["manager"] == managers[1]
                            else row1["net_spar"]
                        ),
                    }
                )
        ...
        if len(partner_pairs) > 0:
            ...
            partner_summary["NET Diff"] = (
                partner_summary["Total NET (A)"] - partner_summary["Total NET (B)"]
            ).abs()
            ...
            st.dataframe(
                ...
                column_config={
                    "Rank": st.column_config.NumberColumn("#", width="small"),
                    "Trading Partners": st.column_config.TextColumn(
                        "Partners", width="large"
                    ),
                    "Trades": st.column_config.NumberColumn(
                        "# Trades", format="%d", width="small"
                    ),
                    "Total NET (A)": st.column_config.NumberColumn(
                        "NET (A)",
                        format="%.1f",
                        help="Total NET SPAR for first manager",
                        width="small",
                    ),
                    "Total NET (B)": st.column_config.NumberColumn(
                        "NET (B)",
                        format="%.1f",
                        help="Total NET SPAR for second manager",
                        width="small",
                    ),
                    "NET Diff": st.column_config.NumberColumn(
                        "Diff",
                        format="%.1f",
                        help="Difference between managers",
                        width="small",
                    ),
