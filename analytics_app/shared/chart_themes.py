...
def create_grade_bar_chart(
    grade_counts: Dict[str, int], title: str = "Grade Distribution", theme: str = None
) -> go.Figure:
    """
...
def create_regret_bar_chart(
    regret_counts: Dict[str, int],
    title: str = "Drop Regret Distribution",
    theme: str = None,
) -> go.Figure:
    """
...
def create_faab_tier_chart(
    tier_counts: Dict[str, int],
    title: str = "FAAB Value Distribution",
    theme: str = None,
) -> go.Figure:
    """
...
def create_horizontal_bar_chart(
    ...
    if color_by_value:
        bar_colors = [
            colors["positive"] if v > 0 else colors["negative"] for v in values
        ]
    else:
    ...
    if color_by_value and any(v > 0 for v in values) and any(v < 0 for v in values):
        fig.add_vline(
            x=0, line_dash="solid", line_color=colors["text_secondary"], line_width=2
        )
