...
def display_optimal_lineup_efficiency_graph(df_dict=None, prefix=""):
    ...
    data["could_have_won"] = (
        (data["win"] == 0) &
        (data["optimal_points"] > data["opponent_points"])
    ).astype(int)
    ...
    # Create tabs
    tab1, tab2, tab3 = st.tabs(["📈 Efficiency Trends", "😢 Missed Opportunities", "📊 Statistics"])
    ...
    with tab1:
        ...
        if selected_year == "All Seasons":
            ...
            filtered_data["game_number"] = filtered_data.groupby("manager").cumcount() + 1
            x_col = "game_number"
            x_title = "Game Number"
        else:
            x_col = "week"
        ...
        for manager in selected_managers:
            manager_data = filtered_data[filtered_data["manager"] == manager].sort_values(x_col)
        ...
        # Add reference lines
        fig_eff.add_hline(y=100, line_dash="dash", line_color="green", opacity=0.5,
                          annotation_text="Perfect (100%)", annotation_position="right")
        fig_eff.add_hline(y=90, line_dash="dot", line_color="orange", opacity=0.3,
                          annotation_text="Good (90%)", annotation_position="right")
        ...
        st.plotly_chart(fig_eff, use_container_width=True, key=f"{prefix}_efficiency_trend")
        ...
        with st.expander("📊 Rolling Average (Last 4 Weeks)", expanded=False):
            ...
            for manager in selected_managers:
                manager_data = filtered_data[filtered_data["manager"] == manager].sort_values(x_col)
                manager_data["rolling_eff"] = manager_data["lineup_efficiency"].rolling(window=4, min_periods=1).mean()
            ...
            fig_rolling.add_hline(y=90, line_dash="dash", line_color="gray", opacity=0.5)
            ...
            st.plotly_chart(fig_rolling, use_container_width=True, key=f"{prefix}_rolling")
    ...
    with tab2:
        ...
        for manager in selected_managers:
            ...
            close_losses = manager_data[
                (manager_data["win"] == 0) &
                (manager_data["points_left_on_bench"] > abs(manager_data["team_points"] - manager_data["opponent_points"]))
            ]
            ...
            missed_wins.append({
                ...
                "Missed Win %": (games_could_win / total_losses * 100) if total_losses > 0 else 0
            })
        ...
        with st.expander("📊 Detailed Missed Opportunities", expanded=False):
            ...
            display_missed['Avg Bench Points'] = display_missed['Avg Bench Points'].round(2)
            display_missed['Total Bench Points'] = display_missed['Total Bench Points'].round(2)
    ...
    with tab3:
        ...
        for manager in selected_managers:
            ...
            efficiency_stats.append({
                ...
                "Win Efficiency": wins["lineup_efficiency"].mean() if len(wins) > 0 else 0,
                "Loss Efficiency": losses["lineup_efficiency"].mean() if len(losses) > 0 else 0,
                "Weeks at 100%": len(manager_data[manager_data["lineup_efficiency"] >= 99.5]),
                "Weeks Below 90%": len(manager_data[manager_data["lineup_efficiency"] < 90])
            })
