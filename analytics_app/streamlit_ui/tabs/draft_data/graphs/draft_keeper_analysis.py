...
def display_draft_keeper_analysis(prefix=""):
    ...
    keeper_avg_ppd = keeper_data['points_per_dollar'].mean() if not keeper_data.empty else 0
    drafted_avg_ppd = drafted_data['points_per_dollar'].mean() if not drafted_data.empty else 0
    ...
    with col1:
        st.metric("Keeper Avg SPAR/$", f"{keeper_avg_roi:.2f}",
                  delta=f"{keeper_avg_roi - drafted_avg_roi:+.2f} vs drafted")

    with col2:
        st.metric("Drafted Avg SPAR/$", f"{drafted_avg_ppd:.2f}",
                  delta=f"{keeper_avg_ppd - drafted_avg_ppd:+.2f} difference")

    with col3:
        st.metric("Keeper %", f"{keeper_pct:.1f}%",
                  delta=f"{keeper_count} of {total_count}")

    with col4:
        st.metric("Keeper Avg SPAR", f"{keeper_avg_value:.1f}",
    ...
    for pos in keeper_data['yahoo_position'].unique():
        ...
        pos_comparison = pd.concat([pos_comparison, pd.DataFrame({
            'Position': [pos],
    ...
    top_keepers = top_keepers.rename(columns={'value': 'spar', 'roi': 'spar_per_dollar'})
    ...
    st.dataframe(top_keepers, hide_index=True, use_container_width=True, column_config=column_config)
    ...
    worst_keepers = worst_keepers.rename(columns={'value': 'spar', 'roi': 'spar_per_dollar'})
    worst_keepers = worst_keepers.round(1)

    st.dataframe(worst_keepers, hide_index=True, use_container_width=True, column_config=column_config)
