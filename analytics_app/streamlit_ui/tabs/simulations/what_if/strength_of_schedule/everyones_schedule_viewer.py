...
class EveryonesScheduleViewer:
    ...
    def display(self):
        ...
        st.caption("What would your record be if you played someone else's schedule? Shows cross-schedule performance.")
        ...
        with col3:
            include_regular_season = st.checkbox(
                "Regular",
                value=True,
                key="everyones_schedule_include_regular_season"
            )
        ...
        with col4:
            include_postseason = st.checkbox(
                "Playoffs",
                value=False,
                key="everyones_schedule_include_postseason"
            )
    ...
    def display_record(self, filtered_df):
        ...
        opponent_names = sorted([re.match(r"w_vs_(.+)_sched", c).group(1) for c in win_cols])
        ...
        st.caption("💡 **How to read:** Each cell shows a manager's record if they played that column's schedule. Highlighted cells show your actual schedule performance.")
    ...
    def display_win_percentage(self, filtered_df):
        ...
        st.caption("💡 **How to read:** Percentage of games won if each manager played that schedule. Higher is better.")
