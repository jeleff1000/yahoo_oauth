#!/usr/bin/env python3
"""
Season team stats data loader - aggregates player data by manager, year, and position.
...
def load_season_team_data(
    ...
    """
    Load season team stats - aggregates player data by manager, year, and position.
    Filters out Unrostered players.
    ...
    Returns:
        DataFrame with season team stats grouped by manager, year, and position
    """
    try:
        where = [
            "manager IS NOT NULL",
        ...
        # Query aggregating by manager, year, fantasy_position
        sql = f"""
            SELECT
                manager,
                year,
            ...
            FROM {T['player']}
            {where_sql}
            GROUP BY manager, year, fantasy_position
            ORDER BY year DESC, fantasy_position, manager
        """
        ...
        st.error(f"Failed to load season team data: {e}")
        return None
