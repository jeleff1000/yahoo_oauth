#!/usr/bin/env python3
"""
Weekly team stats data loader - aggregates player data by manager and position.
Filters out Unrostered players and groups by manager, year, week, and fantasy_position.
"""
...
def load_weekly_team_data(
    ...
    Returns:
        DataFrame with weekly team stats grouped by manager and position (no row limit)
    """
    try:
        where = [
            "manager IS NOT NULL",
        ...
        cols_str = ", ".join(WEEKLY_TEAM_COLUMNS)

        # Query with grouping by manager, year, week, fantasy_position
        sql = f"""
            SELECT {cols_str}
            FROM {T['player']}
            {where_sql}
            GROUP BY manager, year, week, cumulative_week, fantasy_position
            ORDER BY year DESC, week DESC, fantasy_position, manager
        """
        ...
        st.error(f"Failed to load weekly team data: {e}")
        return None
