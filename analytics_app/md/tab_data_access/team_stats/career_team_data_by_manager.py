#!/usr/bin/env python3
"""
Career team stats data loader - aggregates by MANAGER ONLY (all positions combined).
"""
...
def load_career_team_data_by_manager(
    include_regular_season: bool = True,
    include_playoffs: bool = True,
    include_consolation: bool = False,
):
    """
    Load career team stats aggregated by manager only (all positions combined).
    ...
    Returns:
        DataFrame with career team stats grouped by manager only
    """
    try:
        where = [
            "manager IS NOT NULL",
        ...
        # Query aggregating by manager only
        sql = f"""
            SELECT
                manager,
        ...
        st.error(f"Failed to load career team data by manager: {e}")
        return None
