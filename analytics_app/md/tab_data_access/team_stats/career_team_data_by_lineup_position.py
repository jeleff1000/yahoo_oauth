#!/usr/bin/env python3
"""
Career team stats data loader - aggregates player data by manager and lineup_position across all years.
...
def load_career_team_data_by_lineup_position(
    ...
    Returns:
        DataFrame with career team stats grouped by manager and lineup_position
    """
    try:
        where = [
            "manager IS NOT NULL",
        ...
        # Query aggregating by manager and lineup_position
        sql = f"""
            SELECT
                manager,
                lineup_position,
            ...
            FROM {T['player']}
            {where_sql}
            GROUP BY manager, lineup_position
            ORDER BY lineup_position, manager
        """
        ...
        st.error(f"Failed to load career team data by lineup position: {e}")
        return None
