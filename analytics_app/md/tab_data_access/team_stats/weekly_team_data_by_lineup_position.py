#!/usr/bin/env python3
"""
Weekly team stats data loader - aggregates player data by manager and lineup_position.
Groups by lineup_position (WR1, WR2, RB1, etc.) instead of fantasy_position.
"""
...
def load_weekly_team_data_by_lineup_position(
    ...
    """
    Load weekly team stats - aggregates player data by manager and lineup_position.
    ...
    Returns:
        DataFrame with weekly team stats grouped by manager and lineup_position
    """
    try:
        where = [
            "manager IS NOT NULL",
        ...
        # Query aggregating by manager, year, week, lineup_position
        sql = f"""
            SELECT
                manager,
                year,
                ...
                SUM(CAST(carries AS DOUBLE)) as carries,
                ...
                SUM(CAST(rushing_fumbles_lost AS DOUBLE)) as rushing_fumbles_lost,
                ...
                SUM(CAST(def_tds AS DOUBLE)) as def_tds,
            ...
            FROM {T['player']}
            {where_sql}
            GROUP BY manager, year, week, cumulative_week, lineup_position
            ORDER BY year DESC, week DESC, lineup_position, manager
        """
        ...
        st.error(f"Failed to load weekly team data by lineup position: {e}")
        return None
