...
# =========================
# One simulation (FLIPPED: wins = easier week, i.e., LOWER opponent_points)
# =========================
def simulate_once(points_by_mgr_week, managers, n_weeks):
    """
    points_by_mgr_week[(manager, week)] = opponent_points manager actually faced.
    We count a 'win' when your opponent_points < counterpart's opponent_points
    (meaning you drew an easier opponent that week).
    Tie-breaker favors LOWER total opponent_points across weeks.
    """
    sched = schedule_nxN(managers, n_weeks, validate=False)
    wins = defaultdict(int)          # 'wins' == easy weeks
    cum_opp = defaultdict(float)     # sum of opponent_points (lower is easier)
    ...
    # Rank by more easy-weeks (wins), then LOWER total opponent_points
    ladder = [(m, wins[m], cum_opp[m]) for m in managers]
    ladder.sort(key=lambda x: (-x[1], x[2], x[0]))  # more wins better; lower cum_opp better
...
# =========================
# Main processing (manager subject; metric = opponent_points)
# =========================
def process_opp_points_ease_shuffle():
    ...
    # Columns (opp_shuffle_* now reflect EASE — higher wins = easier schedule)
    seed_cols = [f"opp_shuffle_{i}_seed" for i in range(1, 11)]
    win_cols  = [f"opp_shuffle_{w}_win"  for w in range(0, 15)]
    summary_cols = [
        "opp_shuffle_avg_wins",      # expected easy weeks (higher = easier schedule)
        "opp_shuffle_avg_seed",      # better (lower) with easier schedule
        "opp_shuffle_avg_playoffs",  # P(seed ≤ 6) under ease metric
        "opp_shuffle_avg_bye",       # P(seed ≤ 2)
    ]
    ...
    for year, df_year in raw.groupby('year', sort=True):
        ...
        # Use opponent_points as the metric (what you faced)
        df_year_points = (
            df_year[['manager', 'week', 'opponent_points']]
            .rename(columns={'opponent_points': 'metric_points'})
            .copy()
        )
        ...
        for wk in range(1, last_wk + 1):
            ...
            points = {(r['manager'], int(r['week'])): float(r['metric_points'])
            ...
            # Write snapshot on manager rows (subject = manager)
            ...
            for idx in rows:
                ...
                for wv in range(0, min(n_weeks, 14) + 1):
                    raw.at[idx, f"opp_shuffle_{wv}_win"] = round(
                        100.0 * win_hists[mgr][wv] / win_den[mgr], 2
                    )
                for s in range(1, min(n_managers, 10) + 1):
                    raw.at[idx, f"opp_shuffle_{s}_seed"] = round(
    ...
    # =========================
    # Summaries (regular rows only)
    # =========================
    print("\nComputing opponent-points (EASE) summary statistics...")

    for c in summary_cols:
        raw_all[c] = np.nan

    # Expected easy weeks
    win_prob_cols = [c for c in [f"opp_shuffle_{w}_win" for w in range(0, 15)] if c in raw_all.columns]
    if win_prob_cols:
        ...
        if not df_reg.empty:
            ...
            raw_all.loc[regular_mask, "opp_shuffle_avg_wins"] = vals
            raw_all.loc[regular_mask & ~has_data, "opp_shuffle_avg_wins"] = np.nan

    # Expected seed (lower = easier schedule)
    seed_prob_cols = [c for c in [f"opp_shuffle_{s}_seed" for s in range(1, 11)] if c in raw_all.columns]
    if seed_prob_cols:
        ...
        if not df_reg.empty:
            ...
            vals = np.round((df_reg.fillna(0.0).to_numpy(dtype=float) @ weights) / 100.0, 2)
            has_data = df_reg.notna().any(axis=1)
            raw_all.loc[regular_mask, "opp_shuffle_avg_seed"] = vals
            raw_all.loc[regular_mask & ~has_data, "opp_shuffle_avg_seed"] = np.nan

    # "Playoff/bye" odds under this metric (optional but kept for symmetry)
    playoff_cols = [c for c in [f"opp_shuffle_{s}_seed" for s in range(1, 7)] if c in raw_all.columns]
    if playoff_cols:
        df_reg = raw_all.loc[regular_mask, playoff_cols]
        if not df_reg.empty:
            vals = df_reg.fillna(0.0).sum(axis=1)
            has_data = df_reg.notna().any(axis=1)
            raw_all.loc[regular_mask, "opp_shuffle_avg_playoffs"] = vals
            raw_all.loc[regular_mask & ~has_data, "opp_shuffle_avg_playoffs"] = np.nan

    bye_cols = [c for c in ["opp_shuffle_1_seed", "opp_shuffle_2_seed"] if c in raw_all.columns]
    if bye_cols:
        df_reg = raw_all.loc[regular_mask, bye_cols]
        if not df_reg.empty:
            vals = df_reg.fillna(0.0).sum(axis=1)
            has_data = df_reg.notna().any(axis=1)
            raw_all.loc[regular_mask, "opp_shuffle_avg_bye"] = vals
            raw_all.loc[regular_mask & ~has_data, "opp_shuffle_avg_bye"] = np.nan

    # =========================
    # Simple per-week difficulty indicators (no sim)
    # =========================
    if "opponent_points" in raw_all.columns:
        idx = regular_mask & raw_all["opponent_points"].notna()
        grouped = raw_all.loc[idx].groupby(["year", "week"])["opponent_points"]
        # 1 = hardest opponent that week (highest opponent_points)
        raw_all.loc[idx, "opp_pts_week_rank"] = grouped.rank(method="min", ascending=False)
        # Percentile: 100 = hardest
        raw_all.loc[idx, "opp_pts_week_pct"] = 100.0 * grouped.rank(pct=True, ascending=True)

    # =========================
    # Lock postseason rows to final regular-week snapshot
    # =========================
    print("Locking postseason values (opp-points ease sim)...")
    post_mask = (raw_all['is_playoffs'] == 1) | (raw_all['is_consolation'] == 1)
    if post_mask.any():
        cols_to_copy = [c for c in (seed_cols + win_cols + summary_cols
    ...
    print("\nProcessing complete!")
    print(f"Updated {len(raw_all)} records with opponent-points (EASE) shuffle metrics")

# =========================
# Main
# =========================
if __name__ == "__main__":
    process_opp_points_ease_shuffle()
