...
def calculate_player_stats(
    ...
    Columns added/enriched:
        - headshot_url (backfilled from non-null values for same player_id)
        - fantasy_points
    ...
    # =========================================================
    # Step 0: Backfill headshot_url from same player across rows
    # =========================================================
    print("\nStep 0: Backfilling missing headshot_url values...")

    # Fill missing headshot_url by propagating non-null values from same player
    # Uses player_id (unified yahoo/nfl player ID) as the join key
    if "headshot_url" in df.columns and "player_id" in df.columns:
        # Find the first non-null headshot_url for each player_id
        player_headshots = (
            df
            .filter(pl.col("headshot_url").is_not_null())
            .group_by("player_id")
            .agg(pl.col("headshot_url").first().alias("canonical_headshot_url"))
        )

        # Join back to fill missing values
        df = df.join(player_headshots, on="player_id", how="left")

        # Backfill: use original if exists, otherwise use canonical from other rows
        df = df.with_columns(
            pl.coalesce(["headshot_url", "canonical_headshot_url"]).alias("headshot_url")
        )

        # Drop the temporary column
        df = df.drop("canonical_headshot_url")

        filled_count = df.filter(pl.col("headshot_url").is_not_null()).shape[0]
        print(f"  Backfilled headshot_url: {filled_count:,} players now have headshot URLs")
    else:
        if "headshot_url" not in df.columns:
            print(f"  Warning: headshot_url column not found, skipping backfill")
        if "player_id" not in df.columns:
            print(f"  Warning: player_id column not found yet, skipping backfill")

    # =========================================================
    # Step 0.5: Calculate is_rostered and is_started flags
    # =========================================================
    ...
    # ALWAYS recalculate is_rostered based on manager column
    # A player is rostered if they have a manager assigned (not "Unrostered")
    # This ensures is_rostered=0 when manager="Unrostered" (even if is_rostered existed in input data)
    if "manager" in df.columns:
        df = df.with_columns(
        ...
        print(f"  Set is_rostered=0 for Unrostered players, is_rostered=1 for rostered players")
    else:
        # Fallback if manager column doesn't exist
    ...
    # This is league-wide configuration (not player-specific)
