...
def left_join_transactions_player(left: pd.DataFrame, right: pd.DataFrame, import_cols: list[str]) -> pd.DataFrame:
    ...
    # Build player_week if missing and yahoo_player_id + cumulative_week exist
    for df in (L, R):
        if 'player_week' not in df.columns and {'yahoo_player_id', 'cumulative_week'}.issubset(df.columns):
            # ensure cumulative_week is string-safe
...
def enrich_transactions_with_player_data(
    ...
    # Derive cumulative_week if missing.  Some pipelines may not
    # populate this column on initial import.  We derive it from
    # year/week when possible to allow joining on a consistent key.
    for df_name, df in (("player", player), ("transactions", transactions)):
        if 'cumulative_week' not in df.columns:
            ...
            try:
                ...
                print(f"  WARNING: Could not convert cumulative_week to numeric in {df_name}: {e}")
