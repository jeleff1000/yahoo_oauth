...
def merge_transactions_to_player(
    ...
    # Convert all merge keys to string for consistent merging
    print("  Normalizing data types for merge keys...")
    for key in merge_keys:
        if key in player_df.columns:
            player_df[key] = player_df[key].astype(str)
        if key in cumulative_faab_df.columns:
            cumulative_faab_df[key] = cumulative_faab_df[key].astype(str)
...
def import_transactions_to_player(
    ...
    player_updated = merge_transactions_to_player(player, cumulative_faab, transactions)
    new_cols = set(player_updated.columns) - player_before_cols

    print(f"  Added {len(new_cols)} new columns: {sorted(new_cols)}")

    # Count how many player records got FAAB data
    if 'max_faab_bid_to_date' in player_updated.columns:
        records_with_faab = (player_updated['max_faab_bid_to_date'] > 0).sum()
        print(f"  Player-week records with cumulative FAAB > 0: {records_with_faab:,} ({records_with_faab/len(player_updated)*100:.1f}%)")

    if 'faab_bid' in player_updated.columns:
        records_with_transaction = (player_updated['faab_bid'] > 0).sum()
        print(f"  Player-week records with transaction FAAB > 0: {records_with_transaction:,} ({records_with_transaction/len(player_updated)*100:.1f}%)")

    # Statistics
    stats = {
        'player_records_before': len(player),
