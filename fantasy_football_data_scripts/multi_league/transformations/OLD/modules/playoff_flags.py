...
def detect_playoffs_by_seed(df: pd.DataFrame, settings_dir: str = None) -> pd.DataFrame:
    ...
    if settings_dir is None:
        ...
        settings_path = find_league_settings_directory()
        if settings_path:
    ...
    for year, year_settings in settings_by_year.items():
        ...
        playoff_start = year_settings['playoff_start_week']
...
@ensure_normalized
def mark_playoff_rounds(df: pd.DataFrame) -> pd.DataFrame:
    """
...
def mark_champions_and_sackos(df: pd.DataFrame, settings_dir: str = None) -> pd.DataFrame:
    ...
    """
    # Import playoff_bracket module with proper path handling
    try:
        from modules import playoff_bracket
    except ImportError:
        try:
            import playoff_bracket
        except ImportError:
            safe_print("  [ERROR] Could not import playoff_bracket module")
            # Return df with empty champion/sacko columns if import fails
    ...
    df = playoff_bracket.simulate_playoff_brackets(df, settings_dir=settings_dir)
