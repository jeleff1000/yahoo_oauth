...
def enrich_draft_with_player_stats(
    ...
    for key in group_keys:
        if key in draft_df.columns:
            draft_df[key] = draft_df[key].astype(str)
        if key in metrics_df.columns:
            metrics_df[key] = metrics_df[key].astype(str)

    # Log data types for debugging
    print(f"  draft_df yahoo_player_id type: {draft_df['yahoo_player_id'].dtype}")
    print(f"  metrics_df yahoo_player_id type: {metrics_df['yahoo_player_id'].dtype}")
...
def main():
    ...
    # Locate draft data
    draft_dir = ctx.draft_data_directory
    draft_file = draft_dir / "draft_data_all_years.parquet"

    if not draft_file.exists():
        print(f"ERROR: Draft file not found: {draft_file}")
        print("Expected: draft_data_all_years.parquet")
        return 1
