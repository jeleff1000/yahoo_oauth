...
def load_league_settings(year: int, settings_dir: Optional[str] = None, df: Optional[pd.DataFrame] = None) -> Dict:
    """
...
def simulate_playoff_brackets(df: pd.DataFrame, settings_dir: Optional[str] = None) -> pd.DataFrame:
    ...
    for year in years:
        ...
        # Load settings for this year (pass df for league_id extraction)
        settings = load_league_settings(year, settings_dir, df=df)
