#!/usr/bin/env python3
"""
Career team stats data loader - aggregates player data by manager and position across all years.
...
def load_career_team_data(
    include_regular_season: bool = True,
    include_playoffs: bool = True,
    include_consolation: bool = False
):
    """
    Load career team stats - aggregates player data by manager and position across all years.
    Filters out Unrostered players.
    ...
    Returns:
        DataFrame with career team stats grouped by manager and position
    """
    try:
        where = [
            "manager IS NOT NULL",
        ...
        # Query aggregating by manager and fantasy_position
        sql = f"""
            SELECT
                manager,
                fantasy_position,
                ...
                SUM(CAST(points AS DOUBLE)) as total_points,
                COUNT(*) as games_played,
                SUM(CAST(points AS DOUBLE)) / NULLIF(COUNT(*), 0) as ppg_all_time,
            ...
            FROM {T['player']}
            {where_sql}
            GROUP BY manager, fantasy_position
            ORDER BY fantasy_position, manager
        """
        ...
        st.error(f"Failed to load career team data: {e}")
        return None
