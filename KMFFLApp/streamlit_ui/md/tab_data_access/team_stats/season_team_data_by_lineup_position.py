#!/usr/bin/env python3
"""
Season team stats data loader - aggregates player data by manager, year, and lineup_position.
...
def load_season_team_data_by_lineup_position(
    year: int | None = None,
    ...
    """
    Load season team stats - aggregates player data by manager, year, and lineup_position.
    Filters out Unrostered players.
    ...
    Returns:
        DataFrame with season team stats grouped by manager, year, and lineup_position
    """
    try:
        where = [
            "manager IS NOT NULL",
        ...
        # Query aggregating by manager, year, lineup_position
        sql = f"""
            SELECT
                manager,
                year,
            ...
            FROM {T['player']}
            {where_sql}
            GROUP BY manager, year, lineup_position
            ORDER BY year DESC, lineup_position, manager
        """
        ...
        st.error(f"Failed to load season team data by lineup position: {e}")
        return None
