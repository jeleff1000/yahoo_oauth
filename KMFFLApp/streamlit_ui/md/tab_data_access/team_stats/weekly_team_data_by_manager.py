#!/usr/bin/env python3
"""
Weekly team stats data loader - aggregates by MANAGER ONLY (all positions combined).
"""
...
def load_weekly_team_data_by_manager(
    year: int | None = None,
    week: int | None = None,
    ...
    """
    Load weekly team stats aggregated by manager only (all positions combined).
    ...
    Returns:
        DataFrame with weekly team stats grouped by manager only
    """
    try:
        where = [
            "manager IS NOT NULL",
        ...
        # Query aggregating by manager only (all positions combined)
        sql = f"""
            SELECT
                manager,
                year,
            ...
            FROM {T['player']}
            {where_sql}
            GROUP BY manager, year, week, cumulative_week
            ORDER BY year DESC, week DESC, manager
        """
        ...
        st.error(f"Failed to load weekly team data by manager: {e}")
        return None
