...
class PlayoffBracketsViewer:
    ...
    def _extract_matches_from_round(self, round_df: pd.DataFrame) -> List[Dict]:
        ...
        for _, row in round_df.iterrows():
            ...
            pts_a = float(manager_row.get('team_points', 0)) if pd.notna(manager_row.get('team_points')) else 0.0
            pts_b = float(opponent_row.get('team_points', 0)) if opponent_row is not None and pd.notna(opponent_row.get('team_points')) else float(row.get('opponent_points', 0))

            seed_a = manager_row.get('final_playoff_seed')
            seed_b = opponent_row.get('final_playoff_seed') if opponent_row is not None else None
        ...
        matches.sort(key=lambda m: (m['seedA'] if m['seedA'] else 99, m['seedB'] if m['seedB'] else 99))

        return matches

    def _detect_bye_teams(self, rounds_data: List[Tuple[str, List[Dict]]]) -> Dict[str, int]:
        """
    ...
    def _build_bracket_structure(self, rounds_data: List[Tuple[str, List[Dict]]]) -> Dict:
    ...
    def _calculate_y_positions(self, rounds_data: List[Tuple[str, List[Dict]]],
                               bracket_structure: Dict) -> List[List[float]]:
        ...
        for r_idx in range(1, len(rounds_data)):
            ...
            if r_idx - 1 in bracket_structure['flows']:
                ...
                for next_idx in range(len(current_matches)):
                    feeding_matches = [curr_idx for curr_idx, target_idx in flows.items() if target_idx == next_idx]

                    if feeding_matches:
                        feeding_y_values = [prev_y[idx] for idx in feeding_matches if idx < len(prev_y)]
                        current_y.append(sum(feeding_y_values) / len(feeding_y_values))
                    else:
                        current_y.append((next_idx - (len(current_matches) - 1) / 2) * v_gap * 2)
            else:
    ...
    def _draw_match_box(self, fig: go.Figure, x: float, y_center: float, match: Dict):
        ...
        team_a_style = 'font-weight: 700; font-size: 13px;' if winner == team_a else 'font-size: 12px;'
        team_b_style = 'font-weight: 700; font-size: 13px;' if winner == team_b else 'font-size: 12px;'

        fig.add_annotation(
            x=seed_x + 0.5, y=(mid + top) / 2,
        ...
        score_x = right - 0.15
        score_a_style = 'font-weight: 700; font-size: 13px;' if winner == team_a else 'font-size: 12px; color: #9E9E9E;'
        score_b_style = 'font-weight: 700; font-size: 13px;' if winner == team_b else 'font-size: 12px; color: #9E9E9E;'

        fig.add_annotation(
            x=score_x, y=(mid + top) / 2,
    ...
    def _draw_connector_lines(self, fig: go.Figure, x: float, y: float,
                             next_x: float, next_y: float, paired_y: Optional[float] = None):
        ...
        if paired_y is not None:
            fig.add_shape(
                type='line',
                x0=x + box_w / 2 + 0.6, y0=y,
                x1=x + box_w / 2 + 0.6, y1=paired_y,
                line=dict(color=self.COLORS['line'], width=line_width)
            )
        ...
        fig.add_shape(
            type='line',
            x0=x + box_w / 2 + 0.6, y0=next_y,
            x1=next_x - box_w / 2, y1=next_y,
            line=dict(color=self.COLORS['line'], width=line_width)
        )
    ...
    def create_bracket(self, year: int) -> go.Figure:
        ...
        # Extract rounds
        round_order = year_df.groupby('playoff_round')['week'].min().sort_values().index.tolist()
            ...
            for bye_team, bye_seed in bye_teams.items():
                for match_idx, match in enumerate(rounds_data[1][1]):
                    if bye_team in (match['teamA'], match['teamB']):
                        ...
                        fig.add_shape(
                            type='rect',
                            x0=x_bye - 1.2, y0=y_bye - 0.6,
                            x1=x_bye + 1.2, y1=y_bye + 0.6,
                        ...
                        fig.add_shape(
                            type='line',
                            x0=x_bye + 1.2, y0=y_bye,
                            x1=x_positions[1] - self.SPACING['box_width'] / 2, y1=y_bye,
                            line=dict(color=self.COLORS['bye_line'], width=2.5, dash='dot')
                        )

        # Draw all rounds
        for r_idx, ((round_name, matches), x, ys) in enumerate(zip(rounds_data, x_positions, y_positions)):
            # Round label
            fig.add_annotation(
                x=x, y=max(ys) + self.SPACING['vertical_gap'] * 1.5,
                text=f"<b style='font-size: 18px;'>{round_name}</b>",
                showarrow=False,
                font=dict(size=18, color=self.COLORS['text_primary'], family='Arial Black')
            )
                ...
                if r_idx < len(rounds_data) - 1:
                    ...
                    if match_idx in flows:
                        ...
                        for other_idx, other_target in flows.items():
                            if other_target == next_match_idx and other_idx != match_idx:
                                if other_idx < len(ys):
        ...
        if rounds_data:
            ...
            champ_seed = final_match['seedA'] if champion == final_match['teamA'] else final_match['seedB']

            final_x = x_positions[-1]
            final_y = y_positions[-1][0]

            # Champion box position (moved further right)
            champ_x_start = final_x + self.SPACING['box_width'] / 2 + self.SPACING['champion_offset']
        ...
        y_range = [min(all_y) - self.SPACING['vertical_gap'] * 2,
                   max(all_y) + self.SPACING['vertical_gap'] * 2.5] if all_y else [-10, 10]

        fig.update_layout(
            xaxis=dict(
                showgrid=False, zeroline=False, showticklabels=False,
                range=[-5, x_positions[-1] + 6]  # Extended range for bye indicators and champion box
            ),
            yaxis=dict(
                showgrid=False, zeroline=False, showticklabels=False,
                range=y_range, scaleanchor=None
            ),
            plot_bgcolor=self.COLORS['background'],
            showlegend=False,
            margin=dict(l=20, r=20, t=80, b=20),
            height=max(700, int((max(all_y) - min(all_y) + self.SPACING['vertical_gap'] * 5) * 55)) if all_y else 700,
