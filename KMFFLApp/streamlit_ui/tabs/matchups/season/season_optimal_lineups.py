...
class SeasonOptimalLineupsViewer:
    ...
    def _prepare_display_data(self) -> pd.DataFrame:
        ...
        lucky_wins = df[df['win'] & df['low_efficiency']].groupby(['manager', 'year']).size()
        display_df['lucky_wins'] = display_df.apply(
            lambda row: lucky_wins.get((row['manager'], row['year']), 0),
            axis=1
        )

        # Calculate Missed Wins: losses that would have been wins
        missed_wins = df[(~df['win']) & df['optimal_win']].groupby(['manager', 'year']).size()
        display_df['missed_wins'] = display_df.apply(
            lambda row: missed_wins.get((row['manager'], row['year']), 0),
            axis=1
        )
