...
class WeeklyMatchupStatsViewer:
    ...
    def _format_playoff_result(row) -> str:
        ...
        consolation_round = row['consolation_round'] if 'consolation_round' in row.index else ''
    ...
    def _prepare_display_data(self) -> pd.DataFrame:
        ...
        df['win'] = df['win'] == 1
        df['is_playoffs'] = df['is_playoffs'] == 1 if 'is_playoffs' in df.columns else False
        ...
        columns_to_show = [
            'year', 'week', 'manager', 'team_name', 'opponent',
            'team_points', 'opponent_points', 'margin', 'win',
    ...
    def _calculate_stats(self, df: pd.DataFrame) -> dict:
        ...
        best_win = df[df['Result'] == True].nlargest(1, 'Margin') if wins > 0 else None
        worst_loss = df[df['Result'] == False].nsmallest(1, 'Margin') if losses > 0 else None
    ...
    def _calculate_current_streak(self, df: pd.DataFrame) -> dict:
        ...
        for i in range(1, len(sorted_df)):
            ...
            if (streak_type == 'W' and current_result) or (streak_type == 'L' and not current_result):
                streak_count += 1
            else:
                break
    ...
    def _render_quick_stats(self, stats: dict):
        ...
        with col2:
            ...
            delta_color = "normal" if stats['win_pct'] >= 50 else "inverse"
            st.metric(
                "Record",
                record_str,
    ...
    def _render_enhanced_table(self, df: pd.DataFrame, prefix: str):
        ...
        column_config = {
            ...
            'PF': st.column_config.NumberColumn(
                'PF',
                help='Points For (scored)',
                format='%.2f',
                width='small'
            ),
            'PA': st.column_config.NumberColumn(
                'PA',
                help='Points Against (allowed)',
                format='%.2f',
                width='small'
            ),
