...
class WeeklyOptimalLineupsViewer:
    ...
    def display(self, prefix=""):
        ...
        required_columns = [
            'manager', 'opponent', 'week', 'year',
            'team_points', 'opponent_points', 'optimal_points',
            'win', 'margin'
        ]
    ...
    def _prepare_display_data(self) -> pd.DataFrame:
        ...
        df['lineup_efficiency'] = (df['team_points'] / df['optimal_points'] * 100).round(1)
        ...
        columns_to_show = [
            ...
            # Actual performance
            'team_points', 'opponent_points', 'win', 'margin',
            # Optimal performance
            'optimal_points', 'bench_points', 'lineup_efficiency',
            # What if analysis
            'optimal_win', 'optimal_margin', 'outcome_change',
            # Context
            'playoff_round',
        ]
        ...
        numeric_cols = [
            'Actual', 'Opp Pts', 'Margin', 'Optimal', 'Bench Pts',
            'Efficiency', 'Opt Margin'
        ]
    ...
    def _calculate_stats(self, df: pd.DataFrame) -> dict:
        ...
        # Outcome changes
        would_have_won = len(df[(df['Result'] == False) & (df['Opt Result'] == True)])
        lucky_wins = len(df[(df['Result'] == True) & (df['Efficiency'] < 80)])
    ...
    def _render_enhanced_table(self, df: pd.DataFrame, prefix: str):
        ...
        column_config = {
            ...
            'Opt Result': st.column_config.TextColumn(
                'Opt Result',
                help='Result with optimal lineup',
                width='small'
            ),
            'Opt Margin': st.column_config.NumberColumn(
                'Opt Margin',
                help='Optimal lineup margin',
                format='%.2f',
                width='small'
            ),
