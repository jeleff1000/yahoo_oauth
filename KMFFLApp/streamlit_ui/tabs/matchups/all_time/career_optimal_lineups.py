...
def display_alltime_optimal_lineup(matchup_df: pd.DataFrame):
    ...
    need_m = {
        ...
        "is_playoffs", "is_consolation", "optimal_points",
...
def _prepare_display_data(matchup_df: pd.DataFrame) -> pd.DataFrame:
    ...
    display_df['avg_team_points'] = (display_df['team_points'] / display_df['games']).round(2)
    display_df['avg_optimal_points'] = (display_df['optimal_points'] / display_df['games']).round(2)
    display_df['avg_lost_points'] = (display_df['lost_points'] / display_df['games']).round(2)
...
def _render_enhanced_table(df: pd.DataFrame):
    ...
    column_config = {
        ...
        'Total Actual': st.column_config.NumberColumn(
            'Total Actual',
            help='Total actual points',
            format='%.2f',
            width='small'
        ),
        'Total Optimal': st.column_config.NumberColumn(
            'Total Optimal',
            help='Total optimal points',
            format='%.2f',
            width='small'
        ),
...
def _render_quick_stats(stats: dict):
    ...
    with col8:
        avg_lost_per_game = stats['total_lost_points'] / stats['total_games'] if stats['total_games'] > 0 else 0
        st.metric(
            "Avg Lost/Game",
