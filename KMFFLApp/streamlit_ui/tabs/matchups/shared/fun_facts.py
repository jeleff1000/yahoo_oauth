...
def get_weekly_fun_facts(matchup_df: pd.DataFrame) -> List[str]:
    ...
    try:
        ...
        if 'team_points' in matchup_df.columns and 'manager' in matchup_df.columns and 'year' in matchup_df.columns and 'week' in matchup_df.columns:
        ...
        # Closest margin
        if 'margin' in matchup_df.columns and 'week' in matchup_df.columns and 'year' in matchup_df.columns:
        ...
        if 'optimal_points' in matchup_df.columns and 'team_points' in matchup_df.columns and 'win' in matchup_df.columns and 'week' in matchup_df.columns:
            wins_df = matchup_df[matchup_df['win'] == True].copy()
        ...
        if 'team_points' in matchup_df.columns and 'loss' in matchup_df.columns and 'week' in matchup_df.columns:
        ...
        # Lowest scoring win
        if 'team_points' in matchup_df.columns and 'win' in matchup_df.columns and 'week' in matchup_df.columns:
        ...
        if 'manager_proj_score' in matchup_df.columns and 'opponent_proj_score' in matchup_df.columns and 'win' in matchup_df.columns:
            matchup_df['proj_underdog'] = matchup_df['manager_proj_score'] < matchup_df['opponent_proj_score']
            matchup_df['proj_diff'] = matchup_df['opponent_proj_score'] - matchup_df['manager_proj_score']
            underdogs = matchup_df[(matchup_df['proj_underdog'] == True) & (matchup_df['win'] == True)]
        ...
        if 'optimal_points' in matchup_df.columns and 'team_points' in matchup_df.columns and 'win' in matchup_df.columns:
            matchup_df['perfect_lineup'] = (matchup_df['optimal_points'] - matchup_df['team_points']).abs() < 0.5
            perfect_wins = matchup_df[(matchup_df['perfect_lineup'] == True) & (matchup_df['win'] == True)]
        ...
        if 'total_matchup_score' in matchup_df.columns and 'week' in matchup_df.columns:
            max_combined_row = matchup_df.loc[matchup_df['total_matchup_score'].idxmax()]
            facts.append(
        ...
        # Most dominant week
        if 'teams_beat_this_week' in matchup_df.columns and 'margin' in matchup_df.columns and 'week' in matchup_df.columns:
            matchup_df['dominance_score'] = matchup_df['teams_beat_this_week'] * matchup_df['margin']
            if matchup_df['dominance_score'].max() > 0:
                max_dominance_row = matchup_df.loc[matchup_df['dominance_score'].idxmax()]
                facts.append(
        ...
        if 'manager' in matchup_df.columns and 'team_points' in matchup_df.columns:
            avg_by_manager = matchup_df.groupby('manager')['team_points'].mean().sort_values(ascending=False)
        ...
        if 'manager' in matchup_df.columns and 'team_points' in matchup_df.columns:
            std_by_manager = matchup_df.groupby('manager')['team_points'].std().sort_values()
        ...
        if 'manager' in matchup_df.columns and 'win' in matchup_df.columns and 'year' in matchup_df.columns and 'week' in matchup_df.columns:
...
def get_season_fun_facts(matchup_df: pd.DataFrame) -> List[str]:
    ...
    try:
        ...
        season_df = matchup_df.groupby(['manager', 'year']).agg({
            'team_points': 'sum',
            'opponent_points': 'sum',
            'win': 'sum',
            'margin': 'sum'
        ...
        if 'manager' in season_df.columns and 'team_points' in season_df.columns:
            manager_seasons = season_df.groupby('manager')['team_points'].agg(['mean', 'std', 'count'])
            qualified = manager_seasons[manager_seasons['count'] >= 3]  # At least 3 seasons
        ...
        if 'manager' in season_df.columns:
            avg_by_manager = season_df.groupby('manager')['team_points'].mean().sort_values(ascending=False)
            if not avg_by_manager.empty:
                top_manager = avg_by_manager.index[0]
                top_avg = avg_by_manager.iloc[0]
                facts.append(
...
def get_career_fun_facts(matchup_df: pd.DataFrame) -> List[str]:
    ...
    try:
        ...
        career_df = matchup_df.groupby('manager').agg({
            'team_points': 'sum',
            'opponent_points': 'sum',
            'win': 'sum',
            'loss': 'sum',
        ...
        facts.append(f"The league has scored {total_points:,.0f} total points across all-time matchups")
