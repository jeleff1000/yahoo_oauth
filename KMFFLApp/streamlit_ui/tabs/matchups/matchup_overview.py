...
def display_matchup_overview(df_dict, prefix=""):
    ...
    <h2>⚔️ Manager Matchups</h2>
    <p style="margin: 0.5rem 0 0 0;">Analyze head-to-head performance and matchup history.</p>
    </div>
    """, unsafe_allow_html=True)

    # Add a new "Visualize" tab next to Career to surface manager graphs in-place
                    ...
                    if graph_imports:
                        ...
                        with graph_tabs[0]:
                            if 'scoring_trends' in graph_imports:
                                ...
                                st.warning("⚠️ Scoring Trends graph is not available")
                        ...
                        with graph_tabs[1]:
                            if 'win_percentage' in graph_imports:
                                ...
                                st.warning("⚠️ Win Percentage graph is not available")
                        ...
                        with graph_tabs[2]:
                            if 'power_rating' in graph_imports:
                                ...
                                st.warning("⚠️ Power Rating graph is not available")
                        ...
                        with graph_tabs[3]:
                            if 'scoring_distribution' in graph_imports:
                                ...
                                st.warning("⚠️ Score Distribution graph is not available")
                        ...
                        with graph_tabs[4]:
                            if 'margin_of_victory' in graph_imports:
                                ...
                                st.warning("⚠️ Margin of Victory graph is not available")
                        ...
                        with graph_tabs[5]:
                            if 'lineup_efficiency' in graph_imports:
                                ...
                                st.warning("⚠️ Lineup Efficiency graph is not available")
                        ...
                        with graph_tabs[6]:
                            if 'playoff_vs_regular' in graph_imports:
                                ...
                                st.warning("⚠️ Playoff Performance graph is not available")
                        ...
                        with graph_tabs[7]:
                            if 'strength_of_schedule' in graph_imports:
                                ...
                                st.warning("⚠️ Strength of Schedule graph is not available")
                    else:
                        st.warning("⚠️ Manager graphs are currently unavailable. Some graph modules could not be loaded.")
            else:
                st.error(f"❌ {sub_tab_name} Matchup Data not found.")