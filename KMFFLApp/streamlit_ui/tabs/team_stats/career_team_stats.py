#!/usr/bin/env python3
"""Career Team Stats Overview - shows aggregated stats by manager and/or position."""
...
class CareerTeamViewer:
    ...
    def display(self):
        ...
        # Gradient header
        render_gradient_header(
            title="Career Team Stats",
            subtitle="View all-time records and career achievements",
            icon="🏆"
        )
    ...
    def display_by_position(self):
        ...
        with filter_col:
            ...
            if not st.session_state.get("career_team_include_regular_season", True):
                active_filters.append("Regular Season: Excluded")
            if not st.session_state.get("career_team_include_playoffs", True):
                active_filters.append("Playoffs: Excluded")
            if st.session_state.get("career_team_include_consolation", False):
            ...
            if active_filters:
                if st.button("🔄 Clear All Filters", key="career_team_clear_all", use_container_width=True):
                    # Clear session state for filters
                    for key in list(st.session_state.keys()):
                        if key.startswith("career_team"):
        ...
        with data_col:
            ...
            with stat_tabs[0]:
                ...
                if df.empty:
                    ...
                    # Export button
                    self._render_export_button(df, "career_basic_stats")
            ...
            with stat_tabs[1]:
                ...
                if df.empty:
                    ...
                    # Export button
                    self._render_export_button(df, "career_advanced_stats")
    ...
    def display_by_lineup_position(self):
        ...
        if self.team_data_by_lineup_position.empty:
            render_empty_state(
                title="No Team Data Available",
                message="Career team data by lineup position has not been loaded yet.",
                icon="📭"
            )
            return
        ...
        with filter_col:
            ...
            render_info_box(
                "Filter by lineup position (WR1, WR2, RB1, etc.) to see all-time manager performance",
                icon="ℹ️"
            )
            ...
            if 'lineup_position' in self.team_data_by_lineup_position.columns:
                ...
                selected_lineup_pos = st.selectbox(
                    ...
                    index=0,
                    key="career_team_lineup_position",
                    label_visibility="collapsed"
                )
            else:
            ...
            # Clear all filters button
            if active_filters:
                if st.button("🔄 Clear All Filters", key="career_team_lp_clear_all", use_container_width=True):
                    for key in list(st.session_state.keys()):
                        if key.startswith("career_team"):
                            del st.session_state[key]
                    st.rerun()
        ...
        with data_col:
            ...
            with stat_tabs[0]:
                ...
                if df.empty:
                    ...
                    # Export button
                    self._render_export_button(df, "career_lineup_pos_stats")
    ...
    def _get_basic_stats_by_lineup_pos(self, data: pd.DataFrame, lineup_position: str) -> pd.DataFrame:
        ...
        # Select columns to display (career granularity - already aggregated by manager+lineup_position)
        display_cols = []
        for col in ['manager', 'lineup_position', 'total_points', 'manager_spar', 'games_played', 'ppg_all_time']:
        ...
        if 'total_points' in result.columns:
            result['total_points'] = result['total_points'].round(2)
        if 'manager_spar' in result.columns:
            result['manager_spar'] = result['manager_spar'].round(2)
        if 'ppg_all_time' in result.columns:
            result['ppg_all_time'] = result['ppg_all_time'].round(2)
    ...
    def _display_quick_metrics_for_lineup_data(self, data: pd.DataFrame, lineup_position: str):
        ...
        try:
            ...
            # Calculate metrics
            total_points = data["total_points"].sum() if "total_points" in data.columns else 0
            avg_points = data["total_points"].mean() if "total_points" in data.columns else 0
            total_records = len(data)
            unique_managers = data["manager"].nunique() if "manager" in data.columns else 0

            # Display metrics in grid
            metrics = [
                {"label": "Career Points", "value": f"{total_points:,.0f}"},
    ...
    def display_by_manager(self):
        ...
        with filter_col:
            ...
            render_info_box(
                "All-time aggregated stats for each manager across all positions",
                icon="ℹ️"
            )
            ...
            # Clear all filters button
            if active_filters:
                if st.button("🔄 Clear All Filters", key="career_team_mgr_clear_all", use_container_width=True):
                    for key in list(st.session_state.keys()):
                        if key.startswith("career_team"):
                            del st.session_state[key]
                    st.rerun()
        ...
        with data_col:
            ...
            with stat_tabs[0]:
                ...
                if df.empty:
                    ...
                    # Export button
                    self._render_export_button(df, "career_manager_stats")
