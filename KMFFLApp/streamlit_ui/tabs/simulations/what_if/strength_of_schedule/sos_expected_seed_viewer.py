...
def _select_week(base_df):
    ...
    mode = st.radio("Selection Mode", ["Today's Date", "Specific Week"],
                    horizontal=True, key="opp_exp_seed_mode")
...
def _render_expected_seed(base_df, year, week):
    ...
    seed_cols = [c for c in all_cols if c.startswith("opp_shuffle_") and c.endswith("_seed")]

    if not seed_cols:
        st.info("No valid opp shuffle seed cols.")
        return

    # Extract week numbers efficiently
    week_numbers = {c: int(c.split('_')[2]) for c in seed_cols
                    if len(c.split('_')) >= 3 and c.split('_')[2].isdigit()}
    ...
    st.caption("Probability of finishing at each playoff seed based on opponent strength.")
...
@st.fragment
def display_opp_expected_seed(matchup_data_df):
    """Main entry point."""
