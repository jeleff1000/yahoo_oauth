...
def _select_week(base_df):
    ...
    for idx, (col, name) in enumerate(zip(cols, modes)):
        with col:
            ...
            if st.button(name, key=f"opp_exp_record_btn_{idx}", use_container_width=True,
...
def _render_expected_record(base_df, year, week):
    ...
    shuffle_cols = [c for c in all_cols
                    if c.startswith("opp_shuffle_") and c.endswith("_win")]
    ...
    for c in shuffle_cols:
        ...
        if len(parts) >= 3 and parts[2].isdigit():
            col_week = int(parts[2])
            if col_week <= week:
                valid_cols.append((col_week, c))

    if not valid_cols:
        st.info("No opp shuffle win cols.")
        return
    ...
    # Vectorized renaming
    rename_map = {c: f"{int(c.split('_')[2])}-{week - int(c.split('_')[2])}"
                  for c in shuffle_cols}
    ...
    ordered = sorted(record_cols, key=lambda c: int(c.split('-')[0]) if '-' in c else 0, reverse=True)
    ...
    if 'wins_to_date' in week_slice.columns:
        ...
        wins_series = (week_slice[['manager', 'wins_to_date']]
                      .drop_duplicates(subset=['manager'])
    ...
    st.subheader("📊 Expected Record Distribution")
    st.caption("Probability of achieving each win-loss record based on opponent strength.")
    ...
    st.caption("💡 **How to read:** Each column shows % chance of that record. Higher % in winning records = easier schedule.")


@st.fragment
def display_opp_expected_record(matchup_data_df):
    """Main entry point."""
