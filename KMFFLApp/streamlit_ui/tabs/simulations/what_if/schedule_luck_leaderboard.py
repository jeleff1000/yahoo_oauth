...
def display_schedule_luck_leaderboard(matchup_data_df: pd.DataFrame = None):
    ...
    st.caption("Quick snapshot of who's been luckiest/unluckiest. See Schedule Shuffles tab for details.")
    ...
    df['wins_vs_shuffle_wins'] = pd.to_numeric(df['wins_vs_shuffle_wins'], errors='coerce')
    ...
    luck_df = season_data.groupby('manager').agg({
    ...
    with col2:
        ...
        for i, row in luck_df.tail(3).iloc[::-1].iterrows():
            ...
            emoji = "😭" if rank == len(luck_df) else "😔" if rank == len(luck_df)-1 else "😕"
            st.markdown(f"{emoji} **{row['Manager']}** — {luck_val:+.1f} wins")

    # Simple explanation
    st.markdown("---")
    st.caption("**Luck** = Actual Wins − Expected Wins from 100K simulations. Positive = lucky schedule, Negative = tough schedule.")
