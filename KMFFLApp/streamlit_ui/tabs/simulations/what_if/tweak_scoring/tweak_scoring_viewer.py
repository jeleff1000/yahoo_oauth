...
class TweakScoringViewer(WeeklyMatchupDataViewer):
    ...
    def display(self):
        ...
        with col3:
            show_regular_season = st.checkbox("Regular Season", value=True,
                                              key="regular_season_checkbox")
        with col4:
            show_postseason = st.checkbox("Postseason", value=False,
                                          key="postseason_checkbox")

        col5, col6 = st.columns(2)
        with col5:
            tweak_scores_flag = st.checkbox("Tweak Scores", key="tweak_scores_checkbox")
        with col6:
            shuffle_schedule_flag = st.checkbox("Shuffle Schedule",
                                                key="shuffle_schedule_checkbox")
        ...
        if st.button("Simulate"):
            ...
            if tweak_scores_flag:
                std_dev_df = calculate_std_dev(filtered_df, selected_year,
                                               show_regular_season, show_postseason)
            ...
            seeds_df = filtered_df.loc[latest_week_idx, ['year', 'manager', 'playoff_seed_to_date']]

            # Aggregate stats
            aggregated_df = (filtered_df.groupby(['year', 'manager'], as_index=False)
            .agg({
                'team_points': 'sum',
            ...
            if selected_year == "All Years":
                actual_df = aggregated_df[['year', 'manager', 'Seed', 'Wins', 'Losses', 'Points']].copy()
                sim_df = aggregated_df[['year', 'manager', 'Sim_Seed', 'Sim_W', 'Sim_L', 'Sim_Points']].copy()
                ...
                sim_df = sim_df.sort_values(by=['Sim_Seed']).set_index(['year', 'manager'])
                sim_df.index.names = ['Year', 'Manager']
            else:
                actual_df = aggregated_df[['manager', 'Seed', 'Wins', 'Losses', 'Points']].copy()
                sim_df = aggregated_df[['manager', 'Sim_Seed', 'Sim_W', 'Sim_L', 'Sim_Points']].copy()
        ...
        if 'tweak_scoring_actual_df' in st.session_state and 'tweak_scoring_sim_df' in st.session_state:
            st.markdown("---")
