...
def _select_week(base_df):
    ...
    mode = st.radio("Selection Mode", ["Today's Date", "Specific Week"],
                    horizontal=True, key="exp_record_mode")
...
def _render_expected_record(base_df, year, week):
    ...
    shuffle_cols = [c for c in all_cols
                    if c.startswith("shuffle_") and c.endswith("_win")]
    ...
    # Vectorized renaming
    rename_map = {c: f"{int(c.split('_')[1])}-{week - int(c.split('_')[1])}"
                  for c in shuffle_cols}
    ...
    st.caption("Probability of achieving each win-loss record based on schedule shuffles.")
    ...
    st.caption("💡 **How to read:** Each column shows % chance of that record. Higher % in winning records = stronger team.")
