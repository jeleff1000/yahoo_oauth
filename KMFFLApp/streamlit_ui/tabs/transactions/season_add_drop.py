...
def display_season_add_drop(
    ...
    col_names = ['manager', 'year', 'num_adds', 'faab_spent', 'spar_added_managed',
                 'points_added_managed', 'avg_ppg_managed', 'avg_spar_efficiency',
                 'avg_pts_per_faab', 'avg_score', 'avg_rank_at', 'avg_rank_after']
    if 'transaction_score' in adds.columns:
    ...
    drop_col_names = ['manager', 'year', 'num_drops', 'spar_dropped_total', 'points_dropped_total',
                      'avg_ppg_dropped']
    if 'transaction_score' in drops.columns:
    ...
    season_agg['net_spar'] = season_agg['spar_added_managed'] - season_agg['spar_dropped_total']
    season_agg['net_points'] = season_agg['points_added_managed'] - season_agg['points_dropped_total']
    season_agg['rank_improvement'] = season_agg['avg_rank_at'] - season_agg['avg_rank_after']
    ...
    with tab1:
        ...
        if grade_filter != "All":
            filtered = filtered[filtered['Grade'].str.contains(grade_filter, case=False)]
        ...
        display_cols = [
            'manager', 'year', 'Grade', 'total_score', 'total_transactions', 'faab_spent',
            'net_spar', 'spar_added_managed', 'spar_dropped_total',
            'avg_spar_efficiency', 'rank_improvement'
        ]
        ...
        st.dataframe(
            ...
            column_config={
                'total_score': st.column_config.NumberColumn("Score", format="%+.0f", help="Weighted score (timing, FAAB efficiency, regret)"),
                'total_transactions': st.column_config.NumberColumn("Moves", format="%d"),
                'faab_spent': st.column_config.NumberColumn("FAAB", format="$%.0f"),
                'net_spar': st.column_config.NumberColumn("Net SPAR", format="%.1f", help="Managed SPAR Added - Total SPAR Dropped"),
                'spar_added_managed': st.column_config.NumberColumn("SPAR Added", format="%.1f", help="SPAR from adds while rostered"),
                'spar_dropped_total': st.column_config.NumberColumn("SPAR Dropped", format="%.1f", help="Total SPAR opportunity cost"),
                'avg_spar_efficiency': st.column_config.NumberColumn("SPAR/FAAB", format="%.2f", help="SPAR per FAAB dollar"),
                'rank_improvement': st.column_config.NumberColumn("Rank Δ", format="%.1f"),
            }
        )

        csv = filtered[display_cols].to_csv(index=False)
        st.download_button("📥 Export", csv, f"season_add_drops_{year_filter}.csv", "text/csv",
    ...
    with tab2:
        ...
        with col1:
            ...
            mgr_spar = season_agg.groupby('manager')['net_spar'].sum().sort_values(ascending=False).head(
                10).reset_index()
            fig = px.bar(mgr_spar, x='manager', y='net_spar',
        ...
        with col2:
            ...
            fig = px.bar(top_eff, x='manager', y='avg_spar_efficiency',
        ...
        yearly = season_agg.groupby('year').agg({
            'net_spar': 'mean',
            'net_points': 'mean',
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=yearly['year'], y=yearly['net_spar'],
                                 mode='lines+markers', name='Avg Net SPAR',
                                 line=dict(color='green', width=3)))
        fig.add_trace(go.Scatter(x=yearly['year'], y=yearly['total_transactions'],
                                 mode='lines+markers', name='Avg Transactions',
    ...
    with tab3:
        ...
        with col1:
            ...
            st.dataframe(
                best_seasons[['Rank', 'manager', 'year', 'net_points', 'total_transactions', 'faab_spent']],
                hide_index=True,
                use_container_width=True,
                column_config={
                    'net_points': st.column_config.NumberColumn("Net Pts", format="%.1f"),
                    'total_transactions': st.column_config.NumberColumn("Moves", format="%d"),
                    'faab_spent': st.column_config.NumberColumn("FAAB", format="$%.0f"),
                }
            )
        ...
        with col2:
            ...
            efficient = season_agg[season_agg['avg_spar_efficiency'].notna()].nlargest(10, 'avg_spar_efficiency')[
                ['manager', 'year', 'avg_spar_efficiency', 'net_spar', 'faab_spent']
            ]
            efficient['Rank'] = range(1, len(efficient) + 1)
            st.dataframe(
                efficient[['Rank', 'manager', 'year', 'avg_spar_efficiency', 'net_spar', 'faab_spent']],
                hide_index=True,
                use_container_width=True,
                column_config={
                    'avg_spar_efficiency': st.column_config.NumberColumn("SPAR/FAAB", format="%.2f", help="SPAR per FAAB dollar"),
                    'net_spar': st.column_config.NumberColumn("Net SPAR", format="%.1f"),
                    'faab_spent': st.column_config.NumberColumn("FAAB", format="$%.0f"),
                }
            )
