...
def display_season_trade_data(transaction_df, player_df, draft_history_df):
    ...
    season_agg = trade_df.groupby(['manager', 'year']).agg({
    ...
    season_agg.columns = [
        'manager', 'year', 'total_trades', 'total_net_spar',
        'avg_net_per_trade', 'total_spar_acquired', 'total_spar_traded',
        'avg_trade_score', 'avg_rank_improvement'
    ]

    season_agg['trade_efficiency'] = season_agg['total_net_spar'] / season_agg['total_trades']
    season_agg['win_rate'] = trade_df.groupby(['manager', 'year'])['net_spar'].apply(
        lambda x: (x > 0).sum() / len(x) * 100
    ).values

    # Grades based on NET SPAR
    season_agg['Grade'] = season_agg['total_net_spar'].apply(
        lambda x: "🥇 Elite" if pd.notna(x) and x > 200
        else "🥈 Great" if pd.notna(x) and x > 100
        else "🥉 Good" if pd.notna(x) and x > 50
        else "📊 Average" if pd.notna(x) and x > 0
    ...
    with tab1:
        ...
        with col3:
            st.metric("Avg NET SPAR/Season", f"{season_agg['total_net_spar'].mean():.1f}")
        with col4:
            st.metric("Avg Win Rate", f"{season_agg['win_rate'].mean():.1f}%")
        with col5:
            st.metric("Avg Efficiency", f"{season_agg['trade_efficiency'].mean():.1f} SPAR/trade")
        ...
        with col4:
            sort_by = st.selectbox("Sort by", ["NET SPAR ↓", "Efficiency", "Trades", "Win Rate"],
        ...
        display_cols = [
            'manager', 'year', 'Grade', 'total_trades',
            'total_net_spar', 'avg_net_per_trade', 'trade_efficiency',
            'win_rate', 'total_spar_acquired', 'total_spar_traded',
            'avg_trade_score', 'avg_rank_improvement'
        ]
        ...
        st.dataframe(
            ...
            column_config={
                ...
                'total_net_spar': st.column_config.NumberColumn("NET SPAR", format="%.1f", help="Total NET SPAR for season"),
                'avg_net_per_trade': st.column_config.NumberColumn("Avg NET", format="%.1f", help="Average NET SPAR per trade"),
                'trade_efficiency': st.column_config.NumberColumn("Efficiency", format="%.2f", help="NET SPAR per trade"),
        ...
        st.download_button("📥 Export", csv, f"season_trades_{year_filter}.csv", "text/csv", key="season_trade_export")
    ...
    with tab2:
        ...
        with col1:
            ...
            fig.add_trace(go.Bar(
                ...
                name='Avg NET SPAR',
                marker_color=['#2ca02c' if x > 0 else '#d62728' for x in yearly['Avg NET SPAR']],
        ...
        with col2:
            ...
            fig = px.histogram(season_agg, x='win_rate',
        ...
        mgr_profile = season_agg.groupby('manager').agg({
            'total_trades': 'sum',
            'total_net_spar': 'sum',
            'trade_efficiency': 'mean',
            'win_rate': 'mean'
        }).reset_index().sort_values('total_net_spar', ascending=False).head(10)

        fig = px.scatter(mgr_profile, x='total_trades', y='total_net_spar',
                         size=[abs(x)+10 for x in mgr_profile['trade_efficiency']],
                         color='win_rate',
                         hover_data=['manager'],
                         title="Top 10 Managers: Volume vs Results",
                         color_continuous_scale='RdYlGn',
                         labels={'total_trades': 'Total Trades', 'total_net_spar': 'Career NET SPAR'},
                         size_max=35)
        st.plotly_chart(fig, use_container_width=True)
    ...
    with tab3:
        ...
        with col1:
            ...
            st.dataframe(
                best[['Rank', 'manager', 'year', 'total_net_spar', 'total_trades', 'win_rate']],
                hide_index=True,
                use_container_width=True,
                column_config={
                    ...
                    'year': st.column_config.TextColumn("Year", width="small"),
                    'total_net_spar': st.column_config.NumberColumn("NET SPAR", format="%.1f"),
                    'total_trades': st.column_config.NumberColumn("Trades", format="%d", width="small"),
        ...
        with col2:
            ...
            win_rate_seasons = season_agg[season_agg['total_trades'] >= 3].nlargest(10, 'win_rate')[
                ['manager', 'year', 'win_rate', 'total_net_spar', 'total_trades']
            ].copy()
            win_rate_seasons['Rank'] = range(1, len(win_rate_seasons) + 1)
            st.dataframe(
                win_rate_seasons[['Rank', 'manager', 'year', 'win_rate', 'total_net_spar', 'total_trades']],
                hide_index=True,
                use_container_width=True,
                column_config={
                    'Rank': st.column_config.NumberColumn("#", width="small"),
                    ...
                    'year': st.column_config.TextColumn("Year", width="small"),
                    'win_rate': st.column_config.NumberColumn("Win %", format="%.1f"),
                    'total_net_spar': st.column_config.NumberColumn("NET SPAR", format="%.1f", width="small"),
                    'total_trades': st.column_config.NumberColumn("Trades", format="%d", width="small"),
                }
            )
        ...
        with col1:
            ...
            efficient = season_agg[season_agg['total_trades'] >= 3].nlargest(10, 'trade_efficiency')[
                ['manager', 'year', 'trade_efficiency', 'total_net_spar', 'total_trades']
            ].copy()
            efficient['Rank'] = range(1, len(efficient) + 1)
            st.dataframe(
                efficient[['Rank', 'manager', 'year', 'trade_efficiency', 'total_net_spar']],
                hide_index=True,
                use_container_width=True,
                column_config={
                    'Rank': st.column_config.NumberColumn("#", width="small"),
                    'manager': st.column_config.TextColumn("Manager"),
                    'year': st.column_config.TextColumn("Year", width="small"),
                    'trade_efficiency': st.column_config.NumberColumn("SPAR/Trade", format="%.2f", help="NET SPAR per trade"),
                    'total_net_spar': st.column_config.NumberColumn("NET SPAR", format="%.1f"),
                }
            )
        ...
        with col2:
            ...
            st.dataframe(
                most_active[['Rank', 'manager', 'year', 'total_trades', 'total_net_spar', 'win_rate']],
                hide_index=True,
                use_container_width=True,
                column_config={
                    'Rank': st.column_config.NumberColumn("#", width="small"),
                    'manager': st.column_config.TextColumn("Manager"),
                    'year': st.column_config.TextColumn("Year", width="small"),
                    'total_trades': st.column_config.NumberColumn("Trades", format="%d", help="Number of trades in the season"),
                    'total_net_spar': st.column_config.NumberColumn("NET SPAR", format="%.1f", width="small"),
                    'win_rate': st.column_config.NumberColumn("Win %", format="%.1f", width="small"),
                }
            )
