...
from md.data_access import run_query, load_players_season_data
...
class StreamlitSeasonPlayerDataViewer:
    """
    Season Player Data Viewer - Aggregated by Player + Year
    """

    def __init__(self) -> None:
        # Pagination managers for each tab
        self.pagination_basic = PaginationManager("season_basic")
        self.pagination_advanced = PaginationManager("season_advanced")
        self.pagination_matchup = PaginationManager("season_matchup")
    ...
    def _full_query(
        ...
        """
        Query season data with all filters applied at the SQL level.
        """
        df = load_players_season_data(
            position=None,
            player_query=(player_q or None),
    ...
    def _reset_pagination_on_sort_change(self, key_prefix: str):
        ...
        if key_prefix == "season_basic":
            st.session_state["season_basic_offset"] = 0
        elif key_prefix == "season_advanced":
            st.session_state["season_advanced_offset"] = 0
        elif key_prefix == "season_matchup":
            st.session_state["season_matchup_offset"] = 0
    ...
    def display_sortable_dataframe(self, df: pd.DataFrame, key_prefix: str):
        ...
        sort_col = st.session_state.get(f"{key_prefix}_sort_col", "points")
    ...
    def display(self) -> None:
        st.title("Season Player Data Viewer (Aggregated by Player + Year)")
        st.info(
            "Player/Manager search bars and all selectors filter source rows **before** aggregation. "
        ...
        def display_filters(tab_index):
            ...
            with col4:
                opponent_nfl_team_values = st.multiselect(
                    ...
                    key=f"season_opponent_nfl_team_value_{tab_index}"
                )
            ...
            with col1:
                start_year = st.number_input(
                    "Start Year", min_value=1999, max_value=current_year, value=1999, step=1, key=f"season_start_year_{tab_index}"
                )
            with col2:
                end_year = st.number_input(
                    "End Year", min_value=1999, max_value=current_year, value=current_year, step=1, key=f"season_end_year_{tab_index}"
                )

            # Row 4: Rostered Only, Started Only (toggles)
            col1, col2 = st.columns([1, 1])
            with col1:
                rostered_only = st.toggle("Rostered Only", value=False, key=f"season_show_rostered_{tab_index}")
            with col2:
                started_only = st.toggle("Started Only", value=False, key=f"season_show_started_{tab_index}")
        ...
        with tabs[0]:
            ...
            if df is None or df.empty:
                st.warning("No season data matches your filters.")
            else:
                st.info(f"📊 Showing {len(df):,} player-seasons (aggregated by player + year)")
                basic_df = get_basic_stats(df, filters["nfl_position"])
                self.display_sortable_dataframe(basic_df, "season_basic")
        ...
        with tabs[1]:
            ...
            if df is None or df.empty:
                ...
                st.info(f"📊 Showing {len(df):,} player-seasons (aggregated by player + year)")
                adv_df = get_advanced_stats(df, filters["nfl_position"])
                self.display_sortable_dataframe(adv_df, "season_advanced")
        ...
        with tabs[2]:
            ...
            if df is None or df.empty:
                ...
                st.info(f"📊 Showing {len(df):,} player-seasons (aggregated by player + year)")
                CombinedMatchupStatsViewer(df).display(prefix="season_matchup")
