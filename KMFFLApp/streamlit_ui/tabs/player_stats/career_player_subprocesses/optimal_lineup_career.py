...
class OptimalLineupCareerViewer:
    """Display optimal lineups for all-time career stats based on various scoring criteria"""
            ...
            query = f"""
                WITH max_week_data AS (
                    SELECT
                        ...
                        year,
                        manager,
                        ROW_NUMBER() OVER (PARTITION BY NFL_player_id, position ORDER BY points DESC) as rn
                    FROM {T['player']}
                ...
                SELECT
                    ...
                    AVG(p.points) as ppg_all_time,
                    MAX(p.points) as max_points,
                    SUM(CASE WHEN p.league_wide_optimal_player = 1 THEN 1 ELSE 0 END) as times_optimal,
                    COUNT(DISTINCT p.year || '-' || p.week) as games_played,
                    COUNT(DISTINCT p.year) as seasons_played,
                ...
                WHERE p.year >= {start_year}
                  ...
                  AND p.position != ''
                  {season_type_filter}
                GROUP BY p.NFL_player_id, p.position
                HAVING COUNT(DISTINCT p.year || '-' || p.week) > 0
            """
    ...
    def _render_optimal_lineup(self, start_year: int, end_year: int, criteria: str, include_playoffs: bool = False):
        ...
        # Load data
        with st.spinner("Building all-time optimal lineup..."):
