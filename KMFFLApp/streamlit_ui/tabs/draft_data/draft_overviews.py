...
def display_draft_overview(draft_data):
    ...
    non_keeper_data['yahoo_position'] = non_keeper_data['yahoo_position'].astype(position_order)
    ...
    agg_dict = {
        'cost': ['mean', 'max', 'min', 'median', 'count']
    }
    ...
    agg_df = non_keeper_data.groupby(['yahoo_position', 'rank_num']).agg(agg_dict).reset_index()

    # Flatten column names
    if 'spar' in non_keeper_data.columns:
        agg_df.columns = ['yahoo_position', 'rank_num', 'avg_cost', 'max_cost', 'min_cost', 'median_cost', 'times_drafted', 'avg_spar', 'median_spar']
        # Calculate SPAR efficiency
        agg_df['spar_per_dollar'] = (agg_df['avg_spar'] / agg_df['avg_cost']).round(2)
    else:
        agg_df.columns = ['yahoo_position', 'rank_num', 'avg_cost', 'max_cost', 'min_cost', 'median_cost', 'times_drafted']
    agg_df['position_rank'] = agg_df['yahoo_position'].astype(str) + agg_df['rank_num'].astype(str)
    ...
    columns_to_display = ['position_rank', 'yahoo_position', 'avg_cost', 'max_cost', 'min_cost', 'median_cost', 'times_drafted']
    ...
    st.dataframe(
        ...
        column_config={
            ...
            'median_spar': st.column_config.NumberColumn('Median SPAR', format='%.1f'),
            'spar_per_dollar': st.column_config.NumberColumn('SPAR/$', format='%.2f', help='Average SPAR per dollar spent')
        } if 'avg_spar' in agg_df.columns else None
    )
    ...
    if not keeper_data.empty:
        keeper_data['yahoo_position'] = keeper_data['yahoo_position'].astype(position_order)
        ...
        keeper_agg = keeper_data.groupby(['yahoo_position', 'rank_num']).agg(keeper_agg_dict).reset_index()

        # Flatten column names
        if 'spar' in keeper_data.columns:
            keeper_agg.columns = ['yahoo_position', 'rank_num', 'avg_cost', 'max_cost', 'min_cost', 'median_cost', 'times_kept', 'avg_spar', 'median_spar']
            # Calculate SPAR efficiency
            keeper_agg['spar_per_dollar'] = (keeper_agg['avg_spar'] / keeper_agg['avg_cost']).round(2)
        else:
            keeper_agg.columns = ['yahoo_position', 'rank_num', 'avg_cost', 'max_cost', 'min_cost', 'median_cost', 'times_kept']

        keeper_agg['position_rank'] = keeper_agg['yahoo_position'].astype(str) + keeper_agg['rank_num'].astype(str)
        keeper_agg['yahoo_position'] = keeper_agg['yahoo_position'].astype(position_order)
        keeper_agg = keeper_agg.sort_values(['yahoo_position', 'rank_num'])

        # Display columns based on available metrics
        keeper_columns = [
            'position_rank', 'yahoo_position', 'avg_cost', 'max_cost', 'min_cost', 'median_cost', 'times_kept'
        ]
        ...
        st.dataframe(
            ...
            column_config={
                ...
                'median_spar': st.column_config.NumberColumn('Median SPAR', format='%.1f'),
                'spar_per_dollar': st.column_config.NumberColumn('SPAR/$', format='%.2f', help='Average SPAR per dollar spent')
            } if 'avg_spar' in keeper_agg.columns else None
        )
    else:
