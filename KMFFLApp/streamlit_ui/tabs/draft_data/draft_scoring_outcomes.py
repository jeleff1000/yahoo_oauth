...
def display_scoring_outcomes(draft_data):
    ...
    if 'cost' in draft_data.columns:
        draft_data = draft_data[pd.to_numeric(draft_data['cost'], errors='coerce').fillna(0) > 0]
    ...
    positions = [p for p in sorted(draft_data['yahoo_position'].dropna().unique())
    ...
    with col5:
        include_drafted = st.checkbox("Include Drafted", value=True, help="Include non-keeper picks")

    with col6:
        include_keepers = st.checkbox("Include Keepers", value=True, help="Include keeper picks")
    ...
    # Show filter summary
    st.info(f"📊 Showing **{len(filtered_data):,}** picks from **{len(draft_data):,}** total")
...
def calculate_rankings(df):
    ...
    def calc_cost_rank(group):
        ...
        if year in ['2014', '2015']:
            ...
            return group['cost'].rank(method='first', ascending=False, na_option='bottom')
    ...
    cost_rank_series = cost_rank_series.fillna(max_rank + 1 if not pd.isna(max_rank) else 1).astype(int)
    ...
    points_rank_series = points_rank_series.fillna(max_rank + 1 if not pd.isna(max_rank) else 1).astype(int)
    ...
    ppg_rank_series = ppg_rank_series.fillna(max_rank + 1 if not pd.isna(max_rank) else 1).astype(int)
    ...
    if 'manager_spar' in df.columns:
        ...
        df['value_score'] = df['value_score'].replace([np.inf, -np.inf], np.nan).fillna(0)
...
def display_performance_charts(filtered_data):
    ...
    top_surprises = plot_data.nlargest(20, 'rank_diff')[
        ['player', 'year', 'yahoo_position', 'cost_rank', 'points_rank', 'rank_diff', 'manager']
    ]
...
def display_performance_table(filtered_data, allowed_positions):
    ...
    table_data = table_data.sort_values(
        ['year', 'pos_order', 'cost_rank'],
        ascending=[False, True, True]
    )

    # Select columns for display
    display_cols = [
        'year', 'player', 'yahoo_position', 'manager',
        'cost', 'cost_rank', 'points', 'points_rank',
        'season_ppg', 'ppg_rank', 'rank_diff'
    ]
    ...
    display_df = table_data[[col for col in display_cols if col in table_data.columns]].copy()

    # Build column names dynamically
    col_names = [
        'Year', 'Player', 'Pos', 'Manager',
        'Cost', 'Cost Rank', 'Total Pts', 'Pts Rank',
        'PPG', 'PPG Rank', 'Value (+/-)'
    ]
    ...
    st.caption("💡 **Value (+/-)**: Positive = outperformed draft position, Negative = underperformed | **Manager SPAR**: Actual value while rostered | **SPAR/$**: Manager SPAR per dollar (higher = better value)")
...
def display_performance_insights(filtered_data):
    ...
    with col1:
        ...
        for idx, row in top_value.iterrows():
            ...
            spar_text = f"SPAR/$: {row['value_score']:.2f}" if 'value_score' in row and pd.notna(row['value_score']) else ""
            st.caption(f"${row['cost']:.0f} → {row['points']:.1f} pts  +{row['rank_diff']} spots  {spar_text}")
    ...
    pos_stats = analysis_data.groupby('yahoo_position').agg({
        'rank_diff': ['mean', 'std'],
        'value_score': 'mean',
        'cost': 'mean',
        'points': 'mean'
    ...
    pos_stats.columns = ['Avg Rank Diff', 'Std Dev', 'Avg Value', 'Avg Cost', 'Avg Points']
    ...
    **Std Dev**: Consistency (lower = more predictable)
    ...
    if 'manager' in analysis_data.columns and analysis_data['manager'].nunique() > 1:
        ...
        mgr_stats = analysis_data.groupby('manager').agg({
            'rank_diff': 'mean',
            'value_score': 'mean',
            'player': 'count'
        }).round(2)
